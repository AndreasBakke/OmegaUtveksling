...
```js
const ***REMOVED***GoogleAuth} = require('google-auth-library');

/**
* Instead of specifying the type of client you'd like to use (JWT, OAuth2, etc)
* this library will automatically choose the right client based on the environment.
*/
async function main() ***REMOVED***
  const auth = new GoogleAuth(***REMOVED***
    scopes: 'https://www.googleapis.com/auth/cloud-platform'
***REMOVED***);
  const client = await auth.getClient();
  const projectId = await auth.getProjectId();
  const url = `https://dns.googleapis.com/dns/v1/projects/$***REMOVED***projectId}`;
  const res = await client.request(***REMOVED*** url });
...
``` js
const ***REMOVED***OAuth2Client} = require('google-auth-library');
...
async function main() ***REMOVED***
  ...
  const res = await oAuth2Client.request(***REMOVED***url});
  console.log(res.data);
...
*/
function getAuthenticatedClient() ***REMOVED***
  return new Promise((resolve, reject) => ***REMOVED***
    ...
    const authorizeUrl = oAuth2Client.generateAuthUrl(***REMOVED***
      access_type: 'offline',
  ...
  ***REMOVED***);
    ...
    const server = http
      .createServer(async (req, res) => ***REMOVED***
        try ***REMOVED***
          if (req.url.indexOf('/oauth2callback') > -1) ***REMOVED***
            // acquire the code from the querystring, and close the web server.
            const qs = new url.URL(req.url, 'http://localhost:3000')
              .searchParams;
            const code = qs.get('code');
            console.log(`Code is $***REMOVED***code}`);
    ...
    ***REMOVED***)
      .listen(3000, () => ***REMOVED***
        ...
        open(authorizeUrl, ***REMOVED***wait: false}).then(cp => cp.unref());
    ***REMOVED***);
    destroyer(server);
***REMOVED***);
}
...
client.on('tokens', (tokens) => ***REMOVED***
  if (tokens.refresh_token) ***REMOVED***
...
```js
const oAuth2Client = new OAuth2Client(***REMOVED***
...
``` js
const ***REMOVED***JWT} = require('google-auth-library');
const keys = require('./jwt.keys.json');

async function main() ***REMOVED***
  const client = new JWT(***REMOVED***
    email: keys.client_email,
    key: keys.private_key,
    scopes: ['https://www.googleapis.com/auth/cloud-platform'],
***REMOVED***);
  const url = `https://dns.googleapis.com/dns/v1/projects/$***REMOVED***keys.project_id}`;
  const res = await client.request(***REMOVED***url});
  console.log(res.data);
}
...
```js
const ***REMOVED***auth} = require('google-auth-library');

// load the environment variable with our keys
const keysEnvVar = process.env['CREDS'];
if (!keysEnvVar) ***REMOVED***
...
``` js
const ***REMOVED***auth, Compute} = require('google-auth-library');

async function main() ***REMOVED***
  const client = new Compute(***REMOVED***
...
***REMOVED***);
  ...
  const url = `https://storage.googleapis.com/storage/v1/b?project=$***REMOVED***projectId}`;
  const res = await client.request(***REMOVED*** url });
  console.log(res.data);
}
```
...
```js
const ***REMOVED***ExternalAccountClient} = require('google-auth-library');
...
```javascript
const ***REMOVED*** GoogleAuth, Impersonated } = require('google-auth-library');
const ***REMOVED*** SecretManagerServiceClient } = require('@google-cloud/secret-manager');
...
async function main() ***REMOVED***
  ...
  let targetClient = new Impersonated(***REMOVED***
    sourceClient: client,
...
***REMOVED***);
  ...
  const resp = await targetClient.request(***REMOVED*** url });
  for (const bucket of resp.data.items) ***REMOVED***
...
***REMOVED***
  ...
  const smClient = new SecretManagerServiceClient(***REMOVED***
...
***REMOVED***);
  ...
  const [accessResponse] = await smClient.accessSecretVersion(***REMOVED***
    name: secretName,
***REMOVED***);
...
``` js
const ***REMOVED***GoogleAuth, DownscopedClient} = require('google-auth-library');
...
const googleAuth = new GoogleAuth(***REMOVED***
  scopes: ['https://www.googleapis.com/auth/cloud-platform']
});
...
// changes.
oauth2Client.refreshHandler = async () => ***REMOVED***
