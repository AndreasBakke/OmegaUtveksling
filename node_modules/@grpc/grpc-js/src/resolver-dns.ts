...
import ***REMOVED*** extractAndSelectServiceConfig, ServiceConfig } from './service-config';
...
import ***REMOVED*** SubchannelAddress, TcpSubchannelAddress } from './subchannel-address';
import ***REMOVED*** GrpcUri, uriToString, splitHostPort } from './uri-parser';
import ***REMOVED*** isIPv6, isIPv4 } from 'net';
 ...
 * @param arrays
 */
function mergeArrays<T>(...arrays: T[][]): T[] ***REMOVED***
  const result: T[] = [];
  for (
    let i = 0;
    i <
    ...
    for (const array of arrays) ***REMOVED***
      if (i < array.length) ***REMOVED***
        result.push(array[i]);
    ***REMOVED***
  ***REMOVED***
***REMOVED***
 ...
 */
class DnsResolver implements Resolver ***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
      if (isIPv4(hostPort.host) || isIPv6(hostPort.host)) ***REMOVED***
        this.ipResult = [
          ***REMOVED***
            host: hostPort.host,
  ...
  ***REMOVED***
    ...
    this.defaultResolutionError = ***REMOVED***
      code: Status.UNAVAILABLE,
      details: `Name resolution failed for target $***REMOVED***uriToString(this.target)}`,
  ...
  ***REMOVED***;
    ...
    this.backoff = new BackoffTimeout(() => ***REMOVED***
  ...
  ***REMOVED***, backoffOptions);
    ...
    this.nextResolutionTimer = setTimeout(() => ***REMOVED***}, 0);
...
***REMOVED***
   ...
   */
  private startResolution() ***REMOVED***
    if (this.ipResult !== null) ***REMOVED***
    ...
    ***REMOVED***);
      ...
      return;
  ***REMOVED***
    if (this.dnsHostname === null) ***REMOVED***
      trace('Failed to parse DNS address ' + uriToString(this.target));
      setImmediate(() => ***REMOVED***
        this.listener.onError(***REMOVED***
          code: Status.UNAVAILABLE,
          details: `Failed to parse DNS address $***REMOVED***uriToString(this.target)}`,
          metadata: new Metadata(),
      ***REMOVED***);
    ***REMOVED***);
      this.stopNextResolutionTimer();
  ***REMOVED*** else ***REMOVED***
      if (this.pendingLookupPromise !== null) ***REMOVED***
        return;
    ***REMOVED***
      ...
      this.pendingLookupPromise = dnsLookupPromise(hostname, ***REMOVED*** all: true });
      this.pendingLookupPromise.then(
        (addressList) => ***REMOVED***
          ...
          this.latestLookupResult = mergeArrays(
            ip6Addresses,
            ip4Addresses
          ).map((addr) => (***REMOVED*** host: addr.address, port: +this.port! }));
          ...
          if (this.latestLookupResult.length === 0) ***REMOVED***
      ...
      ***REMOVED***
      ...
      if (this.pendingTxtPromise === null) ***REMOVED***
        ...
        this.pendingTxtPromise.then(
          ...
          ***REMOVED*** catch (err) ***REMOVED***
              this.latestServiceConfigError = ***REMOVED***
                code: Status.UNAVAILABLE,
                details: 'Parsing service config failed',
                metadata: new Metadata(),
            ***REMOVED***;
          ***REMOVED***
            if (this.latestLookupResult !== null) ***REMOVED***
...
***REMOVED***
  ...
  private startNextResolutionTimer() ***REMOVED***
    clearTimeout(this.nextResolutionTimer);
    this.nextResolutionTimer = setTimeout(() => ***REMOVED***
      this.stopNextResolutionTimer();
      if (this.continueResolving) ***REMOVED***
        this.startResolutionWithBackoff();
    ***REMOVED***
  ***REMOVED***, this.minTimeBetweenResolutionsMs).unref?.();
    this.isNextResolutionTimerRunning = true;
***REMOVED***

  private stopNextResolutionTimer() ***REMOVED***
    clearTimeout(this.nextResolutionTimer);
    this.isNextResolutionTimerRunning = false;
***REMOVED***

  private startResolutionWithBackoff() ***REMOVED***
    if (this.pendingLookupPromise === null) ***REMOVED***
...
***REMOVED***
  ...
  updateResolution() ***REMOVED***
    /* If there is a pending lookup, just let it finish. Otherwise, if the
     * nextResolutionTimer or backoff timer is running, set the
     * continueResolving flag to resolve when whichever of those timers
     * fires. Otherwise, start resolving immediately. */
    if (this.pendingLookupPromise === null) ***REMOVED***
      if (this.isNextResolutionTimerRunning || this.backoff.isRunning()) ***REMOVED***
...
export interface DnsUrl ***REMOVED***
  host: string;
  port?: string;
}
