...
import ***REMOVED*** Call, Http2CallStream, WriteObject } from './call-stream';
import ***REMOVED*** ChannelOptions } from './channel-options';
import ***REMOVED*** PeerCertificate, checkServerIdentity, TLSSocket, CipherNameAndProtocol } from 'tls';
import ***REMOVED*** ConnectivityState } from './connectivity-state';
import ***REMOVED*** BackoffTimeout, BackoffOptions } from './backoff-timeout';
import ***REMOVED*** getDefaultAuthority } from './resolver';
import * as logging from './logging';
import ***REMOVED*** LogVerbosity, Status } from './constants';
import ***REMOVED*** getProxiedConnection, ProxyConnectionResult } from './http_proxy';
import * as net from 'net';
import ***REMOVED*** GrpcUri, parseUri, splitHostPort, uriToString } from './uri-parser';
import ***REMOVED*** ConnectionOptions } from 'tls';
import ***REMOVED*** FilterFactory, Filter, BaseFilter } from './filter';
import ***REMOVED***
  stringToSubchannelAddress,
  SubchannelAddress,
  subchannelAddressToString,
} from './subchannel-address';
import ***REMOVED*** SubchannelRef, ChannelzTrace, ChannelzChildrenTracker, SubchannelInfo, registerChannelzSubchannel, ChannelzCallTracker, SocketInfo, SocketRef, unregisterChannelzRef, registerChannelzSocket, TlsInfo } from './channelz';
import ***REMOVED*** ConnectivityStateListener } from './subchannel-interface';
...
export interface SubchannelCallStatsTracker ***REMOVED***
  addMessageSent(): void;
 ...
 * @param max
 */
function uniformRandom(min: number, max: number) ***REMOVED***
  return Math.random() * (max - min) + min;
}

const tooManyPingsData: Buffer = Buffer.from('too_many_pings', 'ascii');

export class Subchannel ***REMOVED***
  /**
...
***REMOVED***
  ...
  private getChannelzInfo(): SubchannelInfo ***REMOVED***
    return ***REMOVED***
...
***REMOVED***
  ...
  private getChannelzSocketInfo(): SocketInfo | null ***REMOVED***
...
***REMOVED***
  ...
  private resetChannelzSocketInfo() ***REMOVED***
    if (!this.channelzEnabled) ***REMOVED***
      return;
  ***REMOVED***
...
***REMOVED***
   ...
   */
  private startBackoff() ***REMOVED***
    this.backoffTimeout.runOnce();
***REMOVED***

  private stopBackoff() ***REMOVED***
    this.backoffTimeout.stop();
    this.backoffTimeout.reset();
***REMOVED***

  private sendPing() ***REMOVED***
    if (this.channelzEnabled) ***REMOVED***
  ...
  ***REMOVED***, this.keepaliveTimeoutMs);
    ...
    try ***REMOVED***
      this.session!.ping(
        (err: Error | null, duration: number, payload: Buffer) => ***REMOVED***
...
***REMOVED***
  ...
  private startKeepalivePings() ***REMOVED***
...
***REMOVED***
   ...
   */
  private stopKeepalivePings() ***REMOVED***
    clearInterval(this.keepaliveIntervalId);
    clearTimeout(this.keepaliveTimeoutId);
***REMOVED***

  private createSession(proxyConnectionResult: ProxyConnectionResult) ***REMOVED***
  ...
  ***REMOVED***
    ...
    let connectionOptions: http2.SecureClientSessionOptions =
      this.credentials._getConnectionOptions() || ***REMOVED***};
  ...
  ***REMOVED***);
    session.once(
      'goaway',
      (errorCode: number, lastStreamID: number, opaqueData: Buffer) => ***REMOVED***
        if (this.session === session) ***REMOVED***
          ...
          if (
            ...
            logging.log(
              LogVerbosity.ERROR,
              `Connection to $***REMOVED***uriToString(this.channelTarget)} at $***REMOVED***
                this.subchannelAddressString
            ***REMOVED*** rejected by server because of excess pings. Increasing ping interval to $***REMOVED***
  ...
  ***REMOVED***);
    if (logging.isTracerEnabled(TRACER_NAME)) ***REMOVED***
      session.on('remoteSettings', (settings: http2.Settings) => ***REMOVED***
        this.trace(
    ...
    ***REMOVED***);
      session.on('localSettings', (settings: http2.Settings) => ***REMOVED***
        this.trace(
...
***REMOVED***
  ...
  private startConnectingInternal() ***REMOVED***
    ...
    ***REMOVED*** else ***REMOVED***
        if ('grpc.http_connect_target' in this.options) ***REMOVED***
          ...
          const targetPath = getDefaultAuthority(
            parseUri(this.options['grpc.http_connect_target'] as string) ?? ***REMOVED***
              path: 'localhost',
          ***REMOVED***
          );
...
***REMOVED***
  ...
  private handleDisconnect() ***REMOVED***
    this.transitionToState(
      [ConnectivityState.READY],
      ConnectivityState.TRANSIENT_FAILURE);
...
***REMOVED***
   ...
   * If so, shut it down.
   */
  private checkBothRefcounts() ***REMOVED***
...
***REMOVED***
  ...
  startConnecting() ***REMOVED***
    ...
    if (
      ...
      if (this.connectivityState === ConnectivityState.TRANSIENT_FAILURE) ***REMOVED***
...
***REMOVED***
  ...
  addDisconnectListener(listener: () => void) ***REMOVED***
    this.disconnectListeners.add(listener);
***REMOVED***

  removeDisconnectListener(listener: () => void) ***REMOVED***
...
***REMOVED***
  ...
  getRealSubchannel(): this ***REMOVED***
    return this;
***REMOVED***
}
