...
function getSdkVersion() ***REMOVED***
    if (!sdkVersion) ***REMOVED***
        const ***REMOVED*** version } = require('../../package.json'); // eslint-disable-line @typescript-eslint/no-var-requires
        sdkVersion = version;
  ***REMOVED***
    return sdkVersion;
}
...
function renameProperties(obj, keyMap) ***REMOVED***
    Object.keys(keyMap).forEach((oldKey) => ***REMOVED***
 ...
 */
function getExplicitProjectId(app) ***REMOVED***
 ...
 * @returns A project ID string or null.
 */
function findProjectId(app) ***REMOVED***
    const projectId = getExplicitProjectId(app);
    if (projectId) ***REMOVED***
        return Promise.resolve(projectId);
  ***REMOVED***
    const credential = app.options.credential;
    if (credential instanceof credential_internal_1.ComputeEngineCredential) ***REMOVED***
 ...
 */
function getExplicitServiceAccountEmail(app) ***REMOVED***
    const options = app.options;
    if (validator.isNonEmptyString(options.serviceAccountId)) ***REMOVED***
 ...
 */
function findServiceAccountEmail(app) ***REMOVED***
 ...
 */
function toWebSafeBase64(data) ***REMOVED***
 ...
 */
function formatString(str, params) ***REMOVED***
    let formatted = str;
    Object.keys(params || ***REMOVED***}).forEach((key) => ***REMOVED***
        formatted = formatted.replace(new RegExp('***REMOVED***' + key + '}', 'g'), params[key]);
  ***REMOVED***);
    return formatted;
}
 ...
 */
function generateUpdateMask(obj, terminalPaths = [], root = '') ***REMOVED***
    const updateMask = [];
    if (!validator.isNonNullObject(obj)) ***REMOVED***
        return updateMask;
  ***REMOVED***
    for (const key in obj) ***REMOVED***
        if (typeof obj[key] !== 'undefined') ***REMOVED***
            const nextPath = root ? `$***REMOVED***root}.$***REMOVED***key}` : key;
            // We hit maximum path.
            // Consider switching to Set<string> if the list grows too large.
            if (terminalPaths.indexOf(nextPath) !== -1) ***REMOVED***
                // Add key and stop traversing this branch.
                updateMask.push(key);
          ***REMOVED***
            else ***REMOVED***
                const maskList = generateUpdateMask(obj[key], terminalPaths, nextPath);
                if (maskList.length > 0) ***REMOVED***
                    maskList.forEach((mask) => ***REMOVED***
                        updateMask.push(`$***REMOVED***key}.$***REMOVED***mask}`);
                  ***REMOVED***);
              ***REMOVED***
                else ***REMOVED***
                    updateMask.push(key);
 ...
 */
function transformMillisecondsToSecondsString(milliseconds) ***REMOVED***
    let duration;
    ...
    if (nanos > 0) ***REMOVED***
      ...
      ***REMOVED***
        duration = `$***REMOVED***seconds}.$***REMOVED***nanoString}s`;
  ***REMOVED***
    else ***REMOVED***
        duration = `$***REMOVED***seconds}s`;
  ***REMOVED***
    return duration;
}
 ...
 * `projects/***REMOVED***project}/locations/***REMOVED***location}/functions/***REMOVED***functionName}`, or ***REMOVED***functionName}
 ...
 */
function parseResourceName(resourceName, resourceIdKey) ***REMOVED***
    if (!resourceName.includes('/')) ***REMOVED***
        return ***REMOVED*** resourceId: resourceName };
  ***REMOVED***
    const CHANNEL_NAME_REGEX = new RegExp(`^(projects/([^/]+)/)?locations/([^/]+)/$***REMOVED***resourceIdKey}/([^/]+)$`);
  ...
  ***REMOVED***
    ...
    return ***REMOVED*** projectId, locationId, resourceId };
}
