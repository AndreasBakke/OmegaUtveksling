...
const wrapAnsi = code => `$***REMOVED***ESCAPES.values().next().value}$***REMOVED***ANSI_CSI}$***REMOVED***code}$***REMOVED***ANSI_SGR_TERMINATOR}`;
const wrapAnsiHyperlink = uri => `$***REMOVED***ESCAPES.values().next().value}$***REMOVED***ANSI_ESCAPE_LINK}$***REMOVED***uri}$***REMOVED***ANSI_ESCAPE_BELL}`;
...
const wrapWord = (rows, word, columns) => ***REMOVED***
	...
	for (const [index, character] of characters.entries()) ***REMOVED***
		...
		if (isInsideEscape) ***REMOVED***
			...
			} else if (character === ANSI_SGR_TERMINATOR) ***REMOVED***
				isInsideEscape = false;
			}
...
const exec = (string, columns, options = ***REMOVED***}) => ***REMOVED***
	...
	for (const [index, character] of pre.entries()) ***REMOVED***
		...
		if (ESCAPES.has(character)) ***REMOVED***
			const ***REMOVED***groups} = new RegExp(`(?:\\$***REMOVED***ANSI_CSI}(?<code>\\d+)m|\\$***REMOVED***ANSI_ESCAPE_LINK}(?<uri>.*)$***REMOVED***ANSI_ESCAPE_BELL})`).exec(pre.slice(index).join('')) || ***REMOVED***groups: ***REMOVED***}};
			if (groups.code !== undefined) ***REMOVED***
				const code = Number.parseFloat(groups.code);
				escapeCode = code === END_CODE ? undefined : code;
			} else if (groups.uri !== undefined) ***REMOVED***
