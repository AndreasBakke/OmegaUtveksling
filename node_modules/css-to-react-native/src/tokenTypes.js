import ***REMOVED*** stringify } from 'postcss-value-parser'
import cssColorKeywords from 'css-color-keywords'

const matchString = node => ***REMOVED***
  if (node.type !== 'string') return null
  return node.value
    .replace(/\\([0-9a-f]***REMOVED***1,6})(?:\s|$)/gi, (match, charCode) =>
      String.fromCharCode(parseInt(charCode, 16))
    )
    .replace(/\\/g, '')
}

const hexColorRe = /^(#(?:[0-9a-f]***REMOVED***3,4})***REMOVED***1,2})$/i
const cssFunctionNameRe = /^(rgba?|hsla?|hwb|lab|lch|gray|color)$/

const matchColor = node => ***REMOVED***
  if (
    node.type === 'word' &&
    ...
    return node.value
***REMOVED*** else if (node.type === 'function' && cssFunctionNameRe.test(node.value)) ***REMOVED***
...
export const regExpToken = (regExp, transform = String) => node => ***REMOVED***
