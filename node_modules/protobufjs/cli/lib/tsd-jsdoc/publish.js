...
exports.publish = function publish(taffy, opts) ***REMOVED***
    ...
    // remove undocumented
    taffy(***REMOVED*** undocumented: true }).remove();
    taffy(***REMOVED*** ignore: true }).remove();
    taffy(***REMOVED*** inherited: true }).remove();

    // remove private
    if (!options.private)
        taffy(***REMOVED*** access: "private" }).remove();
...
function parseComment(comment) ***REMOVED***
    ...
    for (var i = 0; i < lines.length; ++i) ***REMOVED***
        ...
        if (match) ***REMOVED***
          ...
          ***REMOVED***
            text = [];
            tags.push(***REMOVED*** name: match[1], text: text });
...
function writeComment(comment, otherwiseNewline) ***REMOVED***
  ...
  ***REMOVED***
    ...
    comment.tags = comment.tags.filter(function(tag) ***REMOVED***
        return keepTags.indexOf(tag.name) > -1 && (tag.name !== "returns" || tag.text[0] !== "***REMOVED***undefined}");
  ***REMOVED***);
    writeln();
...
function getTypeOf(element) ***REMOVED***
  ...
  ***REMOVED***);
    ...
    // Replace Object.<string,number> with ***REMOVED*** [k: string]: number }
    name = replaceRecursive(name, /\bObject\.?<([^,]*), *([^>]*)>/gi, function($0, $1, $2) ***REMOVED***
        return "***REMOVED*** [k: " + $1 + "]: " + $2 + " }";
  ***REMOVED***);
...
function begin(element, is_interface) ***REMOVED***
    if (!seen[element.longname]) ***REMOVED***
        if (isClass(element)) ***REMOVED***
            ...
            var classdesc = comment.tags.find(function(tag) ***REMOVED*** return tag.name === "classdesc"; });
...
function writeFunctionSignature(element, isConstructor, isTypeDef) ***REMOVED***
    ...
    if (element.this)
        params["this"] = ***REMOVED***
            type: element.this.replace(/^***REMOVED***|}$/g, ""),
            optional: false
      ***REMOVED***;
        ...
        element.params.forEach(function(param) ***REMOVED***
            ...
            if (path.length === 1)
              ...
              ***REMOVED***;
                ...
                params[path[0]].type = "***REMOVED*** [k: string]: any }";
      ***REMOVED***);
...
// handles an enum
function handleEnum(element) ***REMOVED***
    begin(element);
