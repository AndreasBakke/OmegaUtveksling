"use strict";
/**
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
Object.defineProperty(exports, "__esModule", ***REMOVED*** value: true });
exports.StreamProxy = exports.StreamType = void 0;
const googleError_1 = require("../googleError");
// eslint-disable-next-line @typescript-eslint/no-var-requires
const duplexify = require('duplexify');
// eslint-disable-next-line @typescript-eslint/no-var-requires
const retryRequest = require('retry-request');
/**
 * The type of gRPC streaming.
 * @enum ***REMOVED***number}
 */
var StreamType;
(function (StreamType) ***REMOVED***
    /** Client sends a single request, server streams responses. */
    StreamType[StreamType["SERVER_STREAMING"] = 1] = "SERVER_STREAMING";
    /** Client streams requests, server returns a single response. */
    StreamType[StreamType["CLIENT_STREAMING"] = 2] = "CLIENT_STREAMING";
    /** Both client and server stream objects. */
    StreamType[StreamType["BIDI_STREAMING"] = 3] = "BIDI_STREAMING";
})(StreamType = exports.StreamType || (exports.StreamType = ***REMOVED***}));
class StreamProxy extends duplexify ***REMOVED***
    /**
     * StreamProxy is a proxy to gRPC-streaming method.
     *
     * @private
     * @constructor
     * @param ***REMOVED***StreamType} type - the type of gRPC stream.
     * @param ***REMOVED***ApiCallback} callback - the callback for further API call.
     */
    constructor(type, callback, rest) ***REMOVED***
        super(undefined, undefined, ***REMOVED***
            objectMode: true,
            readable: type !== StreamType.CLIENT_STREAMING,
            writable: type !== StreamType.SERVER_STREAMING,
      ***REMOVED***);
        this.type = type;
        this._callback = callback;
        this._isCancelCalled = false;
        this._responseHasSent = false;
        this.rest = rest;
  ***REMOVED***
    cancel() ***REMOVED***
        if (this.stream) ***REMOVED***
            this.stream.cancel();
      ***REMOVED***
        else ***REMOVED***
            this._isCancelCalled = true;
      ***REMOVED***
  ***REMOVED***
    /**
     * Forward events from an API request stream to the user's stream.
     * @param ***REMOVED***Stream} stream - The API request stream.
     */
    forwardEvents(stream) ***REMOVED***
        const eventsToForward = ['metadata', 'response', 'status'];
        eventsToForward.forEach(event => ***REMOVED***
            stream.on(event, this.emit.bind(this, event));
      ***REMOVED***);
        // gRPC is guaranteed emit the 'status' event but not 'metadata', and 'status' is the last event to emit.
        // Emit the 'response' event if stream has no 'metadata' event.
        // This avoids the stream swallowing the other events, such as 'end'.
        stream.on('status', () => ***REMOVED***
            if (!this._responseHasSent) ***REMOVED***
                stream.emit('response', ***REMOVED***
                    code: 200,
                    details: '',
                    message: 'OK',
              ***REMOVED***);
          ***REMOVED***
      ***REMOVED***);
        // We also want to supply the status data as 'response' event to support
        // the behavior of google-cloud-node expects.
        // see:
        // https://github.com/GoogleCloudPlatform/google-cloud-node/pull/1775#issuecomment-259141029
        // https://github.com/GoogleCloudPlatform/google-cloud-node/blob/116436fa789d8b0f7fc5100b19b424e3ec63e6bf/packages/common/src/grpc-service.js#L355
        stream.on('metadata', metadata => ***REMOVED***
            // Create a response object with succeeds.
            // TODO: unify this logic with the decoration of gRPC response when it's
            // added. see: https://github.com/googleapis/gax-nodejs/issues/65
            stream.emit('response', ***REMOVED***
                code: 200,
                details: '',
                message: 'OK',
                metadata,
          ***REMOVED***);
            this._responseHasSent = true;
      ***REMOVED***);
        stream.on('error', error => ***REMOVED***
            googleError_1.GoogleError.parseGRPCStatusDetails(error);
      ***REMOVED***);
  ***REMOVED***
    /**
     * Specifies the target stream.
     * @param ***REMOVED***ApiCall} apiCall - the API function to be called.
     * @param ***REMOVED***Object} argument - the argument to be passed to the apiCall.
     */
    setStream(apiCall, argument, retryRequestOptions = ***REMOVED***}) ***REMOVED***
        if (this.type === StreamType.SERVER_STREAMING) ***REMOVED***
            if (this.rest) ***REMOVED***
                const stream = apiCall(argument, this._callback);
                this.stream = stream;
                this.setReadable(stream);
          ***REMOVED***
            else ***REMOVED***
                const retryStream = retryRequest(null, ***REMOVED***
                    objectMode: true,
                    request: () => ***REMOVED***
                        if (this._isCancelCalled) ***REMOVED***
                            if (this.stream) ***REMOVED***
                                this.stream.cancel();
                          ***REMOVED***
                            return;
                      ***REMOVED***
                        const stream = apiCall(argument, this._callback);
                        this.stream = stream;
                        this.forwardEvents(stream);
                        return stream;
                  ***REMOVED***,
                    retries: retryRequestOptions.retries,
                    currentRetryAttempt: retryRequestOptions.currentRetryAttempt,
                    noResponseRetries: retryRequestOptions.noResponseRetries,
                    shouldRetryFn: retryRequestOptions.shouldRetryFn,
              ***REMOVED***);
                this.setReadable(retryStream);
          ***REMOVED***
            return;
      ***REMOVED***
        const stream = apiCall(argument, this._callback);
        this.stream = stream;
        this.forwardEvents(stream);
        if (this.type === StreamType.CLIENT_STREAMING) ***REMOVED***
            this.setWritable(stream);
      ***REMOVED***
        if (this.type === StreamType.BIDI_STREAMING) ***REMOVED***
            this.setReadable(stream);
            this.setWritable(stream);
      ***REMOVED***
        if (this._isCancelCalled && this.stream) ***REMOVED***
            this.stream.cancel();
      ***REMOVED***
  ***REMOVED***
}
exports.StreamProxy = StreamProxy;
//# sourceMappingURL=streaming.js.map