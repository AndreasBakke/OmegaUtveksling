...
const requestSsl = () => ***REMOVED***
    const response = Buffer.allocUnsafe(8);
    response.writeInt32BE(8, 0);
    response.writeInt32BE(80877103, 4);
    return response;
};
const password = (password) => ***REMOVED***
    return writer.addCString(password).flush(112 /* startup */);
};
const sendSASLInitialResponseMessage = function (mechanism, initialResponse) ***REMOVED***
    // 0x70 = 'p'
    writer.addCString(mechanism).addInt32(Buffer.byteLength(initialResponse)).addString(initialResponse);
    return writer.flush(112 /* startup */);
};
const sendSCRAMClientFinalMessage = function (additionalData) ***REMOVED***
    return writer.addString(additionalData).flush(112 /* startup */);
};
const query = (text) => ***REMOVED***
    return writer.addCString(text).flush(81 /* query */);
};
const emptyArray = [];
const parse = (query) => ***REMOVED***
    // expect something like this:
    // ***REMOVED*** name: 'queryName',
    //   text: 'select * from blah',
    //   types: ['int8', 'bool'] }
    // normalize missing query names to allow for null
    const name = query.name || '';
    if (name.length > 63) ***REMOVED***
...
const writeValues = function (values, valueMapper) ***REMOVED***
    for (let i = 0; i < values.length; i++) ***REMOVED***
        const mappedVal = valueMapper ? valueMapper(values[i], i) : values[i];
        if (mappedVal == null) ***REMOVED***
            // add the param type (string) to the writer
            writer.addInt16(0 /* STRING */);
            // write -1 to the param writer to indicate null
            paramWriter.addInt32(-1);
      ***REMOVED***
        else if (mappedVal instanceof Buffer) ***REMOVED***
...
const bind = (config = ***REMOVED***}) => ***REMOVED***
    // normalize config
...
const execute = (config) => ***REMOVED***
    // this is the happy path for most queries
    if (!config || (!config.portal && !config.rows)) ***REMOVED***
        return emptyExecute;
  ***REMOVED***
    const portal = config.portal || '';
...
const cancel = (processID, secretKey) => ***REMOVED***
...
const cstringMessage = (code, string) => ***REMOVED***
...
const describe = (msg) => ***REMOVED***
    return msg.name
        ? cstringMessage(68 /* describe */, `$***REMOVED***msg.type}$***REMOVED***msg.name || ''}`)
        : msg.type === 'P'
            ? emptyDescribePortal
            : emptyDescribeStatement;
};
const close = (msg) => ***REMOVED***
    const text = `$***REMOVED***msg.type}$***REMOVED***msg.name || ''}`;
    return cstringMessage(67 /* close */, text);
};
const copyData = (chunk) => ***REMOVED***
    return writer.add(chunk).flush(100 /* copyFromChunk */);
};
const copyFail = (message) => ***REMOVED***
