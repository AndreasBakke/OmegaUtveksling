...
// Object properties:
typeCheck('***REMOVED***x: Number, y: Boolean}', ***REMOVED***x: 2, y: false});             // true
typeCheck('***REMOVED***x: Number, y: Boolean}',       ***REMOVED***x: 2});                 // false
typeCheck('***REMOVED***x: Number, y: Maybe Boolean}', ***REMOVED***x: 2});                 // true
typeCheck('***REMOVED***x: Number, y: Boolean}',      ***REMOVED***x: 2, y: false, z: 3});  // false
typeCheck('***REMOVED***x: Number, y: Boolean, ...}', ***REMOVED***x: 2, y: false, z: 3});  // true

// A particular type AND object properties:
typeCheck('RegExp***REMOVED***source: String, ...}', /re/i);          // true
typeCheck('RegExp***REMOVED***source: String, ...}', ***REMOVED***source: 're'}); // false

// Custom types:
var opt = ***REMOVED***customTypes:
  ***REMOVED***Even: ***REMOVED*** typeOf: 'Number', validate: function(x) ***REMOVED*** return x % 2 === 0; }}}};
typeCheck('Even', 2, opt); // true

// Nested:
var type = '***REMOVED***a: (String, [Number], ***REMOVED***y: Array, ...}), b: Error***REMOVED***message: String, ...}}'
typeCheck(type, ***REMOVED***a: ['hi', [1, 2, 3], ***REMOVED***y: [1, 'ms']}], b: new Error('oh no')}); // true
```
...
##### example
```js
parseType('Number'); // [***REMOVED***type: 'Number'}]
```
### parsedTypeCheck(parsedType, input, options)
...
##### example
```js
parsedTypeCheck([***REMOVED***type: 'Number'}], 2); // true
...
* __Type__ = an `Identifier`, an `Identifier` followed by a `Structure`, just a `Structure`, or a wildcard `*` - eg. `String`, `Object***REMOVED***x: Number}`, `***REMOVED***x: Number}`, `Array***REMOVED***0: String, 1: Boolean, length: Number}`, `*`
...
* __Structure__ = `Fields`, or a `Tuple`, or an `Array` - eg. `***REMOVED***x: Number}`, `(String, Number)`, `[Date]`
* __Fields__ = a `***REMOVED***`, followed one or more `Field` separated by a comma `,` (trailing comma `,` is permitted), optionally an `...` (always preceded by a comma `,`), followed by a `}` - eg. `***REMOVED***x: Number, y: String}`, `***REMOVED***k: Function, ...}`
...
If used by itself, a 'fields' structure will pass with any type of object as long as it is an instance of `Object` and the properties pass - this allows for duck typing - eg. `***REMOVED***x: Boolean}`.

To check if the properties pass, and the value is of a certain type, you can specify the type - eg. `Error***REMOVED***message: String}`.

If you want to make a field optional, you can simply use `Maybe` - eg. `***REMOVED***x: Boolean, y: Maybe String}` will still pass if `y` is undefined (or null).

If you don't care if the value has properties beyond what you have specified, you can use the 'etc' operator `...` - eg. `***REMOVED***x: Boolean, ...}` will match an object with an `x` property that is a boolean, and with zero or more other properties.
