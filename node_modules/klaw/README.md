...
const items = [] // files, directories, symlinks, etc
klaw('/some/dir')
  .on('readable', function () ***REMOVED***
    let item
    while ((item = this.read())) ***REMOVED***
...
const excludeDirFilter = through2.obj(function (item, enc, next) ***REMOVED***
...
const filterFunc = item => ***REMOVED***
  const basename = path.basename(item)
  return basename === '.' || basename[0] !== '.'
}

klaw('/some/dir', ***REMOVED*** filter: filterFunc })
  .on('data', item => ***REMOVED***
...
const aggregatePngSize = through2.obj(function (item, enc, next) ***REMOVED***
  if (path.extname(item.path) === '.png') ***REMOVED***
...
const deleteAction = through2.obj(function (item, enc, next) ***REMOVED***
  this.push(item)

  if (path.extname(item.path) === '.tmp') ***REMOVED***
    item.deleted = true
