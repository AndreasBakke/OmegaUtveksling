...
s3.getBucketCors(***REMOVED***Bucket: process.env.S3_BUCKET}, function(err, data) ***REMOVED***})
```
...
console.log(typeof config, config) // object ***REMOVED*** BASIC : 'basic' }
```
...
```js
require('dotenv').config(***REMOVED*** path: '/custom/path/to/.env' })
```
...
```js
require('dotenv').config(***REMOVED*** encoding: 'latin1' })
```
...
```js
require('dotenv').config(***REMOVED*** debug: process.env.DEBUG })
```
...
```js
require('dotenv').config(***REMOVED*** override: true })
```
...
```js
require('dotenv').config(***REMOVED*** debug: true })
```
...
- `BASIC=basic` becomes `***REMOVED***BASIC: 'basic'}`
- empty lines are skipped
- lines beginning with `#` are treated as comments
- `#` marks the beginning of a comment (unless when the value is wrapped in quotes)
- empty values become empty strings (`EMPTY=` becomes `***REMOVED***EMPTY: ''}`)
- inner quotes are maintained (think JSON) (`JSON=***REMOVED***"foo": "bar"}` becomes `***REMOVED***JSON:"***REMOVED***\"foo\": \"bar\"}"`)
- whitespace is removed from both ends of unquoted values (see more on [`trim`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/Trim)) (`FOO=  some value  ` becomes `***REMOVED***FOO: 'some value'}`)
- single and double quoted values are escaped (`SINGLE_QUOTE='quoted'` becomes `***REMOVED***SINGLE_QUOTE: "quoted"}`)
- single and double quoted values maintain whitespace from both ends (`FOO="  some value  "` becomes `***REMOVED***FOO: '  some value  '}`)
- double quoted values expand new lines (`MULTILINE="new\nline"` becomes

```
***REMOVED***MULTILINE: 'new
line'}
```
...
```js
// errorReporter.mjs
import ***REMOVED*** Client } from 'best-error-reporting-service'
