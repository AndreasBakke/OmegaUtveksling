"use strict";

Object.defineProperty(exports, "__esModule", ***REMOVED***
  value: true
});
exports.ClassAccessorProperty = ClassAccessorProperty;
exports.ClassBody = ClassBody;
exports.ClassExpression = exports.ClassDeclaration = ClassDeclaration;
exports.ClassMethod = ClassMethod;
exports.ClassPrivateMethod = ClassPrivateMethod;
exports.ClassPrivateProperty = ClassPrivateProperty;
exports.ClassProperty = ClassProperty;
exports.StaticBlock = StaticBlock;
exports._classMethodHead = _classMethodHead;
var _t = require("@babel/types");
const ***REMOVED***
  isExportDefaultDeclaration,
  isExportNamedDeclaration
} = _t;
function ClassDeclaration(node, parent) ***REMOVED***
  ***REMOVED***
    if (!this.format.decoratorsBeforeExport || !isExportDefaultDeclaration(parent) && !isExportNamedDeclaration(parent)) ***REMOVED***
      this.printJoin(node.decorators, node);
  ***REMOVED***
***REMOVED***
  if (node.declare) ***REMOVED***
    this.word("declare");
    this.space();
***REMOVED***
  if (node.abstract) ***REMOVED***
    this.word("abstract");
    this.space();
***REMOVED***
  this.word("class");
  if (node.id) ***REMOVED***
    this.space();
    this.print(node.id, node);
***REMOVED***
  this.print(node.typeParameters, node);
  if (node.superClass) ***REMOVED***
    this.space();
    this.word("extends");
    this.space();
    this.print(node.superClass, node);
    this.print(node.superTypeParameters, node);
***REMOVED***
  if (node.implements) ***REMOVED***
    this.space();
    this.word("implements");
    this.space();
    this.printList(node.implements, node);
***REMOVED***
  this.space();
  this.print(node.body, node);
}
function ClassBody(node) ***REMOVED***
  this.tokenChar(123);
  if (node.body.length === 0) ***REMOVED***
    this.tokenChar(125);
***REMOVED*** else ***REMOVED***
    this.newline();
    this.indent();
    this.printSequence(node.body, node);
    this.dedent();
    if (!this.endsWith(10)) this.newline();
    this.sourceWithOffset("end", node.loc, 0, -1);
    this.rightBrace();
***REMOVED***
}
function ClassProperty(node) ***REMOVED***
  var _node$key$loc, _node$key$loc$end;
  this.printJoin(node.decorators, node);

  const endLine = (_node$key$loc = node.key.loc) == null ? void 0 : (_node$key$loc$end = _node$key$loc.end) == null ? void 0 : _node$key$loc$end.line;
  if (endLine) this.catchUp(endLine);
  this.tsPrintClassMemberModifiers(node);
  if (node.computed) ***REMOVED***
    this.tokenChar(91);
    this.print(node.key, node);
    this.tokenChar(93);
***REMOVED*** else ***REMOVED***
    this._variance(node);
    this.print(node.key, node);
***REMOVED***

  if (node.optional) ***REMOVED***
    this.tokenChar(63);
***REMOVED***
  if (node.definite) ***REMOVED***
    this.tokenChar(33);
***REMOVED***
  this.print(node.typeAnnotation, node);
  if (node.value) ***REMOVED***
    this.space();
    this.tokenChar(61);
    this.space();
    this.print(node.value, node);
***REMOVED***
  this.semicolon();
}
function ClassAccessorProperty(node) ***REMOVED***
  var _node$key$loc2, _node$key$loc2$end;
  this.printJoin(node.decorators, node);

  const endLine = (_node$key$loc2 = node.key.loc) == null ? void 0 : (_node$key$loc2$end = _node$key$loc2.end) == null ? void 0 : _node$key$loc2$end.line;
  if (endLine) this.catchUp(endLine);

  this.tsPrintClassMemberModifiers(node);
  this.word("accessor", true);
  this.space();
  if (node.computed) ***REMOVED***
    this.tokenChar(91);
    this.print(node.key, node);
    this.tokenChar(93);
***REMOVED*** else ***REMOVED***
    this._variance(node);
    this.print(node.key, node);
***REMOVED***

  if (node.optional) ***REMOVED***
    this.tokenChar(63);
***REMOVED***
  if (node.definite) ***REMOVED***
    this.tokenChar(33);
***REMOVED***
  this.print(node.typeAnnotation, node);
  if (node.value) ***REMOVED***
    this.space();
    this.tokenChar(61);
    this.space();
    this.print(node.value, node);
***REMOVED***
  this.semicolon();
}
function ClassPrivateProperty(node) ***REMOVED***
  this.printJoin(node.decorators, node);
  if (node.static) ***REMOVED***
    this.word("static");
    this.space();
***REMOVED***
  this.print(node.key, node);
  this.print(node.typeAnnotation, node);
  if (node.value) ***REMOVED***
    this.space();
    this.tokenChar(61);
    this.space();
    this.print(node.value, node);
***REMOVED***
  this.semicolon();
}
function ClassMethod(node) ***REMOVED***
  this._classMethodHead(node);
  this.space();
  this.print(node.body, node);
}
function ClassPrivateMethod(node) ***REMOVED***
  this._classMethodHead(node);
  this.space();
  this.print(node.body, node);
}
function _classMethodHead(node) ***REMOVED***
  var _node$key$loc3, _node$key$loc3$end;
  this.printJoin(node.decorators, node);

  const endLine = (_node$key$loc3 = node.key.loc) == null ? void 0 : (_node$key$loc3$end = _node$key$loc3.end) == null ? void 0 : _node$key$loc3$end.line;
  if (endLine) this.catchUp(endLine);
  this.tsPrintClassMemberModifiers(node);
  this._methodHead(node);
}
function StaticBlock(node) ***REMOVED***
  this.word("static");
  this.space();
  this.tokenChar(123);
  if (node.body.length === 0) ***REMOVED***
    this.tokenChar(125);
***REMOVED*** else ***REMOVED***
    this.newline();
    this.printSequence(node.body, node, ***REMOVED***
      indent: true
  ***REMOVED***);
    this.sourceWithOffset("end", node.loc, 0, -1);
    this.rightBrace();
***REMOVED***
}

//# sourceMappingURL=classes.js.map
