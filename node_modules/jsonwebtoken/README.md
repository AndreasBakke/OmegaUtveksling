...
encoded private key for RSA and ECDSA. In case of a private key with passphrase an object `***REMOVED*** key, passphrase }` can be used (based on [crypto documentation](https://nodejs.org/api/crypto.html#crypto_sign_sign_private_key_output_format)), in this case be sure you pass the `algorithm` option.
...
var token = jwt.sign(***REMOVED*** foo: 'bar' }, 'shhhhh');
```
...
var token = jwt.sign(***REMOVED*** foo: 'bar' }, privateKey, ***REMOVED*** algorithm: 'RS256' });
```

Sign asynchronously
```js
jwt.sign(***REMOVED*** foo: 'bar' }, privateKey, ***REMOVED*** algorithm: 'RS256' }, function(err, token) ***REMOVED***
  console.log(token);
});
```
...
```js
var older_token = jwt.sign(***REMOVED*** foo: 'bar', iat: Math.floor(Date.now() / 1000) - 30 }, 'shhhhh');
```
...
jwt.sign(***REMOVED***
  data: 'foobar'
}, 'secret', ***REMOVED*** expiresIn: 60 * 60 });

//or even better:

jwt.sign(***REMOVED***
  data: 'foobar'
}, 'secret', ***REMOVED*** expiresIn: '1h' });
```
...
* `audience`: if you want to check audience (`aud`), provide a value here. The audience can be checked against a string, a regular expression or a list of strings and/or regular expressions. 
  > Eg: `"urn:foo"`, `/urn:f[o]***REMOVED***2}/`, `[/urn:f[o]***REMOVED***2}/, "urn:bar"]`
* `complete`: return an object with the decoded `***REMOVED*** payload, header, signature }` instead of only the usual content of the payload.
...
// verify audience
var cert = fs.readFileSync('public.pem');  // get public key
jwt.verify(token, cert, ***REMOVED*** audience: 'urn:foo' }, function(err, decoded) ***REMOVED***
  // if audience mismatch, err == invalid audience
});

// verify issuer
var cert = fs.readFileSync('public.pem');  // get public key
jwt.verify(token, cert, ***REMOVED*** audience: 'urn:foo', issuer: 'urn:issuer' }, function(err, decoded) ***REMOVED***
  // if issuer mismatch, err == invalid issuer
});

// verify jwt id
var cert = fs.readFileSync('public.pem');  // get public key
jwt.verify(token, cert, ***REMOVED*** audience: 'urn:foo', issuer: 'urn:issuer', jwtid: 'jwtid' }, function(err, decoded) ***REMOVED***
  // if jwt id mismatch, err == invalid jwt id
});

// verify subject
var cert = fs.readFileSync('public.pem');  // get public key
jwt.verify(token, cert, ***REMOVED*** audience: 'urn:foo', issuer: 'urn:issuer', jwtid: 'jwtid', subject: 'subject' }, function(err, decoded) ***REMOVED***
  // if subject mismatch, err == invalid subject
});

// alg mismatch
var cert = fs.readFileSync('public.pem'); // get public key
jwt.verify(token, cert, ***REMOVED*** algorithms: ['RS256'] }, function (err, payload) ***REMOVED***
...
var client = jwksClient(***REMOVED***
  jwksUri: 'https://sandrino.auth0.com/.well-known/jwks.json'
});
function getKey(header, callback)***REMOVED***
  client.getSigningKey(header.kid, function(err, key) ***REMOVED***
    var signingKey = key.publicKey || key.rsaPublicKey;
    callback(null, signingKey);
***REMOVED***);
}

jwt.verify(token, getKey, options, function(err, decoded) ***REMOVED***
  console.log(decoded.foo) // bar
});
...
var decoded = jwt.decode(token, ***REMOVED***complete: true});
