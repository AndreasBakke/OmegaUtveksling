...
<Route path="prefix-:id" element=***REMOVED***<Comp /> }>

function Comp() ***REMOVED***
  let params = useParams(); // ***REMOVED*** id: '123' }
  let id = params.id; // "123"
  ...
}

// New behavior at URL /prefix-123
<Route path=":id" element=***REMOVED***<Comp /> }>

function Comp() ***REMOVED***
  let params = useParams(); // ***REMOVED*** id: 'prefix-123' }
...
- Add `requestContext` support to static handler `query`/`queryRoute` ([#9696](https://github.com/remix-run/react-router/pull/9696))
  - Note that the unstable API of `queryRoute(path, routeId)` has been changed to `queryRoute(path, ***REMOVED*** routeId, requestContext })`
