...
function unstable_shouldYield() ***REMOVED***
  return getCurrentTime() >= deadline;
}
function unstable_requestPaint() ***REMOVED***// Since we yield every frame regardless, `requestPaint` has no effect.
}
...
function runTask(priorityLevel, postTaskPriority, node, callback) ***REMOVED***
...
function handleAbortError(error) ***REMOVED***// Abort errors are an implementation detail. We don't expose the
...
function unstable_cancelCallback(node) ***REMOVED***
  var controller = node._controller;
  controller.abort();
}
function unstable_runWithPriority(priorityLevel, callback) ***REMOVED***
...
function unstable_next(callback) ***REMOVED***
  var priorityLevel;

  switch (currentPriorityLevel_DEPRECATED) ***REMOVED***
    case ImmediatePriority:
    case UserBlockingPriority:
    case NormalPriority:
