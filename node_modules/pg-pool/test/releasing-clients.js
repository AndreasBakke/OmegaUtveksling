...
describe('releasing clients', () => ***REMOVED***
  it('removes a client which cannot be queried', async () => ***REMOVED***
    ...
    const ***REMOVED*** rows } = await pool.query('SELECT NOW()')
    expect(rows).to.have.length(1)
    await pool.end()
***REMOVED***)

  it('removes a client which is ending', async () => ***REMOVED***
    // make a pool w/ only 1 client
    const pool = new Pool(***REMOVED*** max: 1 })
    expect(pool.totalCount).to.eql(0)
    const client = await pool.connect()
