'use strict'
const co = require('co')
const expect = require('expect.js')

const describe = require('mocha').describe
const it = require('mocha').it
const path = require('path')

const Pool = require('../')

describe('lifetime timeout', () => ***REMOVED***
  it('connection lifetime should expire and remove the client', (done) => ***REMOVED***
    const pool = new Pool(***REMOVED*** maxLifetimeSeconds: 1 })
    pool.query('SELECT NOW()')
    pool.on('remove', () => ***REMOVED***
      console.log('expired while idle - on-remove event')
      expect(pool.expiredCount).to.equal(0)
      expect(pool.totalCount).to.equal(0)
      done()
  ***REMOVED***)
***REMOVED***)
  it('connection lifetime should expire and remove the client after the client is done working', (done) => ***REMOVED***
    const pool = new Pool(***REMOVED*** maxLifetimeSeconds: 1 })
    pool.query('SELECT pg_sleep(1.01)')
    pool.on('remove', () => ***REMOVED***
      console.log('expired while busy - on-remove event')
      expect(pool.expiredCount).to.equal(0)
      expect(pool.totalCount).to.equal(0)
      done()
  ***REMOVED***)
***REMOVED***)
  it(
    'can remove expired clients and recreate them',
    co.wrap(function* () ***REMOVED***
      const pool = new Pool(***REMOVED*** maxLifetimeSeconds: 1 })
      let query = pool.query('SELECT pg_sleep(1)')
      expect(pool.expiredCount).to.equal(0)
      expect(pool.totalCount).to.equal(1)
      yield query
      expect(pool.expiredCount).to.equal(0)
      expect(pool.totalCount).to.equal(0)
      yield pool.query('SELECT NOW()')
      expect(pool.expiredCount).to.equal(0)
      expect(pool.totalCount).to.equal(1)
  ***REMOVED***)
  )
})
