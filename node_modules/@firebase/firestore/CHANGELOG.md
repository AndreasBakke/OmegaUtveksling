...
- [`5bc6afb75`](https://github.com/firebase/firebase-js-sdk/commit/5bc6afb75b5267bad5940c32458c315e5394321d) [#5268](https://github.com/firebase/firebase-js-sdk/pull/5268) (fixes [#4277](https://github.com/firebase/firebase-js-sdk/issues/4277)) - This change contains multiple quality-of-life improvements when using the `FirestoreDataConverter` in `@firebase/firestore/lite` and `@firebase/firestore`:
  ...
  - Support for omitting properties in nested fields when performing a set operation with `***REMOVED***merge: true}` with a converter (via `PartialWithFieldValue<T>`).
...
- [`169174520`](https://github.com/firebase/firebase-js-sdk/commit/169174520f6451f5741fd50e8957d4097895e97a) [#4929](https://github.com/firebase/firebase-js-sdk/pull/4929) - Added a warning message when the host settings are overridden without `***REMOVED***merge: true}`.
...
- [feature] Added support for calling `FirebaseFiresore.settings` with
  `***REMOVED*** ignoreUndefinedProperties: true }`. When set, Firestore ignores
...
- [fixed] Fixed a regression introduced in v7.14.2 that incorrectly applied
  a `FieldValue.increment` in combination with `set(***REMOVED***...}, ***REMOVED***merge: true})`.
...
- [feature] Added `FieldValue.increment()`, which can be used in `update()`
  and `set(..., ***REMOVED***merge:true})` to increment or decrement numeric field
...
- [feature] Access to offline persistence is no longer limited to a single tab.
  ...
  with `***REMOVED***experimentalTabSynchronization: true}`. All tabs accessing persistence
...
- [fixed] Fixed `get(***REMOVED***source: 'cache'})` to be able to return nonexistent
...
- [changed] `QuerySnapshot.docChanges()` is now a method that optionally takes
  ...
  with `***REMOVED*** includeMetadataChanges:true }`, metadata-only document changes are
  suppressed in `docChanges()`.
- [feature] Added new `***REMOVED*** mergeFields: (string|FieldPath)[] }` option to `set()`
...
- [feature] Added ability to control whether DocumentReference.get() and
  Query.get() should fetch from server only, (by passing ***REMOVED*** source: 'server' }),
  cache only (by passing ***REMOVED*** source: 'cache' }), or attempt server and fall back
