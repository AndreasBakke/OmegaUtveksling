...
var index_standalone = ***REMOVED***};

var safeBuffer = ***REMOVED***exports: ***REMOVED***}};
...
var Headers$3 = function() ***REMOVED***
  this.clear();
};

Headers$3.prototype.ALLOWED_DUPLICATES = ['set-cookie', 'set-cookie2', 'warning', 'www-authenticate'];

Headers$3.prototype.clear = function() ***REMOVED***
  this._sent  = ***REMOVED***};
  this._lines = [];
};

Headers$3.prototype.set = function(name, value) ***REMOVED***
  ...
  if (!this._sent.hasOwnProperty(key) || this.ALLOWED_DUPLICATES.indexOf(key) >= 0) ***REMOVED***
    this._sent[key] = true;
    this._lines.push(name + ': ' + value + '\r\n');
***REMOVED***
};

Headers$3.prototype.toString = function() ***REMOVED***
  return this._lines.join('');
};

Headers$3.prototype._strip = function(string) ***REMOVED***
...
var Base$7 = function(request, url, options) ***REMOVED***
  Emitter.call(this);
  Base$7.validateOptions(options || ***REMOVED***}, ['maxLength', 'masking', 'requireMasking', 'protocols']);
...
Base$7.isWebSocket = function(request) ***REMOVED***
...
Base$7.validateOptions = function(options, validKeys) ***REMOVED***
...
***REMOVED***,
  ...
  _failHandshake: function(error) ***REMOVED***
...
***REMOVED***,
  ...
  _open: function() ***REMOVED***
    this.readyState = 1;
    this.__queue.forEach(function(args) ***REMOVED*** this.frame.apply(this, args); }, this);
    this.__queue = [];
...
Base$7.ConnectEvent = function() ***REMOVED***};

Base$7.OpenEvent = function() ***REMOVED***};

Base$7.CloseEvent = function(code, reason) ***REMOVED***
  this.code   = code;
  this.reason = reason;
};

Base$7.MessageEvent = function(data) ***REMOVED***
  this.data = data;
};

Base$7.PingEvent = function(data) ***REMOVED***
  this.data = data;
};

Base$7.PongEvent = function(data) ***REMOVED***
  this.data = data;
};

var base = Base$7;

var httpParser = ***REMOVED***};

/*jshint node:true */
var assert = require$$0__default$2["default"];

httpParser.HTTPParser = HTTPParser;
function HTTPParser(type) ***REMOVED***
  assert.ok(type === HTTPParser.REQUEST || type === HTTPParser.RESPONSE || type === undefined);
  if (type === undefined) ; else ***REMOVED***
    this.initialize(type);
***REMOVED***
}
HTTPParser.prototype.initialize = function (type, async_resource) ***REMOVED***
...
HTTPParser.prototype[kOnMessageComplete] = function () ***REMOVED***};

var compatMode0_12 = true;
Object.defineProperty(HTTPParser, 'kOnExecute', ***REMOVED***
...
HTTPParser.prototype.free = function () ***REMOVED***};
HTTPParser.prototype._compatMode0_11 = false;
HTTPParser.prototype.getAsyncId = function() ***REMOVED*** return 0; };

var headerState = ***REMOVED***
  REQUEST_LINE: true,
  RESPONSE_LINE: true,
  HEADER: true
};
HTTPParser.prototype.execute = function (chunk, start, length) ***REMOVED***
  if (!(this instanceof HTTPParser)) ***REMOVED***
...
***REMOVED***
  ...
  try ***REMOVED***
    while (this.offset < end) ***REMOVED***
      if (this[this.state]()) ***REMOVED***
        break;
    ***REMOVED***
  ***REMOVED***
***REMOVED*** catch (err) ***REMOVED***
    if (this.isUserCall) ***REMOVED***
      throw err;
  ***REMOVED***
    this.hadError = true;
    return err;
***REMOVED***
  ...
  if (headerState[this.state]) ***REMOVED***
    this.headerSize += length;
    if (this.headerSize > HTTPParser.maxHeaderSize) ***REMOVED***
...
var stateFinishAllowed = ***REMOVED***
  REQUEST_LINE: true,
  RESPONSE_LINE: true,
  BODY_RAW: true
};
HTTPParser.prototype.finish = function () ***REMOVED***
  if (this.hadError) ***REMOVED***
    return;
***REMOVED***
  if (!stateFinishAllowed[this.state]) ***REMOVED***
    return new Error('invalid state for EOF');
***REMOVED***
  if (this.state === 'BODY_RAW') ***REMOVED***
...
HTTPParser.prototype.getCurrentBuffer = function () ***REMOVED***};

//For correct error handling - see HTTPParser#execute
//Usage: this.userCall()(userFunction('arg'));
HTTPParser.prototype.userCall = function () ***REMOVED***
  this.isUserCall = true;
  var self = this;
  return function (ret) ***REMOVED***
    self.isUserCall = false;
    return ret;
***REMOVED***;
};

HTTPParser.prototype.nextRequest = function () ***REMOVED***
  this.userCall()(this[kOnMessageComplete]());
  this.reinitialize(this.type);
};

HTTPParser.prototype.consumeLine = function () ***REMOVED***
  var end = this.end,
      chunk = this.chunk;
  for (var i = this.offset; i < end; i++) ***REMOVED***
    if (chunk[i] === 0x0a) ***REMOVED*** // \n
      var line = this.line + chunk.toString(HTTPParser.encoding, this.offset, i);
      if (line.charAt(line.length - 1) === '\r') ***REMOVED***
...
HTTPParser.prototype.parseHeader = function (line, headers) ***REMOVED***
  if (line.indexOf('\r') !== -1) ***REMOVED***
    throw parseErrorCode('HPE_LF_EXPECTED');
***REMOVED***

  var match = headerExp.exec(line);
  var k = match && match[1];
  if (k) ***REMOVED*** // skip empty string (malformed header)
    headers.push(k);
    headers.push(match[2]);
***REMOVED*** else ***REMOVED***
    var matchContinue = headerContinueExp.exec(line);
    if (matchContinue && headers.length) ***REMOVED***
      if (headers[headers.length - 1]) ***REMOVED***
...
HTTPParser.prototype.REQUEST_LINE = function () ***REMOVED***
...
var responseExp = /^HTTP\/(\d)\.(\d) (\d***REMOVED***3}) ?(.*)$/;
HTTPParser.prototype.RESPONSE_LINE = function () ***REMOVED***
  var line = this.consumeLine();
  if (!line) ***REMOVED***
    return;
***REMOVED***
...
***REMOVED***
  ...
  // Implied zero length.
  if ((statusCode / 100 | 0) === 1 || statusCode === 204 || statusCode === 304) ***REMOVED***
    this.body_bytes = 0;
***REMOVED***
  this.state = 'HEADER';
};

HTTPParser.prototype.shouldKeepAlive = function () ***REMOVED***
  if (this.info.versionMajor > 0 && this.info.versionMinor > 0) ***REMOVED***
    if (this.connection.indexOf('close') !== -1) ***REMOVED***
      return false;
  ***REMOVED***
***REMOVED*** else if (this.connection.indexOf('keep-alive') === -1) ***REMOVED***
    return false;
***REMOVED***
  if (this.body_bytes !== null || this.isChunked) ***REMOVED*** // || skipBody
    return true;
***REMOVED***
  return false;
};

HTTPParser.prototype.HEADER = function () ***REMOVED***
  var line = this.consumeLine();
  if (line === undefined) ***REMOVED***
    return;
***REMOVED***
  var info = this.info;
  if (line) ***REMOVED***
...
***REMOVED*** else ***REMOVED***
    ...
    for (var i = 0; i < headers.length; i += 2) ***REMOVED***
      switch (headers[i].toLowerCase()) ***REMOVED***
        case 'transfer-encoding':
          this.isChunked = headers[i + 1].toLowerCase() === 'chunked';
          break;
        case 'content-length':
          currentContentLengthValue = +headers[i + 1];
          if (hasContentLength) ***REMOVED***
            // Fix duplicate Content-Length header with same values.
            // Throw error only if values are different.
            // Known issues:
            // https://github.com/request/request/issues/2091#issuecomment-328715113
            // https://github.com/nodejs/node/issues/6517#issuecomment-216263771
            if (currentContentLengthValue !== this.body_bytes) ***REMOVED***
  ...
  ***REMOVED***
    ...
    // resilient.
    if (this.isChunked && hasContentLength) ***REMOVED***
  ...
  ***REMOVED***
    ...
    if (hasUpgradeHeader && this.connection.indexOf('upgrade') != -1) ***REMOVED***
      info.upgrade = this.type === HTTPParser.REQUEST || info.statusCode === 101;
  ***REMOVED*** else ***REMOVED***
      info.upgrade = info.method === method_connect;
  ***REMOVED***

    if (this.isChunked && info.upgrade) ***REMOVED***
      this.isChunked = false;
  ***REMOVED***
  ...
  ***REMOVED***
    if (skipBody === 2) ***REMOVED***
      this.nextRequest();
      return true;
  ***REMOVED*** else if (this.isChunked && !skipBody) ***REMOVED***
      this.state = 'BODY_CHUNKHEAD';
  ***REMOVED*** else if (skipBody || this.body_bytes === 0) ***REMOVED***
      this.nextRequest();
      // For older versions of node (v6.x and older?), that return skipBody=1 or skipBody=true,
      //   need this "return true;" if it's an upgrade request.
      return info.upgrade;
  ***REMOVED*** else if (this.body_bytes === null) ***REMOVED***
      this.state = 'BODY_RAW';
  ***REMOVED*** else ***REMOVED***
...
HTTPParser.prototype.BODY_CHUNKHEAD = function () ***REMOVED***
  var line = this.consumeLine();
  if (line === undefined) ***REMOVED***
    return;
***REMOVED***
  this.body_bytes = parseInt(line, 16);
  if (!this.body_bytes) ***REMOVED***
...
HTTPParser.prototype.BODY_CHUNK = function () ***REMOVED***
...
HTTPParser.prototype.BODY_CHUNKEMPTYLINE = function () ***REMOVED***
  var line = this.consumeLine();
  if (line === undefined) ***REMOVED***
    return;
***REMOVED***
...
HTTPParser.prototype.BODY_CHUNKTRAILERS = function () ***REMOVED***
  var line = this.consumeLine();
  if (line === undefined) ***REMOVED***
    return;
***REMOVED***
...
***REMOVED*** else ***REMOVED***
    if (this.trailers.length) ***REMOVED***
...
HTTPParser.prototype.BODY_RAW = function () ***REMOVED***
  var length = this.end - this.offset;
  this.userCall()(this[kOnBody](this.chunk, this.offset, length));
  this.offset = this.end;
};

HTTPParser.prototype.BODY_SIZED = function () ***REMOVED***
  var length = Math.min(this.end - this.offset, this.body_bytes);
  this.userCall()(this[kOnBody](this.chunk, this.offset, length));
  this.offset += length;
  this.body_bytes -= length;
...
['Headers', 'HeadersComplete', 'Body', 'MessageComplete'].forEach(function (name) ***REMOVED***
  var k = HTTPParser['kOn' + name];
  Object.defineProperty(HTTPParser.prototype, 'on' + name, ***REMOVED***
    get: function () ***REMOVED***
      return this[k];
  ***REMOVED***,
    set: function (to) ***REMOVED***
      // hack for backward compatibility
...
function parseErrorCode(code) ***REMOVED***
...
var HttpParser$3 = function(type) ***REMOVED***
  this._type     = type;
...
var VERSION = process.version
  ? process.version.match(/[0-9]+/g).map(function(n) ***REMOVED*** return parseInt(n, 10) })
  : [];
...
HttpParser$3.prototype.isComplete = function() ***REMOVED***
  return this._complete;
};

HttpParser$3.prototype.parse = function(chunk) ***REMOVED***
  ...
  serializeParams: function(name, params) ***REMOVED***
    ...
    var print = function(key, value) ***REMOVED***
      if (value instanceof Array) ***REMOVED***
        value.forEach(function(v) ***REMOVED*** print(key, v); });
...
Offers.prototype.push = function(name, params) ***REMOVED***
  ...
  this._inOrder.push(***REMOVED*** name: name, params: params });
};
...
var RingBuffer$2 = function(bufferSize) ***REMOVED***
  this._bufferSize = bufferSize;
  this.clear();
};

RingBuffer$2.prototype.clear = function() ***REMOVED***
...
RingBuffer$2.prototype.push = function(value) ***REMOVED***
...
RingBuffer$2.prototype.peek = function() ***REMOVED***
  if (this.length === 0) return void 0;
  return this._buffer[this._head];
};

RingBuffer$2.prototype.shift = function() ***REMOVED***
  if (this.length === 0) return void 0;
...
var Functor$1 = function(session, method) ***REMOVED***
...
Functor$1.prototype.call = function(error, message, callback, context) ***REMOVED***
  if (this._stopped) return;

  var record = ***REMOVED*** error: error, message: message, callback: callback, context: context, done: false },
      called = false,
      self   = this;
...
Functor$1.prototype._stop = function() ***REMOVED***
  this.pending  = this._queue.length;
  this._stopped = true;
};

Functor$1.prototype._flushQueue = function() ***REMOVED***
...
Pledge$2.all = function(list) ***REMOVED***
...
Pledge$2.prototype.then = function(callback) ***REMOVED***
  if (this._complete) callback();
  else this._callbacks.push(callback);
};

Pledge$2.prototype.done = function() ***REMOVED***
  this._complete = true;
...
var Cell$1 = function(tuple) ***REMOVED***
  this._ext     = tuple[0];
  this._session = tuple[1];

  this._functors = ***REMOVED***
    incoming: new Functor(this._session, 'processIncomingMessage'),
    outgoing: new Functor(this._session, 'processOutgoingMessage')
***REMOVED***;
};

Cell$1.prototype.pending = function(direction) ***REMOVED***
  var functor = this._functors[direction];
  if (!functor._stopped) functor.pending += 1;
};

Cell$1.prototype.incoming = function(error, message, callback, context) ***REMOVED***
  this._exec('incoming', error, message, callback, context);
};

Cell$1.prototype.outgoing = function(error, message, callback, context) ***REMOVED***
  this._exec('outgoing', error, message, callback, context);
};

Cell$1.prototype.close = function() ***REMOVED***
  this._closed = this._closed || new Pledge$1();
  this._doClose();
  return this._closed;
};

Cell$1.prototype._exec = function(direction, error, message, callback, context) ***REMOVED***
  this._functors[direction].call(error, message, function(err, msg) ***REMOVED***
...
Cell$1.prototype._doClose = function() ***REMOVED***
...
var Pipeline$1 = function(sessions) ***REMOVED***
  this._cells   = sessions.map(function(session) ***REMOVED*** return new Cell(session) });
  this._stopped = ***REMOVED*** incoming: false, outgoing: false };
};

Pipeline$1.prototype.processIncomingMessage = function(message, callback, context) ***REMOVED***
  if (this._stopped.incoming) return;
  this._loop('incoming', this._cells.length - 1, -1, -1, message, callback, context);
};

Pipeline$1.prototype.processOutgoingMessage = function(message, callback, context) ***REMOVED***
  if (this._stopped.outgoing) return;
  this._loop('outgoing', 0, this._cells.length, 1, message, callback, context);
};

Pipeline$1.prototype.close = function(callback, context) ***REMOVED***
  this._stopped = ***REMOVED*** incoming: true, outgoing: true };

  var closed = this._cells.map(function(a) ***REMOVED*** return a.close() });
  if (callback)
    Pledge.all(closed).then(function() ***REMOVED*** callback.call(context); });
};

Pipeline$1.prototype._loop = function(direction, start, end, step, message, callback, context) ***REMOVED***
...
var Extensions$1 = function() ***REMOVED***
...
var instance$a = ***REMOVED***
  add: function(ext) ***REMOVED***
...
***REMOVED***,
  ...
  generateOffer: function() ***REMOVED***
    var sessions = [],
        offer    = [],
        index    = ***REMOVED***};

    this._inOrder.forEach(function(ext) ***REMOVED***
      ...
      offers.forEach(function(off) ***REMOVED***
...
***REMOVED***,
  ...
  activate: function(header) ***REMOVED***
    var responses = Parser.parseHeader(header),
        sessions  = [];

    responses.eachOffer(function(name, params) ***REMOVED***
...
***REMOVED***,
  ...
  generateResponse: function(header) ***REMOVED***
    var sessions = [],
        response = [],
...
***REMOVED***,
  ...
  validFrameRsv: function(frame) ***REMOVED***
    var allowed = ***REMOVED*** rsv1: false, rsv2: false, rsv3: false },
        ext;

    if (Extensions$1.MESSAGE_OPCODES.indexOf(frame.opcode) >= 0) ***REMOVED***
      for (var i = 0, n = this._sessions.length; i < n; i++) ***REMOVED***
...
***REMOVED***,
  ...
  processIncomingMessage: function(message, callback, context) ***REMOVED***
    this._pipeline.processIncomingMessage(message, callback, context);
***REMOVED***,

  processOutgoingMessage: function(message, callback, context) ***REMOVED***
    this._pipeline.processOutgoingMessage(message, callback, context);
***REMOVED***,

  close: function(callback, context) ***REMOVED***
    if (!this._pipeline) return callback.call(context);
    this._pipeline.close(callback, context);
***REMOVED***,

  _reserve: function(ext) ***REMOVED***
    this._rsv1 = this._rsv1 || (ext.rsv1 && ext.name);
    this._rsv2 = this._rsv2 || (ext.rsv2 && ext.name);
    this._rsv3 = this._rsv3 || (ext.rsv3 && ext.name);
***REMOVED***,

  _reserved: function(ext) ***REMOVED***
...
var Frame$1 = function() ***REMOVED***};

var instance$9 = ***REMOVED***
  final:        false,
  rsv1:         false,
  rsv2:         false,
  rsv3:         false,
...
var Message$1 = function() ***REMOVED***
  this.rsv1    = false;
  this.rsv2    = false;
  this.rsv3    = false;
  this.opcode  = null;
...
var Hybi$2 = function(request, url, options) ***REMOVED***
  ...
  if (protos !== undefined) ***REMOVED***
    ...
    this.protocol = protos.filter(function(p) ***REMOVED*** return supported.indexOf(p) >= 0 })[0];
***REMOVED***
...
Hybi$2.mask = function(payload, mask, offset) ***REMOVED***
...
Hybi$2.generateAccept = function(key) ***REMOVED***
...
***REMOVED***,
  ...
  UTF8_MATCH: /^([\x00-\x7F]|[\xC2-\xDF][\x80-\xBF]|\xE0[\xA0-\xBF][\x80-\xBF]|[\xE1-\xEC\xEE\xEF][\x80-\xBF]***REMOVED***2}|\xED[\x80-\x9F][\x80-\xBF]|\xF0[\x90-\xBF][\x80-\xBF]***REMOVED***2}|[\xF1-\xF3][\x80-\xBF]***REMOVED***3}|\xF4[\x80-\x8F][\x80-\xBF]***REMOVED***2})*$/,
...
***REMOVED***,
  ...
  ***REMOVED*** else if (this.readyState === 1) ***REMOVED***
      this.readyState = 2;
      this._extensions.close(function() ***REMOVED*** this.frame(reason, 'close', code); }, this);
      return true;
  ***REMOVED*** else ***REMOVED***
      return false;
  ***REMOVED***
...
var PORTS = ***REMOVED*** 'ws:': 80, 'wss:': 443 };

var Proxy$1 = function(client, origin, options) ***REMOVED***
  this._client  = client;
...
var Client$2 = function(_url, options) ***REMOVED***
...
Client$2.generateKey = function() ***REMOVED***
  return crypto$1.randomBytes(16).toString('base64');
};

var instance$5 = ***REMOVED***
...
var Draft75$2 = function(request, url, options) ***REMOVED***
...
var Draft76$1 = function(request, url, options) ***REMOVED***
...
***REMOVED***,
  ...
  parse: function(chunk) ***REMOVED***
    ...
    this.EVENTS.forEach(function(event) ***REMOVED***
      this._delegate.on(event, function(e) ***REMOVED*** self.emit(event, e); });
  ***REMOVED***, this);
...
['addExtension', 'setHeader', 'start', 'frame', 'text', 'binary', 'ping', 'close'].forEach(function(method) ***REMOVED***
  instance$2[method] = function() ***REMOVED***
...
Server$1.isSecureRequest = function(request) ***REMOVED***
...
Server$1.determineUrl = function(request) ***REMOVED***
  var scheme = this.isSecureRequest(request) ? 'wss:' : 'ws:';
  return scheme + '//' + request.headers.host + request.url;
};

Server$1.http = function(request, options) ***REMOVED***
...
var Event$3 = function(eventType, options) ***REMOVED***
  this.type = eventType;
  for (var key in options)
    this[key] = options[key];
};

Event$3.prototype.initEvent = function(eventType, canBubble, cancelable) ***REMOVED***
  this.type       = eventType;
  this.bubbles    = canBubble;
  this.cancelable = cancelable;
};

Event$3.prototype.stopPropagation = function() ***REMOVED***};
Event$3.prototype.preventDefault  = function() ***REMOVED***};
...
var EventTarget$2 = ***REMOVED***
  onopen:     null,
  onmessage:  null,
  onerror:    null,
  onclose:    null,

  addEventListener: function(eventType, listener, useCapture) ***REMOVED***
    this.on(eventType, listener);
***REMOVED***,

  removeEventListener: function(eventType, listener, useCapture) ***REMOVED***
    this.removeListener(eventType, listener);
***REMOVED***,

  dispatchEvent: function(event) ***REMOVED***
...
var API$3 = function(options) ***REMOVED***
  options = options || ***REMOVED***};
...
***REMOVED***
  ...
  this._driver.on('open',    function(e) ***REMOVED*** self._open(); });
  this._driver.on('message', function(e) ***REMOVED*** self._receiveMessage(e.data); });
  this._driver.on('close',   function(e) ***REMOVED*** self._beginClose(e.reason, e.code); });
...
***REMOVED***,
  ...
  close: function(code, reason) ***REMOVED***
    ...
    if (this.readyState < API$3.CLOSING) ***REMOVED***
      var self = this;
      this._closeTimer = setTimeout(function() ***REMOVED***
        self._beginClose('', 1006);
    ***REMOVED***, API$3.CLOSE_TIMEOUT);
  ***REMOVED***
...
***REMOVED***,
  ...
  _configureStream: function() ***REMOVED***
    ...
    ['close', 'end'].forEach(function(event) ***REMOVED***
      this._stream.on(event, function() ***REMOVED*** self._finalizeClose(); });
  ***REMOVED***, this);
...
***REMOVED***,
  ...
  _receiveMessage: function(data) ***REMOVED***
    ...
    var event = new Event$1('message', ***REMOVED*** data: data });
...
***REMOVED***,
  ...
  _emitError: function(message) ***REMOVED***
    ...
    var event = new Event$1('error', ***REMOVED*** message: message });
...
***REMOVED***,
  ...
  _finalizeClose: function() ***REMOVED***
    ...
    var event = new Event$1('close', ***REMOVED*** code: code, reason: reason });
...
var DEFAULT_PORTS    = ***REMOVED*** 'http:': 80, 'https:': 443, 'ws:':80, 'wss:': 443 },
...
var Client = function(_url, protocols, options) ***REMOVED***
  ...
  this.url     = _url;
  this._driver = driver$2.client(this.url, ***REMOVED*** maxLength: options.maxLength, protocols: protocols });
...
***REMOVED***, this);
  ...
  var proxy      = options.proxy || ***REMOVED***},
      ...
      onConnect  = function() ***REMOVED*** self._onConnect(); },
...
***REMOVED***
  ...
  this._proxy.pipe(this._stream, ***REMOVED*** end: false });
  this._stream.pipe(this._proxy);

  this._proxy.on('connect', function() ***REMOVED***
    if (secure) ***REMOVED***
      var options = ***REMOVED*** socket: self._stream, servername: uri.hostname };
...
var EventSource = function(request, response, options) ***REMOVED***
  this.writable = true;
  options = options || ***REMOVED***};
...
***REMOVED***
  ...
  process.nextTick(function() ***REMOVED*** self._open(); });
  ...
  this._stream.on('drain', function() ***REMOVED*** self.emit('drain'); });

  if (this._ping)
    this._pingTimer = setInterval(function() ***REMOVED*** self.ping(); }, this._ping * 1000);

  ['error', 'end'].forEach(function(event) ***REMOVED***
    self._stream.on(event, function() ***REMOVED*** self.close(); });
***REMOVED***);
};
util$2.inherits(EventSource, Stream);

EventSource.isEventSource = function(request) ***REMOVED***
...
***REMOVED***,
  ...
  write: function(message) ***REMOVED***
    return this.send(message);
***REMOVED***,

  end: function(message) ***REMOVED***
    if (message !== undefined) this.write(message);
    this.close();
***REMOVED***,

  send: function(message, options) ***REMOVED***
...
var WebSocket$1 = function(request, socket, body, protocols, options) ***REMOVED***
  options = options || ***REMOVED***};

  this._stream = socket;
  this._driver = driver.http(request, ***REMOVED*** maxLength: options.maxLength, protocols: protocols });

  var self = this;
  if (!this._stream || !this._stream.writable) return;
  if (!this._stream.readable) return this._stream.end();

  var catchup = function() ***REMOVED*** self._stream.removeListener('data', catchup); };
...
WebSocket$1.isWebSocket = function(request) ***REMOVED***
  return driver.isWebSocket(request);
};

WebSocket$1.validateOptions = function(options, validKeys) ***REMOVED***
...
Object.defineProperty(index_standalone, '__esModule', ***REMOVED*** value: true });
...
function _interopDefaultLegacy$1 (e) ***REMOVED*** return e && typeof e === 'object' && 'default' in e ? e : ***REMOVED*** 'default': e }; }
...
var MemoryStorage = /** @class */ (function () ***REMOVED***
  ...
  ***REMOVED***;
    MemoryStorage.prototype.get = function (key) ***REMOVED***
        if (util.contains(this.cache_, key)) ***REMOVED***
 ...
 */
var LUIDGenerator = (function () ***REMOVED***
    var id = 1;
...
var buildLogMessage_ = function () ***REMOVED***
  ...
  ***REMOVED***
    var message = '';
    for (var i = 0; i < varArgs.length; i++) ***REMOVED***
        var arg = varArgs[i];
 ...
 */
var enableLogging$1 = function (logger_, persistent) ***REMOVED***
...
var logWrapper = function (prefix) ***REMOVED***
    return function () ***REMOVED***
        var varArgs = [];
        for (var _i = 0; _i < arguments.length; _i++) ***REMOVED***
            varArgs[_i] = arguments[_i];
 ...
 * does not use https.
 */
var warnIfPageIsSecure = function () ***REMOVED***
 ...
 */
var isInvalidJSONNumber = function (data) ***REMOVED***
    return (typeof data === 'number' &&
        (data !== data || // NaN
            data === Number.POSITIVE_INFINITY ||
            data === Number.NEGATIVE_INFINITY));
};
var executeWhenDOMReady = function (fn) ***REMOVED***
    if (util.isNodeSdk() || document.readyState === 'complete') ***REMOVED***
        fn();
  ***REMOVED***
    else ***REMOVED***
 ...
 */
var nameCompare = function (a, b) ***REMOVED***
    if (a === b) ***REMOVED***
        return 0;
  ***REMOVED***
 ...
 */
var stringCompare = function (a, b) ***REMOVED***
    if (a === b) ***REMOVED***
        return 0;
  ***REMOVED***
    else if (a < b) ***REMOVED***
...
var requireKey = function (key, obj) ***REMOVED***
...
var ObjectToUniqueKey = function (obj) ***REMOVED***
 ...
 */
var splitStringBySize = function (str, segsize) ***REMOVED***
    var len = str.length;
 ...
 *
 */
var doubleToIEEE754String = function (v) ***REMOVED***
 ...
 */
var isChromeExtensionContentScript = function () ***REMOVED***
 ...
 */
var isWindowsStoreApp = function () ***REMOVED***
 ...
 */
var INTEGER_REGEXP_ = new RegExp('^-?(0*)\\d***REMOVED***1,10}$');
/**
 ...
 */
var tryParseInt = function (str) ***REMOVED***
 ...
 */
var exceptionGuard = function (fn) ***REMOVED***
    try ***REMOVED***
        fn();
  ***REMOVED***
    catch (e) ***REMOVED***
 ...
 */
var beingCrawled = function () ***REMOVED***
 ...
 */
var setTimeoutNonBlocking = function (fn, time) ***REMOVED***
...
var RepoInfo = /** @class */ (function () ***REMOVED***
     ...
     */
    function RepoInfo(host, secure, namespace, webSocketOnly, nodeAdmin, persistenceKey, includeNamespaceInQueryParams) ***REMOVED***
        if (nodeAdmin === void 0) ***REMOVED*** nodeAdmin = false; }
        if (persistenceKey === void 0) ***REMOVED*** persistenceKey = ''; }
        if (includeNamespaceInQueryParams === void 0) ***REMOVED*** includeNamespaceInQueryParams = false; }
        this.secure = secure;
  ...
  ***REMOVED***;
    Object.defineProperty(RepoInfo.prototype, "host", ***REMOVED***
        get: function () ***REMOVED***
            return this._host;
      ***REMOVED***,
        set: function (newHost) ***REMOVED***
...
function repoInfoConnectionURL(repoInfo, type, params) ***REMOVED***
  ...
  ***REMOVED***
    var pairs = [];
    each(params, function (key, value) ***REMOVED***
 ...
 */
var StatsCollection = /** @class */ (function () ***REMOVED***
    function StatsCollection() ***REMOVED***
        this.counters_ = ***REMOVED***};
  ***REMOVED***
    StatsCollection.prototype.incrementCounter = function (name, amount) ***REMOVED***
        if (amount === void 0) ***REMOVED*** amount = 1; }
        if (!util.contains(this.counters_, name)) ***REMOVED***
            this.counters_[name] = 0;
      ***REMOVED***
        this.counters_[name] += amount;
  ***REMOVED***;
    StatsCollection.prototype.get = function () ***REMOVED***
...
var WebSocketConnection = /** @class */ (function () ***REMOVED***
     ...
     * connection
     */
    function WebSocketConnection(connId, repoInfo, applicationId, appCheckToken, authToken, transportSessionId, lastSessionId) ***REMOVED***
        this.connId = connId;
  ...
  ***REMOVED***
     ...
     */
    WebSocketConnection.connectionURL_ = function (repoInfo, transportSessionId, lastSessionId, appCheckToken, applicationId) ***REMOVED***
        var urlParams = ***REMOVED***};
  ...
  ***REMOVED***;
     ...
     */
    WebSocketConnection.prototype.start = function () ***REMOVED*** };
  ...
  ***REMOVED***;
    WebSocketConnection.isAvailable = function () ***REMOVED***
        ...
        if (typeof navigator !== 'undefined' && navigator.userAgent) ***REMOVED***
            var oldAndroidRegex = /Android ([0-9]***REMOVED***0,}\.[0-9]***REMOVED***0,})/;
 ...
 */
var AppCheckTokenProvider = /** @class */ (function () ***REMOVED***
    function AppCheckTokenProvider(appName_, appCheckProvider) ***REMOVED***
        var _this = this;
        ...
        if (!this.appCheck) ***REMOVED***
            appCheckProvider === null || appCheckProvider === void 0 ? void 0 : appCheckProvider.get().then(function (appCheck) ***REMOVED*** return (_this.appCheck = appCheck); });
      ***REMOVED***
  ***REMOVED***
    AppCheckTokenProvider.prototype.getToken = function (forceRefresh) ***REMOVED***
        var _this = this;
        if (!this.appCheck) ***REMOVED***
  ...
  ***REMOVED***;
    AppCheckTokenProvider.prototype.addTokenChangeListener = function (listener) ***REMOVED***
        var _a;
        (_a = this.appCheckProvider) === null || _a === void 0 ? void 0 : _a.get().then(function (appCheck) ***REMOVED*** return appCheck.addTokenListener(listener); });
  ***REMOVED***;
    AppCheckTokenProvider.prototype.notifyForInvalidToken = function () ***REMOVED***
 ...
 */
var FirebaseAuthTokenProvider = /** @class */ (function () ***REMOVED***
    function FirebaseAuthTokenProvider(appName_, firebaseOptions_, authProvider_) ***REMOVED***
        var _this = this;
        this.appName_ = appName_;
        ...
        if (!this.auth_) ***REMOVED***
            authProvider_.onInit(function (auth) ***REMOVED*** return (_this.auth_ = auth); });
      ***REMOVED***
  ***REMOVED***
    FirebaseAuthTokenProvider.prototype.getToken = function (forceRefresh) ***REMOVED***
        var _this = this;
        if (!this.auth_) ***REMOVED***
            return new Promise(function (resolve, reject) ***REMOVED***
          ...
          ***REMOVED***);
      ***REMOVED***
        return this.auth_.getToken(forceRefresh).catch(function (error) ***REMOVED***
      ...
      ***REMOVED***);
  ***REMOVED***;
    FirebaseAuthTokenProvider.prototype.addTokenChangeListener = function (listener) ***REMOVED***
      ...
      ***REMOVED***
            ...
            this.authProvider_
                .get()
                .then(function (auth) ***REMOVED*** return auth.addAuthTokenListener(listener); });
      ***REMOVED***
  ***REMOVED***;
    FirebaseAuthTokenProvider.prototype.removeTokenChangeListener = function (listener) ***REMOVED***
        this.authProvider_
            .get()
            .then(function (auth) ***REMOVED*** return auth.removeAuthTokenListener(listener); });
  ***REMOVED***;
    FirebaseAuthTokenProvider.prototype.notifyForInvalidToken = function () ***REMOVED***
...
var EmulatorTokenProvider = /** @class */ (function () ***REMOVED***
    function EmulatorTokenProvider(accessToken) ***REMOVED***
        this.accessToken = accessToken;
  ***REMOVED***
    EmulatorTokenProvider.prototype.getToken = function (forceRefresh) ***REMOVED***
        return Promise.resolve(***REMOVED***
            accessToken: this.accessToken
      ***REMOVED***);
  ***REMOVED***;
    EmulatorTokenProvider.prototype.addTokenChangeListener = function (listener) ***REMOVED***
        // Invoke the listener immediately to match the behavior in Firebase Auth
        // (see packages/auth/src/auth.js#L1807)
        listener(this.accessToken);
  ***REMOVED***;
    EmulatorTokenProvider.prototype.removeTokenChangeListener = function (listener) ***REMOVED*** };
    EmulatorTokenProvider.prototype.notifyForInvalidToken = function () ***REMOVED*** };
...
var BrowserPollConnection = /** @class */ (function () ***REMOVED***
     ...
     */
    function BrowserPollConnection(connId, repoInfo, applicationId, appCheckToken, authToken, transportSessionId, lastSessionId) ***REMOVED***
        var _this = this;
        this.connId = connId;
        ...
        this.urlFn = function (params) ***REMOVED***
            ...
            if (_this.appCheckToken) ***REMOVED***
      ...
      ***REMOVED***, Math.floor(LP_CONNECT_TIMEOUT));
        ...
        executeWhenDOMReady(function () ***REMOVED***
          ...
          ***REMOVED***
            ...
            _this.scriptTagHolder = new FirebaseIFrameScriptHolder(function () ***REMOVED***
              ...
              ***REMOVED***
                ...
                if (!_this.scriptTagHolder) ***REMOVED***
          ...
          ***REMOVED***, _this.urlFn);
            ...
            if (_this.scriptTagHolder.uniqueCallbackIdentifier) ***REMOVED***
          ...
          ***REMOVED***
            if (_this.applicationId) ***REMOVED***
  ...
  ***REMOVED***;
     ...
     * No-op for polling
     */
    BrowserPollConnection.prototype.markConnectionHealthy = function () ***REMOVED*** };
    /**
 ...
 * they are available.
 */
var TransportManager = /** @class */ (function () ***REMOVED***
    /**
     * @param repoInfo - Metadata around the namespace we're connecting to
     */
    function TransportManager(repoInfo) ***REMOVED***
        this.initTransports_(repoInfo);
  ***REMOVED***
    Object.defineProperty(TransportManager, "ALL_TRANSPORTS", ***REMOVED***
        get: function () ***REMOVED***
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    Object.defineProperty(TransportManager, "IS_TRANSPORT_INITIALIZED", ***REMOVED***
        /**
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    TransportManager.prototype.initTransports_ = function (repoInfo) ***REMOVED***
        var e_1, _a;
      ...
      ***REMOVED***
            ...
            try ***REMOVED***
                for (var _b = tslib.__values(TransportManager.ALL_TRANSPORTS), _c = _b.next(); !_c.done; _c = _b.next()) ***REMOVED***
  ...
  ***REMOVED***;
     ...
     */
    TransportManager.prototype.initialTransport = function () ***REMOVED***
  ...
  ***REMOVED***;
     ...
     */
    TransportManager.prototype.upgradeTransport = function () ***REMOVED***
...
var Connection = /** @class */ (function () ***REMOVED***
     ...
     */
    function Connection(id, repoInfo_, applicationId_, appCheckToken_, authToken_, onMessage_, onReady_, onDisconnect_, onKill_, lastSessionId) ***REMOVED***
        this.id = id;
  ...
  ***REMOVED***;
    Connection.prototype.connReceiver_ = function (conn) ***REMOVED***
        ...
        return function (message) ***REMOVED***
            if (_this.state_ !== 2 /* RealtimeState.DISCONNECTED */) ***REMOVED***
...
var ServerActions = /** @class */ (function () ***REMOVED***
  ...
  ***REMOVED***
    ServerActions.prototype.put = function (pathString, data, onComplete, hash) ***REMOVED*** };
    ServerActions.prototype.merge = function (pathString, data, onComplete, hash) ***REMOVED*** };
    /**
     * Refreshes the auth token for the current connection.
     * @param token - The authentication token
     */
    ServerActions.prototype.refreshAuthToken = function (token) ***REMOVED*** };
    /**
     * Refreshes the app check token for the current connection.
     * @param token The app check token
     */
    ServerActions.prototype.refreshAppCheckToken = function (token) ***REMOVED*** };
    ServerActions.prototype.onDisconnectPut = function (pathString, data, onComplete) ***REMOVED*** };
    ServerActions.prototype.onDisconnectMerge = function (pathString, data, onComplete) ***REMOVED*** };
    ServerActions.prototype.onDisconnectCancel = function (pathString, onComplete) ***REMOVED*** };
    ServerActions.prototype.reportStats = function (stats) ***REMOVED*** };
...
var EventEmitter = /** @class */ (function () ***REMOVED***
  ...
  ***REMOVED***;
    EventEmitter.prototype.on = function (eventType, callback, context) ***REMOVED***
        ...
        this.listeners_[eventType].push(***REMOVED*** callback: callback, context: context });
  ...
  ***REMOVED***;
    EventEmitter.prototype.validateEventType_ = function (eventType) ***REMOVED***
        util.assert(this.allowedEvents_.find(function (et) ***REMOVED***
            return et === eventType;
...
function pathToUrlEncodedString(path) ***REMOVED***
    var pathString = '';
    for (var i = path.pieceNum_; i < path.pieces_.length; i++) ***REMOVED***
 ...
 *
 */
function pathSlice(path, begin) ***REMOVED***
    if (begin === void 0) ***REMOVED*** begin = 0; }
...
function pathParent(path) ***REMOVED***
  ...
  ***REMOVED***
    var pieces = [];
    for (var i = path.pieceNum_; i < path.pieces_.length - 1; i++) ***REMOVED***
...
function pathEquals(path, other) ***REMOVED***
    if (pathGetLength(path) !== pathGetLength(other)) ***REMOVED***
        return false;
  ***REMOVED***
    for (var i = path.pieceNum_, j = other.pieceNum_; i <= path.pieces_.length; i++, j++) ***REMOVED***
...
var PersistentConnection = /** @class */ (function (_super) ***REMOVED***
     ...
     */
    function PersistentConnection(repoInfo_, applicationId_, onDataUpdate_, onConnectStatus_, onServerInfoUpdate_, authTokenProvider_, appCheckTokenProvider_, authOverride_) ***REMOVED***
        ...
        _this.interruptReasons_ = ***REMOVED***};
        ...
        if (authOverride_ && !util.isNodeSdk()) ***REMOVED***
  ...
  ***REMOVED***
    PersistentConnection.prototype.sendRequest = function (action, body, onResponse) ***REMOVED***
        ...
        var msg = ***REMOVED*** r: curReqNum, a: action, b: body };
      ...
      ***REMOVED***
  ***REMOVED***;
    PersistentConnection.prototype.get = function (query) ***REMOVED***
      ...
      ***REMOVED***;
        var outstandingGet = ***REMOVED***
            action: 'g',
            request: request,
            onComplete: function (message) ***REMOVED***
  ...
  ***REMOVED***;
    PersistentConnection.prototype.listen = function (query, currentHashFn, tag, onComplete) ***REMOVED***
      ...
      ***REMOVED***
  ***REMOVED***;
    PersistentConnection.prototype.sendGet_ = function (index) ***REMOVED***
        var _this = this;
        var get = this.outstandingGets_[index];
        this.sendRequest('g', get.request, function (message) ***REMOVED***
            delete _this.outstandingGets_[index];
            _this.outstandingGetCount_--;
            if (_this.outstandingGetCount_ === 0) ***REMOVED***
                _this.outstandingGets_ = [];
          ***REMOVED***
  ...
  ***REMOVED***;
    PersistentConnection.prototype.sendListen_ = function (listenSpec) ***REMOVED***
        ...
        var req = ***REMOVED*** /*path*/ p: pathString };
        var action = 'q';
  ...
  ***REMOVED***;
    PersistentConnection.warnOnListenWarnings_ = function (payload, query) ***REMOVED***
        if (payload && typeof payload === 'object' && util.contains(payload, 'w')) ***REMOVED***
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  ***REMOVED***;
    PersistentConnection.prototype.refreshAuthToken = function (token) ***REMOVED***
      ...
      ***REMOVED***
            ...
            if (this.connected_) ***REMOVED***
                this.sendRequest('unauth', ***REMOVED***}, function () ***REMOVED*** });
          ***REMOVED***
      ***REMOVED***
  ...
  ***REMOVED***;
    PersistentConnection.prototype.reduceReconnectDelayIfAdminCredential_ = function (credential) ***REMOVED***
        ...
        if (isFirebaseSecret || util.isAdmin(credential)) ***REMOVED***
            this.log_('Admin auth credential detected.  Reducing max reconnect time.');
            this.maxReconnectDelay_ = RECONNECT_MAX_DELAY_FOR_ADMINS;
      ***REMOVED***
  ***REMOVED***;
    PersistentConnection.prototype.refreshAppCheckToken = function (token) ***REMOVED***
      ...
      ***REMOVED***
            ...
            if (this.connected_) ***REMOVED***
                this.sendRequest('unappeck', ***REMOVED***}, function () ***REMOVED*** });
          ***REMOVED***
      ***REMOVED***
  ***REMOVED***;
    /**
    ...
    PersistentConnection.prototype.tryAuth = function () ***REMOVED***
        ...
        if (this.connected_ && this.authToken_) ***REMOVED***
            ...
            var requestData = ***REMOVED*** cred: token_1 };
  ...
  ***REMOVED***;
     ...
     * closed).
     */
    PersistentConnection.prototype.tryAppCheck = function () ***REMOVED***
        var _this = this;
        if (this.connected_ && this.appCheckToken_) ***REMOVED***
            this.sendRequest('appcheck', ***REMOVED*** 'token': this.appCheckToken_ }, function (res) ***REMOVED***
                var status = res[ /*status*/'s'];
                var data = res[ /*data*/'d'] || 'error';
                if (status === 'ok') ***REMOVED***
                    _this.invalidAppCheckTokenCount_ = 0;
  ...
  ***REMOVED***;
    PersistentConnection.prototype.sendOnDisconnect_ = function (action, pathString, data, onComplete) ***REMOVED***
        var request = ***REMOVED*** /*path*/ p: pathString, /*data*/ d: data };
  ...
  ***REMOVED***;
    PersistentConnection.prototype.reportStats = function (stats) ***REMOVED***
        ...
        if (this.connected_) ***REMOVED***
            var request = ***REMOVED*** /*counters*/ c: stats };
  ...
  ***REMOVED***;
    PersistentConnection.prototype.initConnection_ = function () ***REMOVED***
                ...
                switch (_b.label) ***REMOVED***
                    case 0:
                        ...
                        closeFn = function () ***REMOVED***
                          ...
                          ***REMOVED***
                      ***REMOVED***;
                        sendRequestFn = function (msg) ***REMOVED***
  ...
  ***REMOVED***;
    PersistentConnection.prototype.resume = function (reason) ***REMOVED***
        ...
        if (util.isEmpty(this.interruptReasons_)) ***REMOVED***
            this.reconnectDelay_ = RECONNECT_MIN_DELAY;
            if (!this.realtime_) ***REMOVED***
                this.scheduleConnect_(0);
          ***REMOVED***
  ...
  ***REMOVED***;
    PersistentConnection.prototype.onListenRevoked_ = function (pathString, query) ***REMOVED***
      ...
      ***REMOVED***
            ...
            queryId = query.map(function (q) ***REMOVED*** return ObjectToUniqueKey(q); }).join('$');
      ***REMOVED***
        var listen = this.removeListen_(pathString, queryId);
  ...
  ***REMOVED***;
    PersistentConnection.prototype.onAuthRevoked_ = function (statusCode, explanation) ***REMOVED***
      ...
      ***REMOVED***
  ***REMOVED***;
    PersistentConnection.prototype.onAppCheckRevoked_ = function (statusCode, explanation) ***REMOVED***
  ...
  ***REMOVED***;
    PersistentConnection.prototype.restoreState_ = function () ***REMOVED***
        ...
        try ***REMOVED***
            ...
            for (var _c = tslib.__values(this.listens.values()), _d = _c.next(); !_d.done; _d = _c.next()) ***REMOVED***
                var queries = _d.value;
                try ***REMOVED***
                    for (var _e = (e_2 = void 0, tslib.__values(queries.values())), _f = _e.next(); !_f.done; _f = _e.next()) ***REMOVED***
      ...
      ***REMOVED***
        for (var i = 0; i < this.outstandingGets_.length; i++) ***REMOVED***
  ...
  ***REMOVED***;
    ...
    PersistentConnection.prototype.sendConnectStats_ = function () ***REMOVED***
      ...
      ***REMOVED***
        ...
        if (util.isMobileCordova()) ***REMOVED***
            stats['framework.cordova'] = 1;
      ***REMOVED***
        else if (util.isReactNative()) ***REMOVED***
...
var SortedMapIterator = /** @class */ (function () ***REMOVED***
    ...
    function SortedMapIterator(node, startKey, comparator, isReverse_, resultGenerator_) ***REMOVED***
        if (resultGenerator_ === void 0) ***REMOVED*** resultGenerator_ = null; }
...
var SortedMap = /** @class */ (function () ***REMOVED***
    ...
    function SortedMap(comparator_, root_) ***REMOVED***
        if (root_ === void 0) ***REMOVED*** root_ = SortedMap.EMPTY_NODE; }
...
var priorityHashText = function (priority) ***REMOVED***
 ...
 */
var validatePriorityNode = function (priorityNode) ***REMOVED***
...
var LeafNode = /** @class */ (function () ***REMOVED***
    ...
    function LeafNode(value_, priorityNode_) ***REMOVED***
        if (priorityNode_ === void 0) ***REMOVED*** priorityNode_ = LeafNode.__childrenNodeConstructor.EMPTY_NODE; }
        this.value_ = value_;
...
var PriorityIndex = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(PriorityIndex, _super);
    function PriorityIndex() ***REMOVED***
        return _super !== null && _super.apply(this, arguments) || this;
  ***REMOVED***
    PriorityIndex.prototype.compare = function (a, b) ***REMOVED***
      ...
      ***REMOVED***
  ***REMOVED***;
    PriorityIndex.prototype.isDefinedOn = function (node) ***REMOVED***
        return !node.getPriority().isEmpty();
  ***REMOVED***;
    PriorityIndex.prototype.indexedValueChanged = function (oldNode, newNode) ***REMOVED***
        return !oldNode.getPriority().equals(newNode.getPriority());
  ***REMOVED***;
    PriorityIndex.prototype.minPost = function () ***REMOVED***
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return NamedNode.MIN;
  ***REMOVED***;
    PriorityIndex.prototype.maxPost = function () ***REMOVED***
        return new NamedNode(MAX_NAME, new LeafNode('[PRIORITY-POST]', MAX_NODE$1));
  ***REMOVED***;
    PriorityIndex.prototype.makePost = function (indexValue, name) ***REMOVED***
  ...
  ***REMOVED***;
     ...
     * @returns String representation for inclusion in a query spec
     */
    PriorityIndex.prototype.toString = function () ***REMOVED***
        return '.priority';
  ***REMOVED***;
...
var Base12Num = /** @class */ (function () ***REMOVED***
    function Base12Num(length) ***REMOVED***
      ...
      ***REMOVED***;
        var bitMask = function (bits) ***REMOVED*** return parseInt(Array(bits + 1).join('1'), 2); };
 ...
 */
var buildChildSet = function (childList, cmp, keyFn, mapSortFn) ***REMOVED***
    childList.sort(cmp);
...
var IndexMap = /** @class */ (function () ***REMOVED***
  ...
  ***REMOVED***;
    IndexMap.prototype.addIndex = function (indexDefinition, existingChildren) ***REMOVED***
      ...
      ***REMOVED***
        ...
        var newIndexSet = tslib.__assign(***REMOVED***}, this.indexSet_);
        newIndexSet[indexName] = indexDefinition;
        var newIndexes = tslib.__assign(***REMOVED***}, this.indexes_);
  ...
  ***REMOVED***;
    ...
    IndexMap.prototype.addToIndexes = function (namedNode, existingChildren) ***REMOVED***
        var _this = this;
        var newIndexes = util.map(this.indexes_, function (indexedChildren, indexName) ***REMOVED***
  ...
  ***REMOVED***;
    ...
    IndexMap.prototype.removeFromIndexes = function (namedNode, existingChildren) ***REMOVED***
        var newIndexes = util.map(this.indexes_, function (indexedChildren) ***REMOVED***
            if (indexedChildren === fallbackObject) ***REMOVED***
...
var ChildrenNode = /** @class */ (function () ***REMOVED***
  ...
  ***REMOVED***;
    ChildrenNode.prototype.getIteratorFrom = function (startPost, indexDefinition) ***REMOVED***
        var idx = this.resolveIndex_(indexDefinition);
        if (idx) ***REMOVED***
            return idx.getIteratorFrom(startPost, function (key) ***REMOVED*** return key; });
      ***REMOVED***
        else ***REMOVED***
 ...
 */
function nodeFromJSON(json, priority) ***REMOVED***
    if (priority === void 0) ***REMOVED*** priority = null; }
  ...
  ***REMOVED***
    if (!(json instanceof Array) && USE_HINZE) ***REMOVED***
        ...
        each(hinzeJsonObj, function (key, child) ***REMOVED***
      ...
      ***REMOVED***);
        if (children_1.length === 0) ***REMOVED***
            return ChildrenNode.EMPTY_NODE;
      ***REMOVED***
        var childSet = buildChildSet(children_1, NAME_ONLY_COMPARATOR, function (namedNode) ***REMOVED*** return namedNode.name; }, NAME_COMPARATOR);
  ...
  ***REMOVED***
        ...
        each(json, function (key, childData) ***REMOVED***
            if (util.contains(json, key)) ***REMOVED***
                if (key.substring(0, 1) !== '.') ***REMOVED***
                    // ignore metadata nodes.
 ...
 * limitations under the License.
 */
var ValueIndex = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(ValueIndex, _super);
    function ValueIndex() ***REMOVED***
        return _super !== null && _super.apply(this, arguments) || this;
  ***REMOVED***
    ValueIndex.prototype.compare = function (a, b) ***REMOVED***
      ...
      ***REMOVED***
  ***REMOVED***;
    ValueIndex.prototype.isDefinedOn = function (node) ***REMOVED***
        return true;
  ***REMOVED***;
    ValueIndex.prototype.indexedValueChanged = function (oldNode, newNode) ***REMOVED***
        return !oldNode.equals(newNode);
  ***REMOVED***;
    ValueIndex.prototype.minPost = function () ***REMOVED***
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return NamedNode.MIN;
  ***REMOVED***;
    ValueIndex.prototype.maxPost = function () ***REMOVED***
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return NamedNode.MAX;
  ***REMOVED***;
    ValueIndex.prototype.makePost = function (indexValue, name) ***REMOVED***
        var valueNode = nodeFromJSON(indexValue);
  ...
  ***REMOVED***;
     ...
     * @returns String representation for inclusion in a query spec
     */
    ValueIndex.prototype.toString = function () ***REMOVED***
        return '.value';
  ***REMOVED***;
    return ValueIndex;
}(Index));
...
function changeValue(snapshotNode) ***REMOVED***
    return ***REMOVED*** type: "value" /* ChangeType.VALUE */, snapshotNode: snapshotNode };
}
function changeChildAdded(childName, snapshotNode) ***REMOVED***
    return ***REMOVED*** type: "child_added" /* ChangeType.CHILD_ADDED */, snapshotNode: snapshotNode, childName: childName };
}
function changeChildRemoved(childName, snapshotNode) ***REMOVED***
    return ***REMOVED*** type: "child_removed" /* ChangeType.CHILD_REMOVED */, snapshotNode: snapshotNode, childName: childName };
}
...
function changeChildMoved(childName, snapshotNode) ***REMOVED***
    return ***REMOVED*** type: "child_moved" /* ChangeType.CHILD_MOVED */, snapshotNode: snapshotNode, childName: childName };
}
...
var LimitedFilter = /** @class */ (function () ***REMOVED***
    function LimitedFilter(params) ***REMOVED***
      ...
      ***REMOVED***;
        this.withinEndPost = function (node) ***REMOVED***
  ...
  ***REMOVED***;
    LimitedFilter.prototype.fullLimitUpdateChild_ = function (snap, childKey, childSnap, source, changeAccumulator) ***REMOVED***
        ...
        if (this.reverse_) ***REMOVED***
            ...
            cmp = function (a, b) ***REMOVED*** return indexCmp_1(b, a); };
      ***REMOVED***
        else ***REMOVED***
...
var QueryParams = /** @class */ (function () ***REMOVED***
  ...
  ***REMOVED***;
    QueryParams.prototype.copy = function () ***REMOVED***
...
var ReadonlyRestClient = /** @class */ (function (_super) ***REMOVED***
     ...
     * @param onDataUpdate_ - A callback for new data from the server
     */
    function ReadonlyRestClient(repoInfo_, onDataUpdate_, authTokenProvider_, appCheckTokenProvider_) ***REMOVED***
        var _this = _super.call(this) || this;
        _this.repoInfo_ = repoInfo_;
        _this.onDataUpdate_ = onDataUpdate_;
        _this.authTokenProvider_ = authTokenProvider_;
  ...
  ***REMOVED***;
    ...
    ReadonlyRestClient.prototype.listen = function (query, currentHashFn, tag, onComplete) ***REMOVED***
        ...
        var thisListen = ***REMOVED***};
        this.listens_[listenId] = thisListen;
        var queryStringParameters = queryParamsToRestQueryStringParameters(query._queryParams);
        this.restRequest_(pathString + '.json', queryStringParameters, function (error, result) ***REMOVED***
            var data = result;
          ...
          ***REMOVED***
            if (util.safeGet(_this.listens_, listenId) === thisListen) ***REMOVED***
                var status_1;
                if (!error) ***REMOVED***
                    status_1 = 'ok';
              ***REMOVED***
  ...
  ***REMOVED***;
    ReadonlyRestClient.prototype.get = function (query) ***REMOVED***
        var _this = this;
        var queryStringParameters = queryParamsToRestQueryStringParameters(query._queryParams);
        var pathString = query._path.toString();
        var deferred = new util.Deferred();
  ...
  ***REMOVED***;
    ...
    ReadonlyRestClient.prototype.restRequest_ = function (pathString, queryStringParameters, callback) ***REMOVED***
        var _this = this;
        if (queryStringParameters === void 0) ***REMOVED*** queryStringParameters = ***REMOVED***}; }
        queryStringParameters['format'] = 'export';
        return Promise.all([
            this.authTokenProvider_.getToken(/*forceRefresh=*/ false),
            this.appCheckTokenProvider_.getToken(/*forceRefresh=*/ false)
        ]).then(function (_a) ***REMOVED***
...
function sparseSnapshotTreeForEachTree(sparseSnapshotTree, prefixPath, func) ***REMOVED***
  ...
  ***REMOVED***
        ...
        sparseSnapshotTreeForEachChild(sparseSnapshotTree, function (key, tree) ***REMOVED***
...
function sparseSnapshotTreeForEachChild(sparseSnapshotTree, func) ***REMOVED***
    sparseSnapshotTree.children.forEach(function (tree, key) ***REMOVED***
        func(key, tree);
  ***REMOVED***);
}
...
var StatsListener = /** @class */ (function () ***REMOVED***
  ...
  ***REMOVED***
    StatsListener.prototype.get = function () ***REMOVED***
        ...
        var delta = tslib.__assign(***REMOVED***}, newStats);
        if (this.last_) ***REMOVED***
            each(this.last_, function (stat, value) ***REMOVED***
...
var StatsReporter = /** @class */ (function () ***REMOVED***
  ...
  ***REMOVED***
    StatsReporter.prototype.reportStats_ = function () ***REMOVED***
        ...
        var reportedStats = ***REMOVED***};
        var haveStatsToReport = false;
        each(stats, function (stat, value) ***REMOVED***
            if (value > 0 && util.contains(_this.statsToReport_, stat)) ***REMOVED***
 ...
 * limitations under the License.
 */
var ListenComplete = /** @class */ (function () ***REMOVED***
    function ListenComplete(source, path) ***REMOVED***
        this.source = source;
        this.path = path;
        /** @inheritDoc */
        this.type = OperationType.LISTEN_COMPLETE;
  ***REMOVED***
    ListenComplete.prototype.operationForChild = function (childName) ***REMOVED***
        if (pathIsEmpty(this.path)) ***REMOVED***
 ...
 * limitations under the License.
 */
var Overwrite = /** @class */ (function () ***REMOVED***
    function Overwrite(source, path, snap) ***REMOVED***
        this.source = source;
        this.path = path;
        this.snap = snap;
        /** @inheritDoc */
  ...
  ***REMOVED***
    Overwrite.prototype.operationForChild = function (childName) ***REMOVED***
        if (pathIsEmpty(this.path)) ***REMOVED***
...
function eventGeneratorGenerateEventsForType(eventGenerator, events, eventType, changes, registrations, eventCache) ***REMOVED***
    var filteredChanges = changes.filter(function (change) ***REMOVED*** return change.type === eventType; });
    filteredChanges.sort(function (a, b) ***REMOVED***
...
function newViewCache(eventCache, serverCache) ***REMOVED***
    return ***REMOVED*** eventCache: eventCache, serverCache: serverCache };
}
...
var ImmutableTree = /** @class */ (function () ***REMOVED***
    function ImmutableTree(value, children) ***REMOVED***
        if (children === void 0) ***REMOVED*** children = EmptyChildren(); }
        this.value = value;
        this.children = children;
  ***REMOVED***
    ImmutableTree.fromObject = function (obj) ***REMOVED***
        var tree = new ImmutableTree(null);
        each(obj, function (childPath, childSnap) ***REMOVED***
  ...
  ***REMOVED***;
    ...
    ImmutableTree.prototype.findRootMostValueAndPath = function (relativePath) ***REMOVED***
        return this.findRootMostMatchingPathAndValue(relativePath, function () ***REMOVED*** return true; });
  ***REMOVED***;
    /**
...
var CompoundWrite = /** @class */ (function () ***REMOVED***
  ...
  ***REMOVED***
    CompoundWrite.empty = function () ***REMOVED***
...
function compoundWriteAddWrites(compoundWrite, path, updates) ***REMOVED***
    ...
    each(updates, function (childKey, node) ***REMOVED***
...
function compoundWriteGetCompleteChildren(compoundWrite) ***REMOVED***
  ...
  ***REMOVED***
        ...
        compoundWrite.writeTree_.children.inorderTraversal(function (childName, childTree) ***REMOVED***
...
function writeTreeGetWrite(writeTree, writeId) ***REMOVED***
    for (var i = 0; i < writeTree.allWrites.length; i++) ***REMOVED***
...
function writeTreeRemoveWrite(writeTree, writeId) ***REMOVED***
    ...
    var idx = writeTree.allWrites.findIndex(function (s) ***REMOVED***
  ...
  ***REMOVED***
      ...
      ***REMOVED***
            ...
            each(children, function (childName) ***REMOVED***
...
function writeTreeRecordContainsPath_(writeRecord, path) ***REMOVED***
  ...
  ***REMOVED***
        ...
        for (var childName in writeRecord.children) ***REMOVED***
...
function writeTreeCalcCompleteEventChildren(writeTree, treePath, completeServerChildren) ***REMOVED***
  ...
  ***REMOVED***
      ...
      ***REMOVED***);
        ...
        compoundWriteGetCompleteChildren(merge_1).forEach(function (namedNode) ***REMOVED***
  ...
  ***REMOVED***
        ...
        var merge = compoundWriteChildCompoundWrite(writeTree.visibleWrites, treePath);
        compoundWriteGetCompleteChildren(merge).forEach(function (namedNode) ***REMOVED***
            completeChildren = completeChildren.updateImmediateChild(namedNode.name, namedNode.node);
      ***REMOVED***);
        return completeChildren;
  ***REMOVED***
...
var WriteTreeCompleteChildSource = /** @class */ (function () ***REMOVED***
    function WriteTreeCompleteChildSource(writes_, viewCache_, optCompleteServerCache_) ***REMOVED***
        if (optCompleteServerCache_ === void 0) ***REMOVED*** optCompleteServerCache_ = null; }
...
function viewProcessorApplyOperation(viewProcessor, oldViewCache, operation, writesCache, completeCache) ***REMOVED***
  ...
  ***REMOVED***
    ...
    return ***REMOVED*** viewCache: newViewCache, changes: changes };
}
...
var View = /** @class */ (function () ***REMOVED***
  ...
  ***REMOVED***
    Object.defineProperty(View.prototype, "query", ***REMOVED***
        get: function () ***REMOVED***
            return this.query_;
      ***REMOVED***,
        enumerable: false,
...
function viewRemoveEventRegistration(view, eventRegistration, cancelError) ***REMOVED***
    ...
    if (cancelError) ***REMOVED***
        ...
        view.eventRegistrations_.forEach(function (registration) ***REMOVED***
  ...
  ***REMOVED***
    if (eventRegistration) ***REMOVED***
        var remaining = [];
        for (var i = 0; i < view.eventRegistrations_.length; ++i) ***REMOVED***
...
function syncPointApplyOperation(syncPoint, operation, writesCache, optCompleteServerCache) ***REMOVED***
  ...
  ***REMOVED***
        ...
        var events = [];
        try ***REMOVED***
            for (var _b = tslib.__values(syncPoint.views.values()), _c = _b.next(); !_c.done; _c = _b.next()) ***REMOVED***
                var view = _c.value;
...
function syncPointRemoveEventRegistration(syncPoint, query, eventRegistration, cancelError) ***REMOVED***
    ...
    if (queryId === 'default') ***REMOVED***
        try ***REMOVED***
            ...
            for (var _b = tslib.__values(syncPoint.views.entries()), _c = _b.next(); !_c.done; _c = _b.next()) ***REMOVED***
  ...
  ***REMOVED***
    return ***REMOVED*** removed: removed, events: cancelEvents };
}
...
function syncPointGetCompleteServerCache(syncPoint, path) ***REMOVED***
    ...
    try ***REMOVED***
      ...
      ***REMOVED***
  ***REMOVED***
    catch (e_4_1) ***REMOVED*** e_4 = ***REMOVED*** error: e_4_1 }; }
    finally ***REMOVED***
        try ***REMOVED***
            if (_c && !_c.done && (_a = _b.return)) _a.call(_b);
      ***REMOVED***
        finally ***REMOVED*** if (e_4) throw e_4.error; }
  ***REMOVED***
    return serverCache;
}
...
function syncPointGetCompleteView(syncPoint) ***REMOVED***
    ...
    try ***REMOVED***
      ...
      ***REMOVED***
  ***REMOVED***
    catch (e_5_1) ***REMOVED*** e_5 = ***REMOVED*** error: e_5_1 }; }
    finally ***REMOVED***
        try ***REMOVED***
            if (_c && !_c.done && (_a = _b.return)) _a.call(_b);
      ***REMOVED***
        finally ***REMOVED*** if (e_5) throw e_5.error; }
  ***REMOVED***
    return null;
}
 ...
 * @returns Events to raise.
 */
function syncTreeAckUserWrite(syncTree, writeId, revert) ***REMOVED***
    if (revert === void 0) ***REMOVED*** revert = false; }
  ...
  ***REMOVED***
      ...
      ***REMOVED***
            ...
            each(write.children, function (pathString) ***REMOVED***
 ...
 * @returns Cancel events, if cancelError was provided.
 */
function syncTreeRemoveEventRegistration(syncTree, query, eventRegistration, cancelError, skipListenerDedup) ***REMOVED***
    if (skipListenerDedup === void 0) ***REMOVED*** skipListenerDedup = false; }
      ...
      ***REMOVED***
        ...
        if (!skipListenerDedup) ***REMOVED***
            ...
            var removingDefault = -1 !==
              ...
              ***REMOVED***);
            var covered = syncTree.syncPointTree_.findOnPath(path, function (relativePath, parentSyncPoint) ***REMOVED***
 ...
 * @returns Events to raise.
 */
function syncTreeAddEventRegistration(syncTree, query, eventRegistration, skipSetupListener) ***REMOVED***
    if (skipSetupListener === void 0) ***REMOVED*** skipSetupListener = false; }
    var path = query._path;
    var serverCache = null;
    var foundAncestorDefaultView = false;
    // Any covering writes will necessarily be at the root, so really all we need to find is the server cache.
    // Consider optimizing this once there's a better understanding of what actual behavior will be.
    syncTree.syncPointTree_.foreachOnPath(path, function (pathToSyncPoint, sp) ***REMOVED***
...
function syncTreeCalcCompleteEventCache(syncTree, path, writeIdsToExclude) ***REMOVED***
    ...
    var serverCache = syncTree.syncPointTree_.findOnPath(path, function (pathSoFar, syncPoint) ***REMOVED***
...
function syncTreeCollectDistinctViewsForSubTree_(subtree) ***REMOVED***
    return subtree.fold(function (relativePath, maybeChildSyncPoint, childMap) ***REMOVED***
      ...
      ***REMOVED***
          ...
          ***REMOVED***
            each(childMap, function (_key, childViews) ***REMOVED***
  ...
  ***REMOVED***
        ...
        var queriesToStop = subtree.fold(function (relativePath, maybeChildSyncPoint, childMap) ***REMOVED***
          ...
          ***REMOVED***
                ...
                var queries_1 = [];
                if (maybeChildSyncPoint) ***REMOVED***
                    queries_1 = queries_1.concat(syncPointGetQueryViews(maybeChildSyncPoint).map(function (view) ***REMOVED*** return view.query; }));
              ***REMOVED***
                each(childMap, function (_key, childQueries) ***REMOVED***
 ...
 * limitations under the License.
 */
var ExistingValueProvider = /** @class */ (function () ***REMOVED***
    function ExistingValueProvider(node_) ***REMOVED***
        this.node_ = node_;
  ***REMOVED***
    ExistingValueProvider.prototype.getImmediateChild = function (childName) ***REMOVED***
        var child = this.node_.getImmediateChild(childName);
        return new ExistingValueProvider(child);
  ***REMOVED***;
    ExistingValueProvider.prototype.node = function () ***REMOVED***
        return this.node_;
  ***REMOVED***;
    return ExistingValueProvider;
}());
var DeferredValueProvider = /** @class */ (function () ***REMOVED***
    function DeferredValueProvider(syncTree, path) ***REMOVED***
        this.syncTree_ = syncTree;
        this.path_ = path;
  ***REMOVED***
    DeferredValueProvider.prototype.getImmediateChild = function (childName) ***REMOVED***
        var childPath = pathChild(this.path_, childName);
        return new DeferredValueProvider(this.syncTree_, childPath);
  ***REMOVED***;
    DeferredValueProvider.prototype.node = function () ***REMOVED***
 ...
 */
var generateWithValues = function (values) ***REMOVED***
 ...
 */
var resolveDeferredLeafValue = function (value, existingVal, serverValues) ***REMOVED***
...
var resolveScalarDeferredValue = function (op, existing, serverValues) ***REMOVED***
    switch (op) ***REMOVED***
        case 'timestamp':
...
var resolveComplexDeferredValue = function (op, existing, unused) ***REMOVED***
 ...
 */
var resolveDeferredValueTree = function (path, node, syncTree, serverValues) ***REMOVED***
 ...
 */
var resolveDeferredValueSnapshot = function (node, existing, serverValues) ***REMOVED***
...
var Tree = /** @class */ (function () ***REMOVED***
     ...
     */
    function Tree(name, parent, node) ***REMOVED***
        if (name === void 0) ***REMOVED*** name = ''; }
        if (parent === void 0) ***REMOVED*** parent = null; }
        if (node === void 0) ***REMOVED*** node = ***REMOVED*** children: ***REMOVED***}, childCount: 0 }; }
        this.name = name;
        this.parent = parent;
        this.node = node;
  ***REMOVED***
...
function treeSubTree(tree, pathObj) ***REMOVED***
    ...
    while (next !== null) ***REMOVED***
        var childNode = util.safeGet(child.node.children, next) || ***REMOVED***
            children: ***REMOVED***},
            childCount: 0
      ***REMOVED***;
...
function treeForEachChild(tree, action) ***REMOVED***
    each(tree.node.children, function (child, childTree) ***REMOVED***
...
function treeForEachDescendant(tree, action, includeSelf, childrenFirst) ***REMOVED***
    if (includeSelf && !childrenFirst) ***REMOVED***
        action(tree);
  ***REMOVED***
    treeForEachChild(tree, function (child) ***REMOVED***
...
var isValidKey = function (key) ***REMOVED***
    return (typeof key === 'string' && key.length !== 0 && !INVALID_KEY_REGEX_.test(key));
};
var isValidPathString = function (pathString) ***REMOVED***
    return (typeof pathString === 'string' &&
        pathString.length !== 0 &&
        !INVALID_PATH_REGEX_.test(pathString));
};
var isValidRootPathString = function (pathString) ***REMOVED***
...
var isValidPriority = function (priority) ***REMOVED***
 ...
 */
var validateFirebaseDataArg = function (fnName, value, path, optional) ***REMOVED***
 ...
 */
var validateFirebaseData = function (errorPrefix, data, path_) ***REMOVED***
  ...
  ***REMOVED***
    ...
    if (typeof data === 'string' &&
        ...
        util.stringLength(data) > MAX_LEAF_SIZE_) ***REMOVED***
        throw new Error(errorPrefix +
 ...
 */
var validateFirebaseMergePaths = function (errorPrefix, mergePaths) ***REMOVED***
    var i, curPath;
 ...
 */
var validateFirebaseMergeDataArg = function (fnName, data, path, optional) ***REMOVED***
...
var validatePriority = function (fnName, priority, optional) ***REMOVED***
...
var validateKey = function (fnName, argumentName, key, optional) ***REMOVED***
 ...
 * @internal
 */
var validatePathString = function (fnName, argumentName, pathString, optional) ***REMOVED***
...
var validateRootPathString = function (fnName, argumentName, pathString, optional) ***REMOVED***
    if (pathString) ***REMOVED***
        // Allow '/.info/' at the beginning.
        pathString = pathString.replace(/^\/*\.info(\/|$)/, '/');
  ***REMOVED***
 ...
 * @internal
 */
var validateWritablePath = function (fnName, path) ***REMOVED***
    if (pathGetFront(path) === '.info') ***REMOVED***
        throw new Error(fnName + " failed = Can't modify data under /.info/");
  ***REMOVED***
};
var validateUrl = function (fnName, parsedUrl) ***REMOVED***
    ...
    for (var i = 0; i < eventDataList.length; i++) ***REMOVED***
      ...
      ***REMOVED***
        if (currList === null) ***REMOVED***
            currList = ***REMOVED*** events: [], path: path };
      ***REMOVED***
...
function eventQueueRaiseEventsAtPath(eventQueue, path, eventDataList) ***REMOVED***
    ...
    eventQueueRaiseQueuedEventsMatchingPredicate(eventQueue, function (eventPath) ***REMOVED***
...
function eventQueueRaiseQueuedEventsMatchingPredicate(eventQueue, predicate) ***REMOVED***
    ...
    var sentAll = true;
    for (var i = 0; i < eventQueue.eventLists_.length; i++) ***REMOVED***
...
function eventListRaise(eventList) ***REMOVED***
    for (var i = 0; i < eventList.events.length; i++) ***REMOVED***
...
var Repo = /** @class */ (function () ***REMOVED***
    function Repo(repoInfo_, forceRestClient_, authTokenProvider_, appCheckProvider_) ***REMOVED***
        this.repoInfo_ = repoInfo_;
...
function repoStart(repo, appId, authOverride) ***REMOVED***
    ...
    if (repo.forceRestClient_ || beingCrawled()) ***REMOVED***
        repo.server_ = new ReadonlyRestClient(repo.repoInfo_, function (pathString, data, isMerge, tag) ***REMOVED***
            repoOnDataUpdate(repo, pathString, data, isMerge, tag);
      ***REMOVED***, repo.authTokenProvider_, repo.appCheckProvider_);
        // Minor hack: Fire onConnect immediately, since there's no actual connection.
        setTimeout(function () ***REMOVED*** return repoOnConnectStatus(repo, /* connectStatus= */ true); }, 0);
  ***REMOVED***
    else ***REMOVED***
        ...
        if (typeof authOverride !== 'undefined' && authOverride !== null) ***REMOVED***
          ...
          ***REMOVED***
      ***REMOVED***
        repo.persistentConnection_ = new PersistentConnection(repo.repoInfo_, appId, function (pathString, data, isMerge, tag) ***REMOVED***
            repoOnDataUpdate(repo, pathString, data, isMerge, tag);
      ***REMOVED***, function (connectStatus) ***REMOVED***
            repoOnConnectStatus(repo, connectStatus);
      ***REMOVED***, function (updates) ***REMOVED***
            repoOnServerInfoUpdate(repo, updates);
      ***REMOVED***, repo.authTokenProvider_, repo.appCheckProvider_, authOverride);
        repo.server_ = repo.persistentConnection_;
  ***REMOVED***
    repo.authTokenProvider_.addTokenChangeListener(function (token) ***REMOVED***
        repo.server_.refreshAuthToken(token);
  ***REMOVED***);
    repo.appCheckProvider_.addTokenChangeListener(function (result) ***REMOVED***
        repo.server_.refreshAppCheckToken(result.token);
  ***REMOVED***);
    // In the case of multiple Repos for the same repoInfo (i.e. there are multiple Firebase.Contexts being used),
    // we only want to create one StatsReporter.  As such, we'll report stats over the first Repo created.
    repo.statsReporter_ = statsManagerGetOrCreateReporter(repo.repoInfo_, function () ***REMOVED*** return new StatsReporter(repo.stats_, repo.server_); });
    // Used for .info.
    ...
    repo.infoSyncTree_ = new SyncTree(***REMOVED***
        startListening: function (query, tag, currentHashFn, onComplete) ***REMOVED***
          ...
          ***REMOVED***
            return infoEvents;
      ***REMOVED***,
        stopListening: function () ***REMOVED*** }
  ***REMOVED***);
    repoUpdateInfo(repo, 'connected', false);
    repo.serverSyncTree_ = new SyncTree(***REMOVED***
        startListening: function (query, tag, currentHashFn, onComplete) ***REMOVED***
            repo.server_.listen(query, currentHashFn, tag, function (status, data) ***REMOVED***
...
function repoOnDataUpdate(repo, pathString, data, isMerge, tag) ***REMOVED***
    ...
    if (tag) ***REMOVED***
        if (isMerge) ***REMOVED***
            var taggedChildren = util.map(data, function (raw) ***REMOVED*** return nodeFromJSON(raw); });
  ...
  ***REMOVED***
        ...
        var changedChildren = util.map(data, function (raw) ***REMOVED*** return nodeFromJSON(raw); });
...
function repoOnServerInfoUpdate(repo, updates) ***REMOVED***
    each(updates, function (key, value) ***REMOVED***
...
function repoGetValue(repo, query, eventRegistration) ***REMOVED***
  ...
  ***REMOVED***
    return repo.server_.get(query).then(function (payload) ***REMOVED***
...
function repoSetWithPriority(repo, path, newVal, newPriority, onComplete) ***REMOVED***
  ...
  ***REMOVED***);
    ...
    repo.server_.put(path.toString(), newNodeUnresolved.val(/*export=*/ true), function (status, errorReason) ***REMOVED***
...
function repoUpdate(repo, path, childrenToMerge, onComplete) ***REMOVED***
    ...
    var empty = true;
    var serverValues = repoGenerateServerValues(repo);
    var changedChildren = ***REMOVED***};
    each(childrenToMerge, function (changedKey, changedValue) ***REMOVED***
        empty = false;
  ...
  ***REMOVED***);
    if (!empty) ***REMOVED***
        ...
        eventQueueQueueEvents(repo.eventQueue_, events);
        repo.server_.merge(path.toString(), childrenToMerge, function (status, errorReason) ***REMOVED***
            var success = status === 'ok';
            if (!success) ***REMOVED***
          ...
          ***REMOVED***
            ...
            repoCallOnCompleteCallback(repo, onComplete, status, errorReason);
      ***REMOVED***);
        each(childrenToMerge, function (changedPath) ***REMOVED***
...
function repoRunOnDisconnectEvents(repo) ***REMOVED***
    ...
    sparseSnapshotTreeForEachTree(repo.onDisconnect_, newEmptyPath(), function (path, node) ***REMOVED***
        var resolved = resolveDeferredValueTree(path, node, repo.serverSyncTree_, serverValues);
        sparseSnapshotTreeRemember(resolvedOnDisconnectTree, path, resolved);
  ***REMOVED***);
    var events = [];
    sparseSnapshotTreeForEachTree(resolvedOnDisconnectTree, newEmptyPath(), function (path, snap) ***REMOVED***
...
function repoOnDisconnectCancel(repo, path, onComplete) ***REMOVED***
    repo.server_.onDisconnectCancel(path.toString(), function (status, errorReason) ***REMOVED***
        if (status === 'ok') ***REMOVED***
...
function repoOnDisconnectSet(repo, path, value, onComplete) ***REMOVED***
    var newNode = nodeFromJSON(value);
    repo.server_.onDisconnectPut(path.toString(), newNode.val(/*export=*/ true), function (status, errorReason) ***REMOVED***
        if (status === 'ok') ***REMOVED***
...
function repoOnDisconnectUpdate(repo, path, childrenToMerge, onComplete) ***REMOVED***
    if (util.isEmpty(childrenToMerge)) ***REMOVED***
        log("onDisconnect().update() called with empty data.  Don't do anything.");
        repoCallOnCompleteCallback(repo, onComplete, 'ok', undefined);
        return;
  ***REMOVED***
    repo.server_.onDisconnectMerge(path.toString(), childrenToMerge, function (status, errorReason) ***REMOVED***
        if (status === 'ok') ***REMOVED***
            each(childrenToMerge, function (childName, childNode) ***REMOVED***
...
function repoStartTransaction(repo, path, transactionUpdate, onComplete, unwatcher, applyLocally) ***REMOVED***
  ...
  ***REMOVED***
        ...
        if (typeof newVal === 'object' &&
            newVal !== null &&
            util.contains(newVal, '.priority')) ***REMOVED***
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
 ...
 */
function repoSendReadyTransactions(repo, node) ***REMOVED***
    if (node === void 0) ***REMOVED*** node = repo.transactionQueueTree_; }
  ...
  ***REMOVED***
    if (treeGetValue(node)) ***REMOVED***
        ...
        var allRun = queue.every(function (transaction) ***REMOVED*** return transaction.status === 0 /* TransactionStatus.RUN */; });
  ...
  ***REMOVED***
        ...
        treeForEachChild(node, function (childNode) ***REMOVED***
...
function repoSendTransactionQueue(repo, path, queue) ***REMOVED***
  ...
  ***REMOVED***
    ...
    var pathToSend = path;
    // Send the put.
    repo.server_.put(pathToSend.toString(), dataToSend, function (status) ***REMOVED***
        ...
        if (status === 'ok') ***REMOVED***
            ...
            var _loop_1 = function (i) ***REMOVED***
                ...
                if (queue[i].onComplete) ***REMOVED***
                    ...
                    callbacks.push(function () ***REMOVED***
...
function repoBuildTransactionQueue(repo, transactionNode) ***REMOVED***
    ...
    transactionQueue.sort(function (a, b) ***REMOVED*** return a.order - b.order; });
...
function repoAggregateTransactionQueuesForNode(repo, node, queue) ***REMOVED***
    ...
    if (nodeQueue) ***REMOVED***
      ...
      ***REMOVED***
  ***REMOVED***
    treeForEachChild(node, function (child) ***REMOVED***
...
function repoAbortTransactions(repo, path) ***REMOVED***
    ...
    treeForEachAncestor(transactionNode, function (node) ***REMOVED***
        repoAbortTransactionsOnNode(repo, node);
  ***REMOVED***);
    repoAbortTransactionsOnNode(repo, transactionNode);
    treeForEachDescendant(transactionNode, function (node) ***REMOVED***
        repoAbortTransactionsOnNode(repo, node);
  ***REMOVED***);
    return affectedPath;
}
...
function decodeQuery(queryString) ***REMOVED***
  ...
  ***REMOVED***
    try ***REMOVED***
        for (var _b = tslib.__values(queryString.split('&')), _c = _b.next(); !_c.done; _c = _b.next()) ***REMOVED***
...
var parseRepoInfo = function (dataURL, nodeAdmin) ***REMOVED***
...
var parseDatabaseURL = function (dataURL) ***REMOVED***
 ...
 */
var nextPushId = (function () ***REMOVED***
 ...
 */
var CallbackContext = /** @class */ (function () ***REMOVED***
    function CallbackContext(snapshotCallback, cancelCallback) ***REMOVED***
        this.snapshotCallback = snapshotCallback;
        this.cancelCallback = cancelCallback;
  ***REMOVED***
    CallbackContext.prototype.onValue = function (expDataSnapshot, previousChildName) ***REMOVED***
        this.snapshotCallback.call(null, expDataSnapshot, previousChildName);
  ***REMOVED***;
    CallbackContext.prototype.onCancel = function (error) ***REMOVED***
        util.assert(this.hasCancelCallback, 'Raising a cancel event on a listener with no cancel callback');
        return this.cancelCallback.call(null, error);
  ***REMOVED***;
    Object.defineProperty(CallbackContext.prototype, "hasCancelCallback", ***REMOVED***
        get: function () ***REMOVED***
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    CallbackContext.prototype.matches = function (other) ***REMOVED***
 ...
 */
var OnDisconnect$1 = /** @class */ (function () ***REMOVED***
    /** @hideconstructor */
    function OnDisconnect(_repo, _path) ***REMOVED***
        this._repo = _repo;
        this._path = _path;
  ***REMOVED***
    /**
     ...
     */
    OnDisconnect.prototype.cancel = function () ***REMOVED***
        var deferred = new util.Deferred();
        repoOnDisconnectCancel(this._repo, this._path, deferred.wrapCallback(function () ***REMOVED*** }));
        return deferred.promise;
  ***REMOVED***;
    /**
     ...
     * @returns Resolves when synchronization to the server is complete.
     */
    OnDisconnect.prototype.remove = function () ***REMOVED***
        validateWritablePath('OnDisconnect.remove', this._path);
        var deferred = new util.Deferred();
        repoOnDisconnectSet(this._repo, this._path, null, deferred.wrapCallback(function () ***REMOVED*** }));
        return deferred.promise;
  ***REMOVED***;
    /**
     ...
     */
    OnDisconnect.prototype.set = function (value) ***REMOVED***
        validateWritablePath('OnDisconnect.set', this._path);
        validateFirebaseDataArg('OnDisconnect.set', value, this._path, false);
        var deferred = new util.Deferred();
        repoOnDisconnectSet(this._repo, this._path, value, deferred.wrapCallback(function () ***REMOVED*** }));
        return deferred.promise;
  ***REMOVED***;
    /**
     ...
     * @returns Resolves when synchronization to the Database is complete.
     */
    OnDisconnect.prototype.setWithPriority = function (value, priority) ***REMOVED***
        validateWritablePath('OnDisconnect.setWithPriority', this._path);
        validateFirebaseDataArg('OnDisconnect.setWithPriority', value, this._path, false);
        validatePriority('OnDisconnect.setWithPriority', priority, false);
        var deferred = new util.Deferred();
        repoOnDisconnectSetWithPriority(this._repo, this._path, value, priority, deferred.wrapCallback(function () ***REMOVED*** }));
        return deferred.promise;
  ***REMOVED***;
    /**
     ...
     * @returns Resolves when synchronization to the Database is complete.
     */
    OnDisconnect.prototype.update = function (values) ***REMOVED***
        validateWritablePath('OnDisconnect.update', this._path);
        validateFirebaseMergeDataArg('OnDisconnect.update', values, this._path, false);
        var deferred = new util.Deferred();
        repoOnDisconnectUpdate(this._repo, this._path, values, deferred.wrapCallback(function () ***REMOVED*** }));
        return deferred.promise;
  ***REMOVED***;
    return OnDisconnect;
}());
 ...
 * @internal
 */
var QueryImpl = /** @class */ (function () ***REMOVED***
    /**
     * @hideconstructor
     */
    function QueryImpl(_repo, _path, _queryParams, _orderByCalled) ***REMOVED***
        this._repo = _repo;
        this._path = _path;
        this._queryParams = _queryParams;
        this._orderByCalled = _orderByCalled;
  ***REMOVED***
    Object.defineProperty(QueryImpl.prototype, "key", ***REMOVED***
        get: function () ***REMOVED***
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    Object.defineProperty(QueryImpl.prototype, "ref", ***REMOVED***
        get: function () ***REMOVED***
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    Object.defineProperty(QueryImpl.prototype, "_queryIdentifier", ***REMOVED***
        get: function () ***REMOVED***
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    Object.defineProperty(QueryImpl.prototype, "_queryObject", ***REMOVED***
        /**
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    QueryImpl.prototype.isEqual = function (other) ***REMOVED***
  ...
  ***REMOVED***;
    QueryImpl.prototype.toJSON = function () ***REMOVED***
        return this.toString();
  ***REMOVED***;
    QueryImpl.prototype.toString = function () ***REMOVED***
 ...
 * @internal
 */
var ReferenceImpl = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(ReferenceImpl, _super);
    /** @hideconstructor */
    function ReferenceImpl(repo, path) ***REMOVED***
        return _super.call(this, repo, path, new QueryParams(), false) || this;
  ***REMOVED***
    Object.defineProperty(ReferenceImpl.prototype, "parent", ***REMOVED***
        get: function () ***REMOVED***
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    Object.defineProperty(ReferenceImpl.prototype, "root", ***REMOVED***
        get: function () ***REMOVED***
            var ref = this;
 ...
 */
var DataSnapshot$1 = /** @class */ (function () ***REMOVED***
    /**
    ...
    ref, _index) ***REMOVED***
        ...
        this._index = _index;
  ***REMOVED***
    Object.defineProperty(DataSnapshot.prototype, "priority", ***REMOVED***
        /**
  ...
  ***REMOVED***);
    Object.defineProperty(DataSnapshot.prototype, "key", ***REMOVED***
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    Object.defineProperty(DataSnapshot.prototype, "size", ***REMOVED***
  ...
  ***REMOVED***);
     ...
     */
    DataSnapshot.prototype.child = function (path) ***REMOVED***
  ...
  ***REMOVED***;
    ...
    DataSnapshot.prototype.forEach = function (action) ***REMOVED***
      ...
      ***REMOVED***
        ...
        return !!childrenNode.forEachChild(this._index, function (key, node) ***REMOVED***
  ...
  ***REMOVED***;
     ...
     *  `false`.
     */
    DataSnapshot.prototype.hasChild = function (path) ***REMOVED***
        var childPath = new Path(path);
...
function push(parent, value) ***REMOVED***
    ...
    var promise;
    if (value != null) ***REMOVED***
        promise = set(pushRef, value).then(function () ***REMOVED*** return pushRef; });
  ***REMOVED***
    else ***REMOVED***
...
function set(ref, value) ***REMOVED***
    ...
    /*priority=*/ null, deferred.wrapCallback(function () ***REMOVED*** }));
    return deferred.promise;
}
/**
...
function setPriority(ref, priority) ***REMOVED***
    ...
    var deferred = new util.Deferred();
    repoSetWithPriority(ref._repo, pathChild(ref._path, '.priority'), priority, null, deferred.wrapCallback(function () ***REMOVED*** }));
    return deferred.promise;
}
/**
...
function setWithPriority(ref, value, priority) ***REMOVED***
  ...
  ***REMOVED***
    var deferred = new util.Deferred();
    repoSetWithPriority(ref._repo, ref._path, value, priority, deferred.wrapCallback(function () ***REMOVED*** }));
    return deferred.promise;
}
/**
...
function update(ref, values) ***REMOVED***
    ...
    var deferred = new util.Deferred();
    repoUpdate(ref._repo, ref._path, values, deferred.wrapCallback(function () ***REMOVED*** }));
    return deferred.promise;
}
/**
...
function get(query) ***REMOVED***
    ...
    var callbackContext = new CallbackContext(function () ***REMOVED*** });
    var container = new ValueEventRegistration(callbackContext);
    return repoGetValue(query._repo, query, container).then(function (node) ***REMOVED***
...
var ValueEventRegistration = /** @class */ (function () ***REMOVED***
    function ValueEventRegistration(callbackContext) ***REMOVED***
...
var ChildEventRegistration = /** @class */ (function () ***REMOVED***
    function ChildEventRegistration(eventType, callbackContext) ***REMOVED***
        this.eventType = eventType;
        this.callbackContext = callbackContext;
  ***REMOVED***
...
function addEventListener(query, eventType, callback, cancelCallbackOrListenOptions, options) ***REMOVED***
  ...
  ***REMOVED***
    if (options && options.onlyOnce) ***REMOVED***
        ...
        var onceCallback = function (dataSnapshot, previousChildName) ***REMOVED***
  ...
  ***REMOVED***
    ...
    return function () ***REMOVED*** return repoRemoveEventCallbackForQuery(query._repo, query, container); };
}
 ...
 */
var QueryConstraint = /** @class */ (function () ***REMOVED***
    function QueryConstraint() ***REMOVED***
  ***REMOVED***
    return QueryConstraint;
}());
var QueryEndAtConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryEndAtConstraint, _super);
    function QueryEndAtConstraint(_value, _key) ***REMOVED***
        var _this = _super.call(this) || this;
        _this._value = _value;
        _this._key = _key;
        return _this;
  ***REMOVED***
    QueryEndAtConstraint.prototype._apply = function (query) ***REMOVED***
...
var QueryEndBeforeConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryEndBeforeConstraint, _super);
    function QueryEndBeforeConstraint(_value, _key) ***REMOVED***
        var _this = _super.call(this) || this;
        _this._value = _value;
        _this._key = _key;
        return _this;
  ***REMOVED***
    QueryEndBeforeConstraint.prototype._apply = function (query) ***REMOVED***
...
var QueryStartAtConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryStartAtConstraint, _super);
    function QueryStartAtConstraint(_value, _key) ***REMOVED***
        var _this = _super.call(this) || this;
        _this._value = _value;
        _this._key = _key;
        return _this;
  ***REMOVED***
    QueryStartAtConstraint.prototype._apply = function (query) ***REMOVED***
 ...
 */
function startAt(value, key) ***REMOVED***
    if (value === void 0) ***REMOVED*** value = null; }
    validateKey('startAt', 'key', key, true);
    return new QueryStartAtConstraint(value, key);
}
var QueryStartAfterConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryStartAfterConstraint, _super);
    function QueryStartAfterConstraint(_value, _key) ***REMOVED***
        var _this = _super.call(this) || this;
        _this._value = _value;
        _this._key = _key;
        return _this;
  ***REMOVED***
    QueryStartAfterConstraint.prototype._apply = function (query) ***REMOVED***
...
var QueryLimitToFirstConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryLimitToFirstConstraint, _super);
    function QueryLimitToFirstConstraint(_limit) ***REMOVED***
        var _this = _super.call(this) || this;
        _this._limit = _limit;
        return _this;
  ***REMOVED***
    QueryLimitToFirstConstraint.prototype._apply = function (query) ***REMOVED***
...
var QueryLimitToLastConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryLimitToLastConstraint, _super);
    function QueryLimitToLastConstraint(_limit) ***REMOVED***
        var _this = _super.call(this) || this;
        _this._limit = _limit;
        return _this;
  ***REMOVED***
    QueryLimitToLastConstraint.prototype._apply = function (query) ***REMOVED***
        if (query._queryParams.hasLimit()) ***REMOVED***
...
var QueryOrderByChildConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryOrderByChildConstraint, _super);
    function QueryOrderByChildConstraint(_path) ***REMOVED***
        var _this = _super.call(this) || this;
        _this._path = _path;
        return _this;
  ***REMOVED***
    QueryOrderByChildConstraint.prototype._apply = function (query) ***REMOVED***
...
var QueryOrderByKeyConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryOrderByKeyConstraint, _super);
    function QueryOrderByKeyConstraint() ***REMOVED***
        return _super !== null && _super.apply(this, arguments) || this;
  ***REMOVED***
    QueryOrderByKeyConstraint.prototype._apply = function (query) ***REMOVED***
...
var QueryOrderByPriorityConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryOrderByPriorityConstraint, _super);
    function QueryOrderByPriorityConstraint() ***REMOVED***
        return _super !== null && _super.apply(this, arguments) || this;
  ***REMOVED***
    QueryOrderByPriorityConstraint.prototype._apply = function (query) ***REMOVED***
...
var QueryOrderByValueConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryOrderByValueConstraint, _super);
    function QueryOrderByValueConstraint() ***REMOVED***
        return _super !== null && _super.apply(this, arguments) || this;
  ***REMOVED***
    QueryOrderByValueConstraint.prototype._apply = function (query) ***REMOVED***
...
var QueryEqualToValueConstraint = /** @class */ (function (_super) ***REMOVED***
    tslib.__extends(QueryEqualToValueConstraint, _super);
    function QueryEqualToValueConstraint(_value, _key) ***REMOVED***
        var _this = _super.call(this) || this;
        _this._value = _value;
        _this._key = _key;
        return _this;
  ***REMOVED***
    QueryEqualToValueConstraint.prototype._apply = function (query) ***REMOVED***
...
function query(query) ***REMOVED***
  ...
  ***REMOVED***
    ...
    try ***REMOVED***
        for (var queryConstraints_1 = tslib.__values(queryConstraints), queryConstraints_1_1 = queryConstraints_1.next(); !queryConstraints_1_1.done; queryConstraints_1_1 = queryConstraints_1.next()) ***REMOVED***
 ...
 */
var Database$1 = /** @class */ (function () ***REMOVED***
    /** @hideconstructor */
  ...
  ***REMOVED***
    Object.defineProperty(Database.prototype, "_repo", ***REMOVED***
        get: function () ***REMOVED***
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    Object.defineProperty(Database.prototype, "_root", ***REMOVED***
        get: function () ***REMOVED***
      ...
      ***REMOVED***,
        ...
        configurable: true
  ***REMOVED***);
    Database.prototype._delete = function () ***REMOVED***
  ...
  ***REMOVED***;
    Database.prototype._checkNotDeleted = function (apiName) ***REMOVED***
        if (this._rootInternal === null) ***REMOVED***
 ...
 */
function connectDatabaseEmulator(db, host, port, options) ***REMOVED***
 ...
 */
var TransactionResult$1 = /** @class */ (function () ***REMOVED***
    /** @hideconstructor */
...
transactionUpdate, options) ***REMOVED***
  ...
  ***REMOVED***
    ...
    var deferred = new util.Deferred();
    var promiseComplete = function (error, committed, node) ***REMOVED***
  ...
  ***REMOVED***;
    ...
    var unwatcher = onValue(ref, function () ***REMOVED*** });
 ...
 * @internal
 */
var hijackHash = function (newHash) ***REMOVED***
 ...
 * @internal
 */
var forceRestClient = function (forceRestClient) ***REMOVED***
 ...
 * limitations under the License.
 */
var validateBoolean = function (fnName, argumentName, bool, optional) ***REMOVED***
    if (optional && bool === undefined) ***REMOVED***
        return;
  ***REMOVED***
...
var validateEventType = function (fnName, eventType, optional) ***REMOVED***
    if (optional && eventType === undefined) ***REMOVED***
        return;
  ***REMOVED***
...
var OnDisconnect = /** @class */ (function () ***REMOVED***
    function OnDisconnect(_delegate) ***REMOVED***
  ...
  ***REMOVED***
    OnDisconnect.prototype.cancel = function (onComplete) ***REMOVED***
        ...
        if (onComplete) ***REMOVED***
            result.then(function () ***REMOVED*** return onComplete(null); }, function (error) ***REMOVED*** return onComplete(error); });
      ***REMOVED***
        return result;
  ***REMOVED***;
...
var DataSnapshot = /** @class */ (function () ***REMOVED***
    function DataSnapshot(_database, _delegate) ***REMOVED***
  ...
  ***REMOVED***;
    ...
    DataSnapshot.prototype.forEach = function (action) ***REMOVED***
        ...
        return this._delegate.forEach(function (expDataSnapshot) ***REMOVED***
...
var Query = /** @class */ (function () ***REMOVED***
    function Query(database, _delegate) ***REMOVED***
  ...
  ***REMOVED***
    Query.prototype.on = function (eventType, callback, cancelCallbackOrContext, context) ***REMOVED***
        ...
        var valueCallback = function (expSnapshot, previousChildName) ***REMOVED***
  ...
  ***REMOVED***;
    Query.prototype.off = function (eventType, callback, context) ***REMOVED***
        ...
        if (callback) ***REMOVED***
            var valueCallback = function () ***REMOVED*** };
            valueCallback.userCallback = callback;
  ...
  ***REMOVED***;
    ...
    Query.prototype.get = function () ***REMOVED***
        var _this = this;
        return get_1(this._delegate).then(function (expSnapshot) ***REMOVED***
  ...
  ***REMOVED***;
     ...
     */
    Query.prototype.once = function (eventType, callback, failureCallbackOrContext, context) ***REMOVED***
        var _this = this;
      ...
      ***REMOVED***;
        valueCallback.userCallback = callback;
        valueCallback.context = ret.context;
        var cancelCallback = function (error) ***REMOVED***
            if (ret.cancel) ***REMOVED***
      ...
      ***REMOVED***;
        switch (eventType) ***REMOVED***
            case 'value':
                onValue_1(this._delegate, valueCallback, cancelCallback, ***REMOVED***
                    onlyOnce: true
              ***REMOVED***);
                break;
            case 'child_added':
                onChildAdded_1(this._delegate, valueCallback, cancelCallback, ***REMOVED***
                    onlyOnce: true
              ***REMOVED***);
                break;
            case 'child_removed':
                onChildRemoved_1(this._delegate, valueCallback, cancelCallback, ***REMOVED***
                    onlyOnce: true
              ***REMOVED***);
                break;
            case 'child_changed':
                onChildChanged_1(this._delegate, valueCallback, cancelCallback, ***REMOVED***
                    onlyOnce: true
              ***REMOVED***);
                break;
            case 'child_moved':
                onChildMoved_1(this._delegate, valueCallback, cancelCallback, ***REMOVED***
                    onlyOnce: true
              ***REMOVED***);
                break;
            default:
  ...
  ***REMOVED***;
    Query.prototype.startAfter = function (value, name) ***REMOVED***
        if (value === void 0) ***REMOVED*** value = null; }
  ...
  ***REMOVED***;
    Query.prototype.endBefore = function (value, name) ***REMOVED***
        if (value === void 0) ***REMOVED*** value = null; }
  ...
  ***REMOVED***;
    ...
    Query.getCancelAndContextArgs_ = function (fnName, cancelOrContext, context) ***REMOVED***
        var ret = ***REMOVED*** cancel: undefined, context: undefined };
...
var Reference = /** @class */ (function (_super) ***REMOVED***
     ...
     */
    function Reference(database, _delegate) ***REMOVED***
  ...
  ***REMOVED***;
    Reference.prototype.set = function (newVal, onComplete) ***REMOVED***
      ...
      ***REMOVED***
        return result;
  ***REMOVED***;
    Reference.prototype.update = function (values, onComplete) ***REMOVED***
  ...
  ***REMOVED***;
    Reference.prototype.transaction = function (transactionUpdate, onComplete, applyLocally) ***REMOVED***
        ...
        var result = runTransaction_1(this._delegate, transactionUpdate, ***REMOVED***
            applyLocally: applyLocally
      ***REMOVED***).then(function (transactionResult) ***REMOVED***
            return new TransactionResult(transactionResult.committed, new DataSnapshot(_this.database, transactionResult.snapshot));
      ***REMOVED***);
        if (onComplete) ***REMOVED***
            result.then(function (transactionResult) ***REMOVED***
                return onComplete(null, transactionResult.committed, transactionResult.snapshot);
          ***REMOVED***, function (error) ***REMOVED*** return onComplete(error, false, null); });
      ***REMOVED***
        return result;
  ***REMOVED***;
  ...
  ***REMOVED***;
    Reference.prototype.push = function (value, onComplete) ***REMOVED***
        ...
        var promise = expPromise.then(function (expRef) ***REMOVED*** return new Reference(_this.database, expRef); });
        if (onComplete) ***REMOVED***
            promise.then(function () ***REMOVED*** return onComplete(null); }, function (error) ***REMOVED*** return onComplete(error); });
      ***REMOVED***
...
var Database = /** @class */ (function () ***REMOVED***
     ...
     */
    function Database(_delegate, app) ***REMOVED***
        var _this = this;
        this._delegate = _delegate;
        this.app = app;
        this.INTERNAL = ***REMOVED***
            delete: function () ***REMOVED*** return _this._delegate._delete(); },
  ...
  ***REMOVED***
     ...
     * @param options.mockUserToken - the mock auth token to use for unit testing Security Rules
     */
    Database.prototype.useEmulator = function (host, port, options) ***REMOVED***
        if (options === void 0) ***REMOVED*** options = ***REMOVED***}; }
        connectDatabaseEmulator_1(this._delegate, host, port, options);
  ***REMOVED***;
    Database.prototype.ref = function (path) ***REMOVED***
        require$$1$3.validateArgCount('database.ref', 0, 1, arguments.length);
        if (path instanceof Reference) ***REMOVED***
  ...
  ***REMOVED***;
    Database.ServerValue = ***REMOVED***
        ...
        increment: function (delta) ***REMOVED*** return increment_1(delta); }
  ***REMOVED***;
    return Database;
}());
 ...
 */
function initStandalone$1(_a) ***REMOVED***
    ...
    authProvider.setComponent(new component.Component('auth-internal', function () ***REMOVED*** return customAuthImpl; }, "PRIVATE" /* ComponentType.PRIVATE */));
    return ***REMOVED***
...
var INTERNAL = /*#__PURE__*/Object.freeze(***REMOVED***
  __proto__: null,
 ...
 */
function initStandalone(app, url, version, nodeAdmin) ***REMOVED***
    if (nodeAdmin === void 0) ***REMOVED*** nodeAdmin = true; }
    require$$1$3.CONSTANTS.NODE_ADMIN = nodeAdmin;
    return initStandalone$1(***REMOVED***
        app: app,
        url: url,
        version: version,
