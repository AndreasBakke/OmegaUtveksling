import firebase from '@firebase/app-compat';
import ***REMOVED*** Provider, ComponentContainer, Component } from '@firebase/component';
import ***REMOVED*** _validatePathString, onChildMoved, onChildChanged, onChildRemoved, onChildAdded, onValue, off, get, query, limitToFirst, limitToLast, orderByChild, orderByKey, orderByPriority, orderByValue, startAt, startAfter, endAt, endBefore, equalTo, _ReferenceImpl, _QueryImpl, _QueryParams, child, set, _validateWritablePath, update, setWithPriority, remove, runTransaction, setPriority, push, OnDisconnect as OnDisconnect$1, forceWebSockets, forceLongPolling, connectDatabaseEmulator, refFromURL, ref, goOffline, goOnline, serverTimestamp, increment, _setSDKVersion, _repoManagerDatabaseFromApp, enableLogging } from '@firebase/database';
import ***REMOVED*** errorPrefix, validateArgCount, validateCallback, validateContextObject, Deferred } from '@firebase/util';
import ***REMOVED*** Logger } from '@firebase/logger';

const name = "@firebase/database-compat";
const version = "0.3.1";

/**
 * @license
 * Copyright 2021 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
const logClient = new Logger('@firebase/database-compat');
const warn = function (msg) ***REMOVED***
    const message = 'FIREBASE WARNING: ' + msg;
    logClient.warn(message);
};

/**
 * @license
 * Copyright 2021 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
const validateBoolean = function (fnName, argumentName, bool, optional) ***REMOVED***
    if (optional && bool === undefined) ***REMOVED***
        return;
  ***REMOVED***
    if (typeof bool !== 'boolean') ***REMOVED***
        throw new Error(errorPrefix(fnName, argumentName) + 'must be a boolean.');
  ***REMOVED***
};
const validateEventType = function (fnName, eventType, optional) ***REMOVED***
    if (optional && eventType === undefined) ***REMOVED***
        return;
  ***REMOVED***
    switch (eventType) ***REMOVED***
        case 'value':
        case 'child_added':
        case 'child_removed':
        case 'child_changed':
        case 'child_moved':
            break;
        default:
            throw new Error(errorPrefix(fnName, 'eventType') +
                'must be a valid event type = "value", "child_added", "child_removed", ' +
                '"child_changed", or "child_moved".');
  ***REMOVED***
};

/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
class OnDisconnect ***REMOVED***
    constructor(_delegate) ***REMOVED***
        this._delegate = _delegate;
  ***REMOVED***
    cancel(onComplete) ***REMOVED***
        validateArgCount('OnDisconnect.cancel', 0, 1, arguments.length);
        validateCallback('OnDisconnect.cancel', 'onComplete', onComplete, true);
        const result = this._delegate.cancel();
        if (onComplete) ***REMOVED***
            result.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        return result;
  ***REMOVED***
    remove(onComplete) ***REMOVED***
        validateArgCount('OnDisconnect.remove', 0, 1, arguments.length);
        validateCallback('OnDisconnect.remove', 'onComplete', onComplete, true);
        const result = this._delegate.remove();
        if (onComplete) ***REMOVED***
            result.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        return result;
  ***REMOVED***
    set(value, onComplete) ***REMOVED***
        validateArgCount('OnDisconnect.set', 1, 2, arguments.length);
        validateCallback('OnDisconnect.set', 'onComplete', onComplete, true);
        const result = this._delegate.set(value);
        if (onComplete) ***REMOVED***
            result.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        return result;
  ***REMOVED***
    setWithPriority(value, priority, onComplete) ***REMOVED***
        validateArgCount('OnDisconnect.setWithPriority', 2, 3, arguments.length);
        validateCallback('OnDisconnect.setWithPriority', 'onComplete', onComplete, true);
        const result = this._delegate.setWithPriority(value, priority);
        if (onComplete) ***REMOVED***
            result.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        return result;
  ***REMOVED***
    update(objectToMerge, onComplete) ***REMOVED***
        validateArgCount('OnDisconnect.update', 1, 2, arguments.length);
        if (Array.isArray(objectToMerge)) ***REMOVED***
            const newObjectToMerge = ***REMOVED***};
            for (let i = 0; i < objectToMerge.length; ++i) ***REMOVED***
                newObjectToMerge['' + i] = objectToMerge[i];
          ***REMOVED***
            objectToMerge = newObjectToMerge;
            warn('Passing an Array to firebase.database.onDisconnect().update() is deprecated. Use set() if you want to overwrite the ' +
                'existing data, or an Object with integer keys if you really do want to only update some of the children.');
      ***REMOVED***
        validateCallback('OnDisconnect.update', 'onComplete', onComplete, true);
        const result = this._delegate.update(objectToMerge);
        if (onComplete) ***REMOVED***
            result.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        return result;
  ***REMOVED***
}

/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
class TransactionResult ***REMOVED***
    /**
     * A type for the resolve value of Firebase.transaction.
     */
    constructor(committed, snapshot) ***REMOVED***
        this.committed = committed;
        this.snapshot = snapshot;
  ***REMOVED***
    // Do not create public documentation. This is intended to make JSON serialization work but is otherwise unnecessary
    // for end-users
    toJSON() ***REMOVED***
        validateArgCount('TransactionResult.toJSON', 0, 1, arguments.length);
        return ***REMOVED*** committed: this.committed, snapshot: this.snapshot.toJSON() };
  ***REMOVED***
}

/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
/**
 * Class representing a firebase data snapshot.  It wraps a SnapshotNode and
 * surfaces the public methods (val, forEach, etc.) we want to expose.
 */
class DataSnapshot ***REMOVED***
    constructor(_database, _delegate) ***REMOVED***
        this._database = _database;
        this._delegate = _delegate;
  ***REMOVED***
    /**
     * Retrieves the snapshot contents as JSON.  Returns null if the snapshot is
     * empty.
     *
     * @returns JSON representation of the DataSnapshot contents, or null if empty.
     */
    val() ***REMOVED***
        validateArgCount('DataSnapshot.val', 0, 0, arguments.length);
        return this._delegate.val();
  ***REMOVED***
    /**
     * Returns the snapshot contents as JSON, including priorities of node.  Suitable for exporting
     * the entire node contents.
     * @returns JSON representation of the DataSnapshot contents, or null if empty.
     */
    exportVal() ***REMOVED***
        validateArgCount('DataSnapshot.exportVal', 0, 0, arguments.length);
        return this._delegate.exportVal();
  ***REMOVED***
    // Do not create public documentation. This is intended to make JSON serialization work but is otherwise unnecessary
    // for end-users
    toJSON() ***REMOVED***
        // Optional spacer argument is unnecessary because we're depending on recursion rather than stringifying the content
        validateArgCount('DataSnapshot.toJSON', 0, 1, arguments.length);
        return this._delegate.toJSON();
  ***REMOVED***
    /**
     * Returns whether the snapshot contains a non-null value.
     *
     * @returns Whether the snapshot contains a non-null value, or is empty.
     */
    exists() ***REMOVED***
        validateArgCount('DataSnapshot.exists', 0, 0, arguments.length);
        return this._delegate.exists();
  ***REMOVED***
    /**
     * Returns a DataSnapshot of the specified child node's contents.
     *
     * @param path - Path to a child.
     * @returns DataSnapshot for child node.
     */
    child(path) ***REMOVED***
        validateArgCount('DataSnapshot.child', 0, 1, arguments.length);
        // Ensure the childPath is a string (can be a number)
        path = String(path);
        _validatePathString('DataSnapshot.child', 'path', path, false);
        return new DataSnapshot(this._database, this._delegate.child(path));
  ***REMOVED***
    /**
     * Returns whether the snapshot contains a child at the specified path.
     *
     * @param path - Path to a child.
     * @returns Whether the child exists.
     */
    hasChild(path) ***REMOVED***
        validateArgCount('DataSnapshot.hasChild', 1, 1, arguments.length);
        _validatePathString('DataSnapshot.hasChild', 'path', path, false);
        return this._delegate.hasChild(path);
  ***REMOVED***
    /**
     * Returns the priority of the object, or null if no priority was set.
     *
     * @returns The priority.
     */
    getPriority() ***REMOVED***
        validateArgCount('DataSnapshot.getPriority', 0, 0, arguments.length);
        return this._delegate.priority;
  ***REMOVED***
    /**
     * Iterates through child nodes and calls the specified action for each one.
     *
     * @param action - Callback function to be called
     * for each child.
     * @returns True if forEach was canceled by action returning true for
     * one of the child nodes.
     */
    forEach(action) ***REMOVED***
        validateArgCount('DataSnapshot.forEach', 1, 1, arguments.length);
        validateCallback('DataSnapshot.forEach', 'action', action, false);
        return this._delegate.forEach(expDataSnapshot => action(new DataSnapshot(this._database, expDataSnapshot)));
  ***REMOVED***
    /**
     * Returns whether this DataSnapshot has children.
     * @returns True if the DataSnapshot contains 1 or more child nodes.
     */
    hasChildren() ***REMOVED***
        validateArgCount('DataSnapshot.hasChildren', 0, 0, arguments.length);
        return this._delegate.hasChildren();
  ***REMOVED***
    get key() ***REMOVED***
        return this._delegate.key;
  ***REMOVED***
    /**
     * Returns the number of children for this DataSnapshot.
     * @returns The number of children that this DataSnapshot contains.
     */
    numChildren() ***REMOVED***
        validateArgCount('DataSnapshot.numChildren', 0, 0, arguments.length);
        return this._delegate.size;
  ***REMOVED***
    /**
     * @returns The Firebase reference for the location this snapshot's data came
     * from.
     */
    getRef() ***REMOVED***
        validateArgCount('DataSnapshot.ref', 0, 0, arguments.length);
        return new Reference(this._database, this._delegate.ref);
  ***REMOVED***
    get ref() ***REMOVED***
        return this.getRef();
  ***REMOVED***
}
/**
 * A Query represents a filter to be applied to a firebase location.  This object purely represents the
 * query expression (and exposes our public API to build the query).  The actual query logic is in ViewBase.js.
 *
 * Since every Firebase reference is a query, Firebase inherits from this object.
 */
class Query ***REMOVED***
    constructor(database, _delegate) ***REMOVED***
        this.database = database;
        this._delegate = _delegate;
  ***REMOVED***
    on(eventType, callback, cancelCallbackOrContext, context) ***REMOVED***
        var _a;
        validateArgCount('Query.on', 2, 4, arguments.length);
        validateCallback('Query.on', 'callback', callback, false);
        const ret = Query.getCancelAndContextArgs_('Query.on', cancelCallbackOrContext, context);
        const valueCallback = (expSnapshot, previousChildName) => ***REMOVED***
            callback.call(ret.context, new DataSnapshot(this.database, expSnapshot), previousChildName);
      ***REMOVED***;
        valueCallback.userCallback = callback;
        valueCallback.context = ret.context;
        const cancelCallback = (_a = ret.cancel) === null || _a === void 0 ? void 0 : _a.bind(ret.context);
        switch (eventType) ***REMOVED***
            case 'value':
                onValue(this._delegate, valueCallback, cancelCallback);
                return callback;
            case 'child_added':
                onChildAdded(this._delegate, valueCallback, cancelCallback);
                return callback;
            case 'child_removed':
                onChildRemoved(this._delegate, valueCallback, cancelCallback);
                return callback;
            case 'child_changed':
                onChildChanged(this._delegate, valueCallback, cancelCallback);
                return callback;
            case 'child_moved':
                onChildMoved(this._delegate, valueCallback, cancelCallback);
                return callback;
            default:
                throw new Error(errorPrefix('Query.on', 'eventType') +
                    'must be a valid event type = "value", "child_added", "child_removed", ' +
                    '"child_changed", or "child_moved".');
      ***REMOVED***
  ***REMOVED***
    off(eventType, callback, context) ***REMOVED***
        validateArgCount('Query.off', 0, 3, arguments.length);
        validateEventType('Query.off', eventType, true);
        validateCallback('Query.off', 'callback', callback, true);
        validateContextObject('Query.off', 'context', context, true);
        if (callback) ***REMOVED***
            const valueCallback = () => ***REMOVED*** };
            valueCallback.userCallback = callback;
            valueCallback.context = context;
            off(this._delegate, eventType, valueCallback);
      ***REMOVED***
        else ***REMOVED***
            off(this._delegate, eventType);
      ***REMOVED***
  ***REMOVED***
    /**
     * Get the server-value for this query, or return a cached value if not connected.
     */
    get() ***REMOVED***
        return get(this._delegate).then(expSnapshot => ***REMOVED***
            return new DataSnapshot(this.database, expSnapshot);
      ***REMOVED***);
  ***REMOVED***
    /**
     * Attaches a listener, waits for the first event, and then removes the listener
     */
    once(eventType, callback, failureCallbackOrContext, context) ***REMOVED***
        validateArgCount('Query.once', 1, 4, arguments.length);
        validateCallback('Query.once', 'callback', callback, true);
        const ret = Query.getCancelAndContextArgs_('Query.once', failureCallbackOrContext, context);
        const deferred = new Deferred();
        const valueCallback = (expSnapshot, previousChildName) => ***REMOVED***
            const result = new DataSnapshot(this.database, expSnapshot);
            if (callback) ***REMOVED***
                callback.call(ret.context, result, previousChildName);
          ***REMOVED***
            deferred.resolve(result);
      ***REMOVED***;
        valueCallback.userCallback = callback;
        valueCallback.context = ret.context;
        const cancelCallback = (error) => ***REMOVED***
            if (ret.cancel) ***REMOVED***
                ret.cancel.call(ret.context, error);
          ***REMOVED***
            deferred.reject(error);
      ***REMOVED***;
        switch (eventType) ***REMOVED***
            case 'value':
                onValue(this._delegate, valueCallback, cancelCallback, ***REMOVED***
                    onlyOnce: true
              ***REMOVED***);
                break;
            case 'child_added':
                onChildAdded(this._delegate, valueCallback, cancelCallback, ***REMOVED***
                    onlyOnce: true
              ***REMOVED***);
                break;
            case 'child_removed':
                onChildRemoved(this._delegate, valueCallback, cancelCallback, ***REMOVED***
                    onlyOnce: true
              ***REMOVED***);
                break;
            case 'child_changed':
                onChildChanged(this._delegate, valueCallback, cancelCallback, ***REMOVED***
                    onlyOnce: true
              ***REMOVED***);
                break;
            case 'child_moved':
                onChildMoved(this._delegate, valueCallback, cancelCallback, ***REMOVED***
                    onlyOnce: true
              ***REMOVED***);
                break;
            default:
                throw new Error(errorPrefix('Query.once', 'eventType') +
                    'must be a valid event type = "value", "child_added", "child_removed", ' +
                    '"child_changed", or "child_moved".');
      ***REMOVED***
        return deferred.promise;
  ***REMOVED***
    /**
     * Set a limit and anchor it to the start of the window.
     */
    limitToFirst(limit) ***REMOVED***
        validateArgCount('Query.limitToFirst', 1, 1, arguments.length);
        return new Query(this.database, query(this._delegate, limitToFirst(limit)));
  ***REMOVED***
    /**
     * Set a limit and anchor it to the end of the window.
     */
    limitToLast(limit) ***REMOVED***
        validateArgCount('Query.limitToLast', 1, 1, arguments.length);
        return new Query(this.database, query(this._delegate, limitToLast(limit)));
  ***REMOVED***
    /**
     * Given a child path, return a new query ordered by the specified grandchild path.
     */
    orderByChild(path) ***REMOVED***
        validateArgCount('Query.orderByChild', 1, 1, arguments.length);
        return new Query(this.database, query(this._delegate, orderByChild(path)));
  ***REMOVED***
    /**
     * Return a new query ordered by the KeyIndex
     */
    orderByKey() ***REMOVED***
        validateArgCount('Query.orderByKey', 0, 0, arguments.length);
        return new Query(this.database, query(this._delegate, orderByKey()));
  ***REMOVED***
    /**
     * Return a new query ordered by the PriorityIndex
     */
    orderByPriority() ***REMOVED***
        validateArgCount('Query.orderByPriority', 0, 0, arguments.length);
        return new Query(this.database, query(this._delegate, orderByPriority()));
  ***REMOVED***
    /**
     * Return a new query ordered by the ValueIndex
     */
    orderByValue() ***REMOVED***
        validateArgCount('Query.orderByValue', 0, 0, arguments.length);
        return new Query(this.database, query(this._delegate, orderByValue()));
  ***REMOVED***
    startAt(value = null, name) ***REMOVED***
        validateArgCount('Query.startAt', 0, 2, arguments.length);
        return new Query(this.database, query(this._delegate, startAt(value, name)));
  ***REMOVED***
    startAfter(value = null, name) ***REMOVED***
        validateArgCount('Query.startAfter', 0, 2, arguments.length);
        return new Query(this.database, query(this._delegate, startAfter(value, name)));
  ***REMOVED***
    endAt(value = null, name) ***REMOVED***
        validateArgCount('Query.endAt', 0, 2, arguments.length);
        return new Query(this.database, query(this._delegate, endAt(value, name)));
  ***REMOVED***
    endBefore(value = null, name) ***REMOVED***
        validateArgCount('Query.endBefore', 0, 2, arguments.length);
        return new Query(this.database, query(this._delegate, endBefore(value, name)));
  ***REMOVED***
    /**
     * Load the selection of children with exactly the specified value, and, optionally,
     * the specified name.
     */
    equalTo(value, name) ***REMOVED***
        validateArgCount('Query.equalTo', 1, 2, arguments.length);
        return new Query(this.database, query(this._delegate, equalTo(value, name)));
  ***REMOVED***
    /**
     * @returns URL for this location.
     */
    toString() ***REMOVED***
        validateArgCount('Query.toString', 0, 0, arguments.length);
        return this._delegate.toString();
  ***REMOVED***
    // Do not create public documentation. This is intended to make JSON serialization work but is otherwise unnecessary
    // for end-users.
    toJSON() ***REMOVED***
        // An optional spacer argument is unnecessary for a string.
        validateArgCount('Query.toJSON', 0, 1, arguments.length);
        return this._delegate.toJSON();
  ***REMOVED***
    /**
     * Return true if this query and the provided query are equivalent; otherwise, return false.
     */
    isEqual(other) ***REMOVED***
        validateArgCount('Query.isEqual', 1, 1, arguments.length);
        if (!(other instanceof Query)) ***REMOVED***
            const error = 'Query.isEqual failed: First argument must be an instance of firebase.database.Query.';
            throw new Error(error);
      ***REMOVED***
        return this._delegate.isEqual(other._delegate);
  ***REMOVED***
    /**
     * Helper used by .on and .once to extract the context and or cancel arguments.
     * @param fnName - The function name (on or once)
     *
     */
    static getCancelAndContextArgs_(fnName, cancelOrContext, context) ***REMOVED***
        const ret = ***REMOVED*** cancel: undefined, context: undefined };
        if (cancelOrContext && context) ***REMOVED***
            ret.cancel = cancelOrContext;
            validateCallback(fnName, 'cancel', ret.cancel, true);
            ret.context = context;
            validateContextObject(fnName, 'context', ret.context, true);
      ***REMOVED***
        else if (cancelOrContext) ***REMOVED***
            // we have either a cancel callback or a context.
            if (typeof cancelOrContext === 'object' && cancelOrContext !== null) ***REMOVED***
                // it's a context!
                ret.context = cancelOrContext;
          ***REMOVED***
            else if (typeof cancelOrContext === 'function') ***REMOVED***
                ret.cancel = cancelOrContext;
          ***REMOVED***
            else ***REMOVED***
                throw new Error(errorPrefix(fnName, 'cancelOrContext') +
                    ' must either be a cancel callback or a context object.');
          ***REMOVED***
      ***REMOVED***
        return ret;
  ***REMOVED***
    get ref() ***REMOVED***
        return new Reference(this.database, new _ReferenceImpl(this._delegate._repo, this._delegate._path));
  ***REMOVED***
}
class Reference extends Query ***REMOVED***
    /**
     * Call options:
     *   new Reference(Repo, Path) or
     *   new Reference(url: string, string|RepoManager)
     *
     * Externally - this is the firebase.database.Reference type.
     */
    constructor(database, _delegate) ***REMOVED***
        super(database, new _QueryImpl(_delegate._repo, _delegate._path, new _QueryParams(), false));
        this.database = database;
        this._delegate = _delegate;
  ***REMOVED***
    /** @returns ***REMOVED***?string} */
    getKey() ***REMOVED***
        validateArgCount('Reference.key', 0, 0, arguments.length);
        return this._delegate.key;
  ***REMOVED***
    child(pathString) ***REMOVED***
        validateArgCount('Reference.child', 1, 1, arguments.length);
        if (typeof pathString === 'number') ***REMOVED***
            pathString = String(pathString);
      ***REMOVED***
        return new Reference(this.database, child(this._delegate, pathString));
  ***REMOVED***
    /** @returns ***REMOVED***?Reference} */
    getParent() ***REMOVED***
        validateArgCount('Reference.parent', 0, 0, arguments.length);
        const parent = this._delegate.parent;
        return parent ? new Reference(this.database, parent) : null;
  ***REMOVED***
    /** @returns ***REMOVED***!Reference} */
    getRoot() ***REMOVED***
        validateArgCount('Reference.root', 0, 0, arguments.length);
        return new Reference(this.database, this._delegate.root);
  ***REMOVED***
    set(newVal, onComplete) ***REMOVED***
        validateArgCount('Reference.set', 1, 2, arguments.length);
        validateCallback('Reference.set', 'onComplete', onComplete, true);
        const result = set(this._delegate, newVal);
        if (onComplete) ***REMOVED***
            result.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        return result;
  ***REMOVED***
    update(values, onComplete) ***REMOVED***
        validateArgCount('Reference.update', 1, 2, arguments.length);
        if (Array.isArray(values)) ***REMOVED***
            const newObjectToMerge = ***REMOVED***};
            for (let i = 0; i < values.length; ++i) ***REMOVED***
                newObjectToMerge['' + i] = values[i];
          ***REMOVED***
            values = newObjectToMerge;
            warn('Passing an Array to Firebase.update() is deprecated. ' +
                'Use set() if you want to overwrite the existing data, or ' +
                'an Object with integer keys if you really do want to ' +
                'only update some of the children.');
      ***REMOVED***
        _validateWritablePath('Reference.update', this._delegate._path);
        validateCallback('Reference.update', 'onComplete', onComplete, true);
        const result = update(this._delegate, values);
        if (onComplete) ***REMOVED***
            result.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        return result;
  ***REMOVED***
    setWithPriority(newVal, newPriority, onComplete) ***REMOVED***
        validateArgCount('Reference.setWithPriority', 2, 3, arguments.length);
        validateCallback('Reference.setWithPriority', 'onComplete', onComplete, true);
        const result = setWithPriority(this._delegate, newVal, newPriority);
        if (onComplete) ***REMOVED***
            result.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        return result;
  ***REMOVED***
    remove(onComplete) ***REMOVED***
        validateArgCount('Reference.remove', 0, 1, arguments.length);
        validateCallback('Reference.remove', 'onComplete', onComplete, true);
        const result = remove(this._delegate);
        if (onComplete) ***REMOVED***
            result.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        return result;
  ***REMOVED***
    transaction(transactionUpdate, onComplete, applyLocally) ***REMOVED***
        validateArgCount('Reference.transaction', 1, 3, arguments.length);
        validateCallback('Reference.transaction', 'transactionUpdate', transactionUpdate, false);
        validateCallback('Reference.transaction', 'onComplete', onComplete, true);
        validateBoolean('Reference.transaction', 'applyLocally', applyLocally, true);
        const result = runTransaction(this._delegate, transactionUpdate, ***REMOVED***
            applyLocally
      ***REMOVED***).then(transactionResult => new TransactionResult(transactionResult.committed, new DataSnapshot(this.database, transactionResult.snapshot)));
        if (onComplete) ***REMOVED***
            result.then(transactionResult => onComplete(null, transactionResult.committed, transactionResult.snapshot), error => onComplete(error, false, null));
      ***REMOVED***
        return result;
  ***REMOVED***
    setPriority(priority, onComplete) ***REMOVED***
        validateArgCount('Reference.setPriority', 1, 2, arguments.length);
        validateCallback('Reference.setPriority', 'onComplete', onComplete, true);
        const result = setPriority(this._delegate, priority);
        if (onComplete) ***REMOVED***
            result.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        return result;
  ***REMOVED***
    push(value, onComplete) ***REMOVED***
        validateArgCount('Reference.push', 0, 2, arguments.length);
        validateCallback('Reference.push', 'onComplete', onComplete, true);
        const expPromise = push(this._delegate, value);
        const promise = expPromise.then(expRef => new Reference(this.database, expRef));
        if (onComplete) ***REMOVED***
            promise.then(() => onComplete(null), error => onComplete(error));
      ***REMOVED***
        const result = new Reference(this.database, expPromise);
        result.then = promise.then.bind(promise);
        result.catch = promise.catch.bind(promise, undefined);
        return result;
  ***REMOVED***
    onDisconnect() ***REMOVED***
        _validateWritablePath('Reference.onDisconnect', this._delegate._path);
        return new OnDisconnect(new OnDisconnect$1(this._delegate._repo, this._delegate._path));
  ***REMOVED***
    get key() ***REMOVED***
        return this.getKey();
  ***REMOVED***
    get parent() ***REMOVED***
        return this.getParent();
  ***REMOVED***
    get root() ***REMOVED***
        return this.getRoot();
  ***REMOVED***
}

/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
/**
 * Class representing a firebase database.
 */
class Database ***REMOVED***
    /**
     * The constructor should not be called by users of our public API.
     */
    constructor(_delegate, app) ***REMOVED***
        this._delegate = _delegate;
        this.app = app;
        this.INTERNAL = ***REMOVED***
            delete: () => this._delegate._delete(),
            forceWebSockets,
            forceLongPolling
      ***REMOVED***;
  ***REMOVED***
    /**
     * Modify this instance to communicate with the Realtime Database emulator.
     *
     * <p>Note: This method must be called before performing any other operation.
     *
     * @param host - the emulator host (ex: localhost)
     * @param port - the emulator port (ex: 8080)
     * @param options.mockUserToken - the mock auth token to use for unit testing Security Rules
     */
    useEmulator(host, port, options = ***REMOVED***}) ***REMOVED***
        connectDatabaseEmulator(this._delegate, host, port, options);
  ***REMOVED***
    ref(path) ***REMOVED***
        validateArgCount('database.ref', 0, 1, arguments.length);
        if (path instanceof Reference) ***REMOVED***
            const childRef = refFromURL(this._delegate, path.toString());
            return new Reference(this, childRef);
      ***REMOVED***
        else ***REMOVED***
            const childRef = ref(this._delegate, path);
            return new Reference(this, childRef);
      ***REMOVED***
  ***REMOVED***
    /**
     * Returns a reference to the root or the path specified in url.
     * We throw a exception if the url is not in the same domain as the
     * current repo.
     * @returns Firebase reference.
     */
    refFromURL(url) ***REMOVED***
        const apiName = 'database.refFromURL';
        validateArgCount(apiName, 1, 1, arguments.length);
        const childRef = refFromURL(this._delegate, url);
        return new Reference(this, childRef);
  ***REMOVED***
    // Make individual repo go offline.
    goOffline() ***REMOVED***
        validateArgCount('database.goOffline', 0, 0, arguments.length);
        return goOffline(this._delegate);
  ***REMOVED***
    goOnline() ***REMOVED***
        validateArgCount('database.goOnline', 0, 0, arguments.length);
        return goOnline(this._delegate);
  ***REMOVED***
}
Database.ServerValue = ***REMOVED***
    TIMESTAMP: serverTimestamp(),
    increment: (delta) => increment(delta)
};

/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
/**
 * Used by console to create a database based on the app,
 * passed database URL and a custom auth implementation.
 *
 * @param app - A valid FirebaseApp-like object
 * @param url - A valid Firebase databaseURL
 * @param version - custom version e.g. firebase-admin version
 * @param customAuthImpl - custom auth implementation
 */
function initStandalone(***REMOVED*** app, url, version, customAuthImpl, namespace, nodeAdmin = false }) ***REMOVED***
    _setSDKVersion(version);
    /**
     * ComponentContainer('database-standalone') is just a placeholder that doesn't perform
     * any actual function.
     */
    const authProvider = new Provider('auth-internal', new ComponentContainer('database-standalone'));
    authProvider.setComponent(new Component('auth-internal', () => customAuthImpl, "PRIVATE" /* ComponentType.PRIVATE */));
    return ***REMOVED***
        instance: new Database(_repoManagerDatabaseFromApp(app, authProvider, 
        /* appCheckProvider= */ undefined, url, nodeAdmin), app),
        namespace
  ***REMOVED***;
}

var INTERNAL = /*#__PURE__*/Object.freeze(***REMOVED***
  __proto__: null,
  initStandalone: initStandalone
});

/**
 * @license
 * Copyright 2021 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
const ServerValue = Database.ServerValue;
function registerDatabase(instance) ***REMOVED***
    // Register the Database Service with the 'firebase' namespace.
    instance.INTERNAL.registerComponent(new Component('database-compat', (container, ***REMOVED*** instanceIdentifier: url }) => ***REMOVED***
        /* Dependencies */
        // getImmediate for FirebaseApp will always succeed
        const app = container.getProvider('app-compat').getImmediate();
        const databaseExp = container
            .getProvider('database')
            .getImmediate(***REMOVED*** identifier: url });
        return new Database(databaseExp, app);
  ***REMOVED***, "PUBLIC" /* ComponentType.PUBLIC */)
        .setServiceProps(
    // firebase.database namespace properties
    ***REMOVED***
        Reference,
        Query,
        Database,
        DataSnapshot,
        enableLogging,
        INTERNAL,
        ServerValue
  ***REMOVED***)
        .setMultipleInstances(true));
    instance.registerVersion(name, version);
}
registerDatabase(firebase);

export ***REMOVED*** registerDatabase };
//# sourceMappingURL=index.esm2017.js.map
