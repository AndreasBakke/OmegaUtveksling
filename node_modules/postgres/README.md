...
const sql = postgres(***REMOVED*** /* options */ }) // will use psql environment variables
...
async function getUsersOver(age) ***REMOVED***
  const users = await sql`
    select
      name,
      age
    from users
    where age > $***REMOVED*** age }
  `
  // users = Result [***REMOVED*** name: "Walter", age: 80 }, ***REMOVED*** name: 'Murray', age: 68 }, ...]
  return users
}


async function insertUser(***REMOVED*** name, age }) ***REMOVED***
  const users = await sql`
    insert into users
      (name, age)
    values
    ...
    returning name, age
  `
  // users = Result [***REMOVED*** name: "Murray", age: 68 }]
  return users
}
```
...
```js
const sql = postgres('postgres://username:password@host:port/database', ***REMOVED***
...
// xs = [***REMOVED*** user_id: 1, name: 'Murray', age: 68 }]
```
...
const users = await sql`
  ...
  from users
  where
    name like $***REMOVED*** name + '%' }
    and age > $***REMOVED*** age }
`
// users = [***REMOVED*** name: 'Murray', age: 68 }]
```

> Be careful with quotation marks here. Because Postgres infers column types, you do not need to wrap your interpolated parameters in quotes like `'$***REMOVED***name}'`. This will cause an error because the tagged template replaces `$***REMOVED***name}` with `$1` in the query string, leaving Postgres to do the interpolation. If you wrap that in a string, Postgres will see `'$1'` and interpret it as a string as opposed to a parameter.
...
sql`insert into users $***REMOVED*** sql(users, 'name', 'age') }`

// Is translated to:
insert into users ("name", "age") values ($1, $2), ($3, $4)

// Here you can also omit column names which will use object keys as columns
sql`insert into users $***REMOVED*** sql(users) }`
...
sql`
  ...
  from (values $***REMOVED***sql(users)}) as update_data (id, name, age)
...
const users = await sql`
  select
    *
  from users
  where age in $***REMOVED*** sql([68, 75, 23]) }
`
```

or
```js
const [***REMOVED*** a, b, c }] => await sql`
  select
    *
  from (values $***REMOVED*** sql(['a', 'b', 'c']) }) as x(a, b, c)
`
```
...
### Partial queries
```js
const olderThan = x => sql`and age > $***REMOVED*** x }`
...
sql`
   ...
   *
  from users
  where name is not null $***REMOVED***
    filterAge
      ? olderThan(50)
      : sql``
***REMOVED***
...
sql`
  update users set updated_at = $***REMOVED*** date || sql`now()` }
`
...
sql`
  select $***REMOVED*** sql(column) } from $***REMOVED*** sql(table) }
`
...
| -------------              | -------------                 | -------------                                             |
| `$***REMOVED*** sql`` }`               | for keywords or sql fragments | ``sql`SELECT * FROM users $***REMOVED***sql`order by age desc` }` ``  |
| `$***REMOVED*** sql(string) }`         | for identifiers               | ``sql`SELECT * FROM $***REMOVED***sql('table_name')` ``               |
| `$***REMOVED*** sql([] or ***REMOVED***}, ...) }`  | for helpers                   | ``sql`INSERT INTO users $***REMOVED***sql(***REMOVED*** name: 'Peter'})}` ``      |
| `$***REMOVED*** 'somevalue' }`         | for values                    | ``sql`SELECT * FROM users WHERE age = $***REMOVED***42}` ``           |
...
`.cursor(async([row]) => ***REMOVED***
  // row = ***REMOVED*** x: 1 }
  await http.request('https://example.com/wat', ***REMOVED*** row })
})
```
...
for await (const [row] of cursor) ***REMOVED***
  // row = ***REMOVED*** x: 1 }
  await http.request('https://example.com/wat', ***REMOVED*** row })
}
```
...
`.cursor(10, async rows => ***REMOVED***
  // rows = [***REMOVED*** x: 1 }, ***REMOVED*** x: 2 }, ... ]
  await Promise.all(rows.map(row =>
    http.request('https://example.com/wat', ***REMOVED*** row })
  ))
})
```
...
`.forEach(row => ***REMOVED***
  // row = ***REMOVED*** created_at: '2019-11-22T14:22:00Z', name: 'connected' }
})
...
```js
import ***REMOVED*** pipeline } from 'node:stream/promises'
...
```js
import ***REMOVED*** pipeline } from 'node:stream/promises'
import ***REMOVED*** createWriteStream } from 'node:fs'
...
`.readable()
for await (const chunk of readableStream) ***REMOVED***
...
```js
const [user, account] = await sql.begin(async sql => ***REMOVED***
...
```js
sql.begin('read write', async sql => ***REMOVED***
  const [user] = await sql`
    insert into users (
      name
    ) values (
...
These built in transformations will only convert to/from snake_case. For example, using `***REMOVED*** transform: postgres.toCamel }` will convert the column names to camelCase only if the column names are in snake_case to begin with. `***REMOVED*** transform: postgres.fromCamel }` will convert camelCase only to snake_case.
...
const sql = postgres(***REMOVED*** transform: postgres.camel })

await sql`CREATE TABLE IF NOT EXISTS camel_case (a_test INTEGER, b_test TEXT)`
await sql`INSERT INTO camel_case $***REMOVED*** sql([***REMOVED*** aTest: 1, bTest: 1 }]) }`
const data = await sql`SELECT $***REMOVED*** sql('aTest', 'bTest') } FROM camel_case`

console.log(data) // [ ***REMOVED*** aTest: 1, bTest: '1' } ]
```
...
postgres(***REMOVED*** transform: postgres.toCamel })

await sql`CREATE TABLE IF NOT EXISTS camel_case (a_test INTEGER)`
await sql`INSERT INTO camel_case $***REMOVED*** sql([***REMOVED*** a_test: 1 }]) }`
const data = await sql`SELECT a_test FROM camel_case`

console.log(data) // [ ***REMOVED*** aTest: 1 } ]
```

```js
// Transform the column names only from camel case
// (for interpolated inserts, updates, and selects)
const sql = postgres(***REMOVED*** transform: postgres.fromCamel })

await sql`CREATE TABLE IF NOT EXISTS camel_case (a_test INTEGER)`
await sql`INSERT INTO camel_case $***REMOVED*** sql([***REMOVED*** aTest: 1 }]) }`
const data = await sql`SELECT $***REMOVED*** sql('aTest') } FROM camel_case`

console.log(data) // [ ***REMOVED*** a_test: 1 } ]
```

> Note that Postgres.js does not rewrite the static parts of the tagged template strings. So to transform column names in your queries, the `sql()` helper must be used - eg. `$***REMOVED*** sql('columnName') }` as in the examples above.
...
await sql`INSERT INTO transform_undefined $***REMOVED*** sql([***REMOVED*** a_test: undefined }]) }`
const data = await sql`SELECT a_test FROM transform_undefined`

console.log(data) // [ ***REMOVED*** a_test: null } ]
```
...
await sql`CREATE TABLE IF NOT EXISTS transform_undefined (a_test INTEGER)`
await sql`INSERT INTO transform_undefined $***REMOVED*** sql([***REMOVED*** aTest: undefined }]) }`
const data = await sql`SELECT $***REMOVED*** sql('aTest') } FROM transform_undefined`

console.log(data) // [ ***REMOVED*** aTest: null } ]
```
...
* `to`: The function to transform the outgoing query column name to, i.e `SELECT $***REMOVED*** sql('aName') }` to `SELECT a_name` when using `postgres.toCamel`.
...
function transformColumnToDatabase() ***REMOVED*** /* ... */ }
function transformColumnFromDatabase() ***REMOVED*** /* ... */ }
...
### Listen `await sql.listen(channel, onnotify, [onlisten]) -> ***REMOVED*** state }`
...
```js
await sql.listen('news', payload => ***REMOVED***
...
```js
sql.notify('news', JSON.stringify(***REMOVED*** no: 'this', is: 'news' }))
```
...
```js
const sql = postgres(***REMOVED*** publications: 'alltables' })

const ***REMOVED*** unsubscribe } = await sql.subscribe(
  'insert:events', 
  (row, ***REMOVED*** command, relation, key, old }) => ***REMOVED***
...
This is the state `***REMOVED*** pid, secret }` of the connection that executed the query.
...
const sql =
  process.env.NODE_ENV === 'production'
    ? // "Unless you're using a Private or Shield Heroku Postgres database, Heroku Postgres does not currently support verifiable certificates"
      ...
      postgres(***REMOVED*** ssl: ***REMOVED*** rejectUnauthorized: false } })
    : postgres()
```
...
> When there are high amount of concurrent queries, `postgres` will open as many connections as needed up until `max` number of connections is reached. By default `max` is 10. This can be changed by setting `max` in the `postgres()` call. Example - `postgres('connectionURL', ***REMOVED*** max: 20 })`.
...
const sql = postgres(***REMOVED***
  types: ***REMOVED***
    rect: ***REMOVED***
      ...
      serialize : (***REMOVED*** x, y, width, height }) => [x, y, width, height],

      // Function that transforms values coming from the db.
      parse     : ([x, y, width, height]) => ***REMOVED*** x, y, width, height }
  ***REMOVED***
***REMOVED***
})
...
const [custom] = sql`
  insert into rectangles (
    ...
    'wat',
    $***REMOVED*** sql.typed.rect(***REMOVED*** x: 13, y: 37, width: 42, height: 80 }) }
  )
  returning *
`

// custom = ***REMOVED*** name: 'wat', rect: ***REMOVED*** x: 13, y: 37, width: 42, height: 80 } }
...
const sql = postgres(***REMOVED***
  ...options,
  socket: (***REMOVED*** host: [host], port: [port] }) => new Promise((resolve, reject) => ***REMOVED***
...
Calling `sql.end()` will reject new queries and return a Promise which resolves when all queries are finished and the underlying connections are closed. If a `***REMOVED*** timeout }` option is provided any pending queries will be rejected once the timeout (in seconds) is reached and the connections will be destroyed.
...
prexit(async () => ***REMOVED***
  await sql.end(***REMOVED*** timeout: 5 })
...
This error is thrown for any queries that were pending when the timeout to [`sql.end(***REMOVED*** timeout: X })`](#teardown--cleanup) was reached.
...
```ts
const users = await sql<User[]>`SELECT * FROM users WHERE id = $***REMOVED***id}`
if (!users.length)
...
```ts
const [user]: [User?] = await sql`SELECT * FROM users WHERE id = $***REMOVED***id}`
if (!user) // => User | undefined
  throw new Error('Not found')
return user // => User

// NOTE:
const [first, second]: [User?] = await sql`SELECT * FROM users WHERE id = $***REMOVED***id}` // fails: `second` does not exist on `[User?]`
const [first, second] = await sql<[User?]>`SELECT * FROM users WHERE id = $***REMOVED***id}` // don't fail : `second: User | undefined`
```
