...
var request = forge.http.createRequest(***REMOVED***method: 'GET', path: url.path});
...
forge.ssh.getPublicKeyFingerprint(key, ***REMOVED***encoding: 'hex', delimiter: ':'});
```
...
function encrypt(password) ***REMOVED***
  var input = fs.readFileSync('input.txt', ***REMOVED***encoding: 'binary'});
...
***REMOVED***
  ...
  fs.writeFileSync('input.enc', output.getBytes(), ***REMOVED***encoding: 'binary'});
}

// openssl enc -d -des3 -in input.enc -out input.dec.txt
function decrypt(password) ***REMOVED***
  var input = fs.readFileSync('input.enc', ***REMOVED***encoding: 'binary'});
  ...
  fs.writeFileSync(
    'input.dec.txt', decipher.output.getBytes(), ***REMOVED***encoding: 'binary'});
}
```
...
var keypair = ed25519.generateKeyPair(***REMOVED***seed: seed});
...
var signature = ED25519.sign(***REMOVED***
  message: 'test',
...
var verified = ED25519.verify(***REMOVED***
  message: 'test',
  encoding: 'utf8',
...
var keypair = rsa.generateKeyPair(***REMOVED***bits: 2048, e: 0x10001});
...
rsa.generateKeyPair(***REMOVED***bits: 2048, workers: 2}, function(err, keypair) ***REMOVED***
...
// generate an RSA key pair asynchronously (uses web workers if available)
// use workers: -1 to run a fast core estimator to optimize # of workers
forge.rsa.generateKeyPair(***REMOVED***bits: 2048, workers: -1}, function(err, keypair) ***REMOVED***
  // keypair.privateKey, keypair.publicKey
});
...
// decrypt some bytes
var decipher = forge.cipher.createDecipher('AES-GCM', key);
decipher.start(***REMOVED***iv: iv, tag: tag});
...
pki.getPublicKeyFingerprint(key, ***REMOVED***type: 'SubjectPublicKeyInfo'});

// gets a hex-encoded, colon-delimited SHA-1 RSAPublicKey public key fingerprint
pki.getPublicKeyFingerprint(key, ***REMOVED***encoding: 'hex', delimiter: ':'});
...
var extensions = csr.getAttribute(***REMOVED***name: 'extensionRequest'}).extensions;
...
var encryptedPrivateKeyInfo = pki.encryptPrivateKeyInfo(
  privateKeyInfo, 'myCustomPasswordHere', ***REMOVED***
...
var pem = pki.encryptRsaPrivateKey(privateKey, 'password', ***REMOVED***legacy: true});
...
// get an attribute
csr.getAttribute(***REMOVED***name: 'challengePassword'});

// get extensions array
csr.getAttribute(***REMOVED***name: 'extensionRequest'}).extensions;
...
to `forge.pkcs12.toPkcs12Asn1` using the `***REMOVED***algorithm: '3des'}` option.
...
var bags = p12.getBags(***REMOVED***friendlyName: 'test'});
...
var bags = p12.getBags(***REMOVED***localKeyId: buffer});
// bags are key'd by attribute type (here "localKeyId")
// and the key values are an array of matching objects
var cert = bags.localKeyId[0];

// get bags by localKeyId (input in hex)
var bags = p12.getBags(***REMOVED***localKeyIdHex: '7b59377ff142d0be4565e9ac3d396c01401cd879'});
// bags are key'd by attribute type (here "localKeyId", *not* "localKeyIdHex")
// and the key values are an array of matching objects
var cert = bags.localKeyId[0];

// get bags by type
var bags = p12.getBags(***REMOVED***bagType: forge.pki.oids.certBag});
...
// get key bags
var bags = p12.getBags(***REMOVED***bagType: forge.pki.oids.keyBag});
// get key
