...
let parsedType;
try ***REMOVED***
    parsedType = catharsis.parse(type); // ***REMOVED***"type":"NameExpression,"name":"Object","nullable":false}
} catch(e) ***REMOVED***
...
let parsedJsdocType;
try ***REMOVED***
    parsedJsdocType = catharsis.parse(jsdocType, ***REMOVED***jsdoc: true});
} catch (e) ***REMOVED***
...
catharsis.stringify(parsedJsdocType, ***REMOVED***restringify: true});    // Array.<string>
...
+ 0.8.7 (June 2015):
    ...
    `***REMOVED***0: string}`) are now parsed correctly.
    + Record types with a property that contains a function, with no space after
    the preceding colon (for example, `***REMOVED***foo:function()}`), are now parsed
    correctly.
