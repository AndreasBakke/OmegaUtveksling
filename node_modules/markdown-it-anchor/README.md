...
const md = require('markdown-it')()
  .use(require('markdown-it-anchor'), ***REMOVED*** slugify })
```
...
const md = require('markdown-it')()
  .use(require('markdown-it-anchor'), ***REMOVED*** slugify: s => slugify(s) })
```
...
```js
function getTokensText (tokens) ***REMOVED***
  return tokens
...
const md = require('markdown-it')()
  .use(require('markdown-it-anchor'), ***REMOVED*** getTokensText })
```
...
```markdown
# Your title ***REMOVED***#your-custom-id}
```
...
```js
const ***REMOVED*** parse } = require('node-html-parser')

const root = parse(html)

for (const h of root.querySelectorAll('h1, h2, h3, h4, h5, h6')) ***REMOVED***
  const slug = h.getAttribute('id') || slugify(h.textContent)
  h.setAttribute('id', slug)
  h.innerHTML = `<a href="#$***REMOVED***slug}>$***REMOVED***h.innerHTML}</a>`
}
...
```js
h.insertAdjacentHTML('afterbegin', `<a class="anchor" aria-hidden="true" href="#$***REMOVED***slug}">🔗</a> `)
```
...
md.use(anchor, ***REMOVED***
  permalink: anchor.permalink.headerLink(***REMOVED*** safariReaderFix: true })
})
```
...
md.use(anchor, ***REMOVED***
  permalink: anchor.permalink.linkAfterHeader(***REMOVED***
    style: 'visually-hidden',
    assistiveText: title => `Permalink to “$***REMOVED***title}”`,
...
md.use(anchor, ***REMOVED***
  permalink: anchor.permalink.linkInsideHeader(***REMOVED***
    symbol: `
...
md.use(anchor, ***REMOVED***
  permalink: anchor.permalink.ariaHidden(***REMOVED***
    placement: 'before'
***REMOVED***)
})
```
...
```js
function renderPermalink (slug, opts, state, idx) ***REMOVED***}
```
