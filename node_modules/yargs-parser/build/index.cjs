...
function decamelize(str, joinString) ***REMOVED***
    ...
    for (let i = 0; i < str.length; i++) ***REMOVED***
        ...
        if (chrLower !== chrString && i > 0) ***REMOVED***
            notCamelcase += `$***REMOVED***joinString}$***REMOVED***lowercase.charAt(i)}`;
      ***REMOVED***
        else ***REMOVED***
...
function looksLikeNumber(x) ***REMOVED***
...
function tokenizeArgString(argString) ***REMOVED***
    if (Array.isArray(argString)) ***REMOVED***
  ...
  ***REMOVED***
    ...
    let opening = null;
    const args = [];
    for (let ii = 0; ii < argString.length; ii++) ***REMOVED***
        prevC = c;
...
})(DefaultValuesForTypeKey || (DefaultValuesForTypeKey = ***REMOVED***}));

let mixin;
class YargsParser ***REMOVED***
  ...
  ***REMOVED***
    parse(argsInput, options) ***REMOVED***
      ...
      ***REMOVED***);
        ...
        checkConfiguration();
        let notFlags = [];
        const argv = Object.assign(Object.create(null), ***REMOVED*** _: [] });
        const argvReturn = ***REMOVED***};
        for (let i = 0; i < args.length; i++) ***REMOVED***
            const arg = args[i];
            const truncatedArg = arg.replace(/^-***REMOVED***3,}/, '---');
            let broken;
            let key;
            let letters;
            let m;
      ...
      ***REMOVED***
        function processValue(key, val) ***REMOVED***
            if (typeof val === 'string' &&
                (val[0] === "'" || val[0] === '"') &&
                val[val.length - 1] === val[0]) ***REMOVED***
          ...
          ***REMOVED***
            let value = Array.isArray(val)
                ? val.map(function (v) ***REMOVED*** return maybeCoerceNumber(key, v); })
          ...
          ***REMOVED***
            if (checkAllAliases(key, flags.normalize) && checkAllAliases(key, flags.arrays)) ***REMOVED***
                if (Array.isArray(val))
                    value = val.map((val) => ***REMOVED*** return mixin.normalize(val); });
                else
      ...
      ***REMOVED***
        function maybeCoerceNumber(key, value) ***REMOVED***
            ...
            if (!checkAllAliases(key, flags.strings) && !checkAllAliases(key, flags.bools) && !Array.isArray(value)) ***REMOVED***
                const shouldCoerceNumber = looksLikeNumber(value) && configuration['parse-numbers'] && (Number.isSafeInteger(Math.floor(parseFloat(`$***REMOVED***value}`))));
      ...
      ***REMOVED***
        function defaultValue(key) ***REMOVED***
            if (!checkAllAliases(key, flags.bools) &&
                !checkAllAliases(key, flags.counts) &&
                `$***REMOVED***key}` in defaults) ***REMOVED***
                return defaults[key];
          ***REMOVED***
            else ***REMOVED***
...
if (process && process.version) ***REMOVED***
    ...
    if (major < minNodeVersion) ***REMOVED***
        throw Error(`yargs parser supports a minimum Node.js version of $***REMOVED***minNodeVersion}. Read our version support policy: https://github.com/yargs/yargs-parser#supported-nodejs-versions`);
  ***REMOVED***
}
