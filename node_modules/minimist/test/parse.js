...
test('flag boolean', function (t) ***REMOVED***
    var argv = parse([ '-t', 'moo' ], ***REMOVED*** boolean: 't' });
    t.deepEqual(argv, ***REMOVED*** t : true, _ : [ 'moo' ] });
...
test('strings' , function (t) ***REMOVED***
    var s = parse([ '-s', '0001234' ], ***REMOVED*** string: 's' }).s;
    t.equal(s, '0001234');
    t.equal(typeof s, 'string');
    
    var x = parse([ '-x', '56' ], ***REMOVED*** string: 'x' }).x;
    t.equal(x, '56');
...
test('stringArgs', function (t) ***REMOVED***
    var s = parse([ '  ', '  ' ], ***REMOVED*** string: '_' })._;
    t.same(s.length, 2);
...
test('empty strings', function(t) ***REMOVED***
    var s = parse([ '-s' ], ***REMOVED*** string: 's' }).s;
    t.equal(s, '');
    t.equal(typeof s, 'string');

    var str = parse([ '--str' ], ***REMOVED*** string: 'str' }).str;
    t.equal(str, '');
...
test('slashBreak', function (t) ***REMOVED***
    t.same(
        parse([ '-I/foo/bar/baz' ]),
        ***REMOVED*** I : '/foo/bar/baz', _ : [] }
    );
    t.same(
        parse([ '-xyz/foo/bar/baz' ]),
        ***REMOVED*** x : true, y : true, z : '/foo/bar/baz', _ : [] }
    );
    t.end();
});
...
test('nested dotted objects', function (t) ***REMOVED***
    ...
    t.same(argv.foo, ***REMOVED***
      ...
      ***REMOVED***
  ***REMOVED***);
    t.same(argv.beep, ***REMOVED*** boop : true });
    t.end();
});
