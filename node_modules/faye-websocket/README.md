...
var ws = new WebSocket.Client('ws://www.example.com/', [], ***REMOVED***
  proxy: ***REMOVED***
    ...
    headers: ***REMOVED*** 'User-Agent': 'node' },
    tls:     ***REMOVED*** cert: fs.readFileSync('client.crt') }
***REMOVED***
});
```
...
var ws = new WebSocket(request, socket, body, [], ***REMOVED*** extensions: [deflate] });
```
...
- `tls` - an object containing TLS settings for the origin server, this will be
  passed to
  [`tls.connect()`](https://nodejs.org/api/tls.html#tls_tls_connect_options_callback)
- `ca` - (legacy) a shorthand for passing `***REMOVED*** tls: ***REMOVED*** ca: value } }`
...
server.on('request', function(request, response) ***REMOVED***
  if (EventSource.isEventSource(request)) ***REMOVED***
    ...
    var loop = setInterval(function() ***REMOVED*** es.send('Hello') }, 1000);
...
```js
es.send('Breaking News!', ***REMOVED*** event: 'notification', id: '99' });
```
...
var es = new EventSource(request, response, ***REMOVED***
  headers: ***REMOVED*** 'Access-Control-Allow-Origin': '*' },
  ping:    15,
  retry:   10
});
```
